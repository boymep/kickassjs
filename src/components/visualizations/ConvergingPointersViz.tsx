...
function computeSteps(): Step[] {
  ...
  while (left < right) {
    ...
    if (sum === target) {
      result.push({ left, right, sum, found: true, description: `${arr[left]} + ${arr[right]} = ${sum} — пара найдена!` });
      break;
    } else if (sum < target) {
      result.push({ left, right, sum, found: false, description: `${arr[left]} + ${arr[right]} = ${sum} < ${target} — сдвигаем left вправо` });
      left++;
    } else {
      result.push({ left, right, sum, found: false, description: `${arr[left]} + ${arr[right]} = ${sum} > ${target} — сдвигаем right влево` });
      right--;
    }
  }
  return result;

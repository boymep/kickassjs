...
const steps: Step[] = [
  {
    start: 0, end: 2, sum: 8, maxSum: 8, isNewMax: true,
    addedIdx: null, removedIdx: null,
    formula: 'sum = arr[0] + arr[1] + arr[2] = 2 + 1 + 5 = 8',
    description: 'Считаем сумму первого окна — фаза набора',
  },
  {
    start: 1, end: 3, sum: 7, maxSum: 8, isNewMax: false,
    addedIdx: 3, removedIdx: 0,
    formula: 'sum = 8 + arr[3] − arr[0] = 8 + 1 − 2 = 7',
    description: 'Сдвиг окна: +arr[3], −arr[0]',
  },
  {
    start: 2, end: 4, sum: 9, maxSum: 9, isNewMax: true,
    addedIdx: 4, removedIdx: 1,
    formula: 'sum = 7 + arr[4] − arr[1] = 7 + 3 − 1 = 9',
    description: 'Сдвиг: +arr[4], −arr[1] → новый максимум',
  },
  {
    start: 3, end: 5, sum: 6, maxSum: 9, isNewMax: false,
    addedIdx: 5, removedIdx: 2,
    formula: 'sum = 9 + arr[5] − arr[2] = 9 + 2 − 5 = 6',
    description: 'Сдвиг: +arr[5], −arr[2]. Финальный ответ: 9',
  },
];
...
export default function FixedWindowViz() {

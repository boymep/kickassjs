...
const steps: Step[] = [
  {
    ...
    found: false,
    resultIndices: null,
    mapEntries: [{ key: 2, value: 0, isNew: true }],
    description: 'i = 0, num = 2, complement = 7. Map пуст → записываем 2 → 0',
  },
  {
    currentIdx: 1,
    num: 7,
    ...
    description: 'i = 1, num = 7, complement = 2. Map.has(2) = true → пара найдена: [0, 1]',
  },
];
...
export default function TwoSumLookupViz() {
  ...
  return (
    ...
    >
      <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
        Массив [{ARRAY.join(', ')}], target = <b>{TARGET}</b>
      </Typography>
            ...
            step.mapEntries.map((entry, i) => {
              ...
              return (
                ...
                >
                  ...
                  <Box>{entry.value}</Box>
                  <Box sx={{ fontSize: '0.7rem', color: isNew ? PALETTE.newColor : highlight ? PALETTE.foundColor : 'transparent', fontWeight: 600 }}>
                    {isNew ? 'новая' : highlight ? 'найдено' : ''}
                  </Box>
                </Box>
              );
            })

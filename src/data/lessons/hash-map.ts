...
export const hashMapLesson: Lesson = {
  ...
  intro: {
    whyItMatters: `Хеш-таблица даёт поиск, вставку и удаление за O(1) в среднем. В JavaScript это \`Map\`, \`Set\`, \`Object\` и \`WeakMap\` — все построены на одной идее: получить значение по ключу без перебора всей коллекции.

На собеседовании хеш-таблицы встречаются почти в каждой второй задаче среднего уровня: подсчёт частот, поиск пары с заданной суммой, группировка по признаку, проверка уникальности. Часто наивное решение O(n²) превращается в O(n) одной строкой — \`map.set(...)\`. Если в задаче встречаются слова «уже встречали», «дубликаты», «частоты», ответ почти наверняка начинается со \`new Map()\`.`,
    estimatedMinutes: 28,
    interviewAngle:
      'Интервьюера интересует не API \`Map\`, а понимание устройства: почему O(1) — амортизированная, а не гарантированная константа; чем \`Map\` отличается от \`Object\` на уровне движка; в каких случаях операции деградируют до O(n). Без этого ответ «использую Map» звучит как заученное правило, а не понимание.',
    prerequisites: [],
  },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Хеш-таблица — это массив бакетов плюс хеш-функция, которая превращает ключ в индекс бакета. Чтобы найти значение по ключу, считается хеш и берётся элемент по индексу. Получается O(1) — но только при условии, что хеш-функция распределяет ключи равномерно.',
        },
        {
          type: 'text',
          content:
            'Две разные строки-ключа могут получить один и тот же индекс — это коллизия. Избежать коллизий полностью нельзя: ключей бесконечно много, а индексов конечное число. Поэтому каждая реализация содержит механизм разрешения коллизий.',
        },
        {
          type: 'visualization',
          content: '',
...
Открытая адресация (open addressing): при коллизии ищется следующий свободный бакет. Лучше работает с кэшем процессора, но требует перестройки таблицы при загрузке выше 50–70 %.`,
        ...
        { type: 'heading', content: 'Почему O(1) — амортизированная' },
        {
          type: 'text',
          content:
            'При заполнении выше определённого порога (обычно ~75 %) движок выделяет массив большего размера и перехеширует все ключи — это O(n). Такая операция случается раз на n вставок, поэтому в среднем по большой серии операций цена остаётся константной. Но отдельная вставка в худшем случае может стоить O(n) — это иногда стреляет в high-latency системах.',
        },
      ],
      checkpoint: [Q['hm-q5']!, {
        type: 'multi-select',
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Два базовых паттерна с \`Map\`, которые закрывают добрую половину медиум-задач:\n- Быстрый поиск: проходим коллекцию один раз, для каждого элемента проверяем, есть ли в \`Map\` его «дополнение». Превращает O(n²) в O(n). Канонический пример — Two Sum.\n- Подсчёт частот: считаем, сколько раз встречается каждый элемент. Нужно для анаграмм, группировок, поиска уникальных.',
        },
        {
          type: 'text',
          content:
            'Триггер в формулировке: слова «найдите пару», «посчитайте», «уникальные», «дубликаты», «частоты» — почти всегда означают \`Map\` или \`Set\`.',
        },
        {
          type: 'visualization',
          content: '',
...
// Map(5) { 'a' => 5, 'b' => 2, 'r' => 2, 'c' => 1, 'd' => 1 }`,
        },
        {
          ...
          calloutType: 'warning',
          content:
            'Обращение к несуществующему ключу возвращает \`undefined\`, не \`0\`. Идиома \`(map.get(key) || 0) + 1\` — стандартный способ инкремента. Если значение 0 валидное и его нельзя «перезаписать», вместо \`||\` берётся \`??\`.',
        },
        ...
        },
        {
          type: 'text',
          content:
            'Когда задача звучит как «найдите пару элементов с суммой, разностью или произведением, равным X», работает один из двух подходов: бинпоиск, если массив отсортирован, или хеш-таблица за один проход.',
        },
        {
          type: 'code',
          language: 'javascript',
  ...
  return [];
        ...
        },
        {
          ...
          calloutType: 'tip',
          content:
            'Проверка \`seen.has(complement)\` идёт до записи текущего элемента. Это естественно решает случай с дубликатами: \`twoSum([3, 3], 6)\` возвращает \`[0, 1]\`, а не \`[0, 0]\`. Если перепутать порядок, тест с дубликатами развалится — частый промах на собеседовании.',
        },
      ],
...
// → [["eat","tea","ate"], ["tan","nat"], ["bat"]]`,
        },
        {
          ...
          calloutType: 'info',
          content:
            'В современных рантаймах появились встроенные \`Object.groupBy(items, fn)\` (ES2024) и \`Map.groupBy\` для случая, когда ключи — объекты. Шаблон выше остаётся универсальным и работает в более старых окружениях.',
        },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'В JavaScript три «хеш-подобные» структуры: \`Object\` (ключи — строки и символы, есть прототип), \`Map\` (любые ключи, гарантированный порядок, есть \`.size\`), \`WeakMap\` (слабые ключи, нет итерации). Понимание различий между ними — один из самых частых вопросов на JavaScript-собеседовании.',
        },
...
console.log(map.get(1));          // 'one'`,
        ...
        { type: 'heading', content: 'WeakMap — ассоциация без удержания' },
        {
          type: 'text',
          content:
            'У \`WeakMap\` ключи — только объекты, и удерживаются слабо. Если на ключ-объект больше нет ссылок снаружи, сборщик мусора уберёт его, а запись из \`WeakMap\` исчезнет сама. Удобно, когда нужно прикрепить к объекту приватные данные, не мешая его сборке.',
        },
        {
          type: 'code',
          language: 'javascript',
...
// запись из cache исчезнет автоматически — утечки нет.`,
        },
        {
          ...
          calloutType: 'warning',
          content:
            'У \`WeakMap\` нет \`.size\`, итераторов и метода \`clear\`. Это не упущение — иначе видимость ключей мешала бы сборщику мусора. Если нужно перебрать все элементы — берётся обычный \`Map\`, и за утечки приходится отвечать самостоятельно.',
        },
      ],
      checkpoint: [Q['hm-q15']!],
    },
...
console.log(dict.toString); // undefined`,
        ...
        { type: 'heading', content: 'Деградация O(1) до O(n)' },
        {
          type: 'text',
          content:
            'В худшем случае все ключи попадают в один бакет, и операции деградируют до O(n). На практике в V8 это маловероятно — хеш-функция инициализируется случайным значением при старте процесса. Но если в задаче явно упомянуты специально подобранные ключи (hash flooding-атака) — эту деградацию стоит назвать вслух, интервьюер именно её и проверяет.',
        },
      ],
      playground: {

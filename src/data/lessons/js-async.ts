      ...
      blocks: [
        {
          type: 'text',
          content:
            '**Promise** — объект-обёртка над **будущим** результатом асинхронной операции. Три состояния:\n\n- **`pending`** — ожидание (начальное)\n- **`fulfilled`** — успех со значением\n- **`rejected`** — ошибка с причиной\n\nПереход из `pending` — **однократный и необратимый**: промис «оседает» ровно один раз.',
        },
        {
          type: 'code',
          language: 'javascript',
...
promise
  ...
  .finally(() => hideSpinner());              // выполнится в любом случае`,
        },
        {
          type: 'text',
          content:
            'На собеседовании часто спрашивают разницу между `.then().catch()` и `try/catch` с `await` — это разные способы обработки ошибок с немного разным поведением. Ещё одна классическая задача: «что выведет этот код?» с микс промисов и `setTimeout`. Ключ — промисы используют очередь микрозадач и всегда выполняются раньше таймеров.',
        },
        {
          type: 'callout',
          calloutType: 'tip',
...
- возврат уже отклонённого промиса из callback.`,
      ...
      checkpoint: [Q['jsa-q3']!, Q['jsa-q1']!, {
        type: 'ordering',
        id: 'jsa-ord1',
        description: 'Расставь в порядке выполнения event loop',
        items: [
          'Синхронный код (call stack)',
          'Microtask queue (Promise.then, queueMicrotask)',
          'Macrotask queue (setTimeout, setInterval)',
        ],
        explanation: 'Event loop сначала выполняет весь синхронный код, затем полностью опустошает microtask queue (включая задачи добавленные другими microtask), и только потом берёт следующую macrotask.',
      }],
      video: {
        source: 'youtube',
        id: 'PoRJizFvM7s',
        ...
        language: 'en',
        durationSec: 58 * 60,
        description: 'Глубокий разбор цикла Promise: pending/fulfilled/rejected, цепочки then, обработка ошибок — с примерами в DevTools.',
      },
      links: [
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Promise',
          title: 'Promise — MDN (ru)',
          source: 'mdn',
          language: 'ru',
          note: 'Каноническая справка: конструктор, методы экземпляра, состояния Promise.',
        },
      ],
      ...
      blocks: [
        {
          type: 'text',
          content:
            '`async/await` — это синтаксический сахар поверх промисов. Функция с ключевым словом `async` всегда возвращает промис. `await` внутри неё приостанавливает выполнение функции (не весь код!) до разрешения промиса — и возвращает результат. Код выглядит синхронным, хотя работает асинхронно. Это главная причина его популярности.',
        },
        {
          type: 'text',
          content:
  ...
  try {
    ...
    hideSpinner();
        ...
        },
        {
          type: 'text',
          content:
            'Главная ловушка: `await` работает только ВНУТРИ async-функции. Поставить `await` снаружи или в обычном колбэке — это распространённая ошибка новичков. Ещё одна классика на собеседовании: последовательные `await` вместо `Promise.all` когда запросы независимы — это в несколько раз медленнее и интервьюер точно спросит об оптимизации.',
        },
...
async function fast() {
  ...
  return [a, b];               // итого ~100 мс
      ...
      links: [
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Statements/async_function',
          title: 'async function — MDN (ru)',
          source: 'mdn',
          language: 'ru',
          note: 'Полная справка: синтаксис, возвращаемое значение, обработка ошибок через try/catch.',
        },
        {
          ...
          source: 'mdn',
          language: 'ru',
          note: 'Оператор await: что происходит под капотом, top-level await, ожидание не-Promise.',
        },
      ],
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Когда надо запустить несколько асинхронных операций сразу, на помощь приходят четыре статических метода Promise. Каждый решает свою задачу: `all` ждёт всех, `allSettled` собирает итоги даже при ошибках, `race` берёт первый ответ, `any` берёт первый успешный. Правильный выбор между ними — это отдельный вопрос на собеседовании.',
        },
        {
          type: 'text',
          content:
...
await withTimeout(fetch('/slow'), 5000);`,
        },
        {
          type: 'text',
          content:
            'Типичная ловушка: `Promise.all` упадёт при первой же ошибке — остальные результаты потеряются. Если тебе нужен отчёт по каждому запросу, используй `allSettled`. Интервьюер любит спрашивать: «Как загрузить данные из нескольких источников и показать что получилось, а что нет?» — и ждёт именно `allSettled`.',
        },
...
try {
  ...
  console.log(err.errors); // массив reasons всех rejected
      ...
      checkpoint: [Q['jsa-q2']!, Q['jsa-q9']!, {
        type: 'multi-select',
        id: 'jsa-ms1',
        description: 'Какие из этих методов всегда возвращают новый Promise? (выбери все верные ответы)',
        options: [
          'Promise.then()',
          'Promise.catch()',
          'Promise.finally()',
        ...
        explanation: 'Все четыре метода возвращают новый Promise. `.then()`, `.catch()`, `.finally()` создают промис-цепочку. `Promise.resolve()` оборачивает значение в уже выполненный промис.',
      }],
      links: [
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Promise/allSettled',
          title: 'Promise.allSettled — MDN (ru)',
          source: 'mdn',
          language: 'ru',
          note: 'Отличие от Promise.all: возвращает статус каждого промиса, не прерывается при ошибке.',
        },
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Promise/race',
          title: 'Promise.race — MDN (ru)',
          source: 'mdn',
          language: 'ru',
          note: 'Паттерн timeout через Promise.race — пример в справке.',
        },
      ],
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Асинхронные ошибки коварны тем что код компилируется без ошибок, тесты могут пройти, а баг проявляется только под нагрузкой или в определённом порядке запросов. Четыре самых частых: забыл `await` — функция запустилась но результат потерян. Написал `async` в `forEach` — думаешь что ждёшь, а не ждёшь. Создал промис без `.catch` — ошибки проглочены. Поставил `try/catch` — но ошибка в другом месте.',
        },
        {
          type: 'text',
          content:
            'Асинхронный код полон ловушек, которые сложно заметить при беглом просмотре — всё выглядит правильно, но работает не так. Самые частые: забытый `await`, `async` внутри `forEach`, проглоченный rejected-промис и `try/catch` который не ловит то что ты думаешь. Проверь что ты знаешь все четыре.',
        },
...
await Promise.all(items.map((item) => process(item)));`,
        },
        {
          type: 'text',
          content:
            'Аналогия с `forEach` и `async`: представь что ты раздаёшь задания десяти сотрудникам, не дожидаясь ответа от каждого, и сразу говоришь «всё сделано». Но ничего ещё не сделано — сотрудники только начали. `forEach` ведёт себя именно так: запускает все колбэки не дожидаясь ни одного. Если нужен контроль — `Promise.all` с `map`, если нужна очередь — `for...of` с `await`.',
        },
        {
          type: 'callout',
          calloutType: 'tip',
        ...
        },
        {
          type: 'text',
          content:
            'Вопрос про `async` внутри `forEach` — один из самых частых на собеседованиях по JS. Код выглядит как будто ждёт завершения, а на деле всё запускается параллельно и не ждётся. Правило простое: `forEach` не async-aware. Для последовательности — `for...of` с `await`, для параллельного с ожиданием — `Promise.all` с `map`.',
        },
  ...
  await new Promise((_, reject) => {
    setTimeout(() => reject(new Error('boom')), 0);
      ...
      video: {
        ...
        language: 'en',
        durationSec: 35 * 60,
        description: 'Разбор микрозадач, почему Promise.then выполняется раньше setTimeout, и как цепочка промисов влияет на event loop.',
      },
      links: [
        {
          url: 'https://learn.javascript.ru/microtask-queue',
          title: 'Микрозадачи — learn.javascript.ru',
          source: 'learn-js',
          language: 'ru',
          note: 'Очередь микрозадач: когда опустошается, как связана с промисами и event loop.',
        },
      ],
      ...
      blocks: [
        {
          type: 'text',
          content:
            'В продакшне недостаточно просто сделать запрос и ждать ответа. Сервер может не отвечать — нужен timeout. Ответ может упасть из-за временной ошибки — нужен retry. Сотня одновременных запросов может положить API — нужен rate limit. Это три паттерна которые делают асинхронный код устойчивым, и на собеседовании уровня middle+ обязательно спросят хотя бы один из них.',
        },
        {
          type: 'text',
          content:
            'Реальный асинхронный код требует трёх паттернов: timeout (запрос завис — прерываем), retry (запрос упал — пробуем снова), limit (не спамим сервер сотнями запросов одновременно). Все три реализуются через промисы и `await` — без сторонних библиотек. На собеседовании могут попросить написать любой из них.',
        },
...
await retry(() => fetch('/flaky'), { attempts: 5 });`,
        },
        {
          type: 'text',
          content:
            'Аналогия: timeout через `Promise.race` — это как поставить будильник. Если ты не вернулся до звонка — уходишь без результата. Но твой коллега который идёт за кофе всё ещё идёт — ты просто перестал его ждать. Для настоящей отмены нужен `AbortController` — это уже как позвонить коллеге и сказать «возвращайся, кофе не нужен». Интервьюер-сениор точно спросит об этом различии.',
        },
        {
          type: 'text',
          content:
            'Timeout через `Promise.race` — красивый трюк, но с подводным камнем: исходная операция не отменяется, она продолжает работать в фоне и просто никто не читает результат. Для настоящей отмены нужен `AbortController`. Интервьюер-сениор точно спросит об этом различии — знай что ответить.',
        },
      ...
      links: [
        {
          ...
          source: 'mdn',
          language: 'en',
          note: 'Отмена fetch-запросов через AbortSignal — паттерн из главы advanced patterns.',
        },
        {
          url: 'https://www.patterns.dev/vanilla/promise-based-patterns',
          title: 'Promise-based patterns — Patterns.dev',
          source: 'article',
          language: 'en',
          note: 'Паттерны concurrency limiter, retry, cache — на основе Promise.',
        },
      ],
      docsLink: { url: 'https://learn.javascript.ru/async-await', title: 'async/await — learn.javascript.ru' },
      playground: {

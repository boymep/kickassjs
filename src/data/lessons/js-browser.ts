      ...
      blocks: [
        {
          type: 'text',
          content:
            'Прежде чем ты увидишь что-либо на экране, браузер проходит цепочку шагов — **Critical Rendering Path** (критический путь рендеринга). Каждый шаг блокируется предыдущим, и любой может стать узким местом производительности.',
        },
        {
          type: 'callout',
          calloutType: 'tip',
          content:
            '**CSS и синхронные скрипты в `<head>`** блокируют рендер — пользователь видит белый экран пока они не загрузятся. На собеседовании обязательно спросят про `defer` vs `async` — у них разное поведение и разный порядок выполнения.',
        },
        {
          type: 'list',
...
6. **Composite.** Слои собираются в финальное изображение, чаще всего на GPU.`,
  ...
  <link rel="dns-prefetch" href="https://cdn.example.com">
        ...
        },
        {
          type: 'text',
          content:
            'Ключевое различие defer и async: defer сохраняет порядок выполнения скриптов и запускает их после парсинга HTML — удобно для зависимых скриптов. async выполняется как только загрузился — порядок не гарантирован. Для приложения обычно нужен defer. Для независимых инструментов вроде аналитики — async. preload не выполняет скрипт — только загружает ресурс заранее с высоким приоритетом.',
        },
        {
          type: 'callout',
          calloutType: 'tip',
      ...
      checkpoint: [Q['jsbr-q8']!, {
        type: 'ordering',
        id: 'jsbr-ord1',
        description: 'Расставь этапы загрузки страницы в браузере по порядку',
        items: [
          'DNS resolution: домен → IP-адрес',
          'TCP handshake + TLS (для HTTPS)',
          'HTTP запрос и получение HTML',
          'Parsing HTML, построение DOM',
          'Загрузка и парсинг CSS, CSSOM',
          'Layout (Reflow): вычисление размеров и позиций',
          'Paint + Composite: пиксели на экране',
        ],
        explanation:
          'Каждый этап критичен для производительности. DNS и TCP — сетевые задержки. Blocking resources (синхронные скрипты, CSS) останавливают парсинг. Layout и Paint — дорогие операции, которые триггерят reflow.',
      }],
      ...
      video: {
        ...
        language: 'en',
        durationSec: 6 * 60,
        description: '6-минутный обзор: HTML → DOM → CSSOM → Render Tree → Paint. Хороший разогрев перед темой.',
      },
      links: [
        {
          ...
          source: 'web-dev',
          language: 'en',
          note: 'Детальный гайд по каждому шагу CRP с диаграммами и примерами.',
        },
      ],
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Любое изменение DOM запускает **часть** Critical Rendering Path заново. Три уровня по убыванию стоимости:\n- **Reflow (Layout)** — пересчёт геометрии: ширина, высота, отступы, шрифт, добавление узлов\n- **Repaint** — перерисовка пикселей без изменения геометрии: цвет, фон, тени\n- **Composite** — только `transform` и `opacity` (GPU-слой, самый дешёвый)\n\nНа собеседовании по производительности спросят: **«какие CSS-свойства дешёвые для анимации?»** — ответ: только `transform` и `opacity`. Всё остальное дороже.',
        },
        {
          type: 'list',
...
el.style.opacity = '0.5';`,
        },
        {
          type: 'text',
          content:
            'Подводный камень: display: none вызывает reflow, а visibility: hidden — только repaint. Это часто удивляет. display: none полностью убирает элемент из потока — браузер пересчитывает layout. visibility: hidden только скрывает пиксели, элемент занимает место как раньше — только repaint. Если тебе нужно скрыть без reflow — visibility или opacity, не display.',
        },
  ...
  elements.forEach((el, i) => {
    el.style.width = (widths[i] + 10) + 'px';          // WRITE-фаза
      ...
      checkpoint: [Q['jsbr-q1']!, Q['jsbr-q2']!, {
        type: 'multi-select',
        id: 'jsbr-ms1',
        description: 'Что блокирует построение DOM (останавливает HTML parser)?',
        options: [
        ...
        explanation:
          'Синхронный `<script>` блокирует парсер — браузер ждёт загрузки и выполнения. CSS тоже блокирует: парсер ждёт CSSOM перед выполнением скриптов. `<img>` не блокирует парсинг. `defer` — скрипт загружается параллельно, выполняется после парсинга.',
      }],
      docsLink: { url: 'https://developer.mozilla.org/ru/docs/Web/Performance/Critical_rendering_path', title: 'Critical Rendering Path — MDN (ru)' },
      links: [
        {
          ...
          source: 'web-dev',
          language: 'en',
          note: 'Пиксельный конвейер, что вызывает reflow/repaint/composite и как профилировать.',
        },
        {
          url: 'https://csstriggers.com/',
          title: 'CSS Triggers — какие свойства вызывают reflow/repaint',
          source: 'other',
          language: 'en',
          note: 'Справочник: для каждого CSS-свойства показано, запускает ли оно Layout, Paint или Composite.',
        },
      ],
      playground: {
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Изображения — 50–70% общего веса страницы. Современный стек: **AVIF/WebP** вместо JPEG, **`srcset` + `sizes`** для адаптивных размеров, **`loading="lazy"`** для офф-скрин картинок и **`fetchpriority="high"`** для LCP-изображения.',
        },
        {
          type: 'callout',
          calloutType: 'warning',
          content:
            'Классический подводный камень: `loading="lazy"` на LCP-картинке ухудшит LCP — браузер откладывает загрузку самого важного изображения. LCP-картинка должна иметь `fetchpriority="high"` и **никакого** `lazy`. Ещё ловушка: картинки без `width`/`height` атрибутов вызывают CLS (прыжки макета).',
        },
        {
          type: 'code',
          language: 'html',
...
<picture>
  ...
  <img src="photo.jpg" alt="photo" loading="lazy" width="800" height="600">
        ...
        },
        {
          type: 'text',
          content:
            'Тег picture с несколькими source — это graceful degradation: браузер выбирает первый поддерживаемый формат сверху вниз. AVIF поддерживается не всеми — Safari добавил поддержку только в 2022. WebP поддерживается 96% браузеров. img в конце — это fallback для старых браузеров. srcset позволяет браузеру самому выбрать подходящий размер для текущего экрана, DPR и скорости сети.',
        },
        {
          type: 'list',
...
document.querySelectorAll('img[data-src]').forEach((img) => observer.observe(img));`,
      ...
      links: [
        {
          url: 'https://web.dev/articles/use-srcset-to-automatically-choose-the-right-image',
          title: 'Responsive images with srcset — web.dev',
          source: 'web-dev',
          language: 'en',
          note: 'Как srcset + sizes выбирают правильный размер изображения для каждого устройства.',
        },
        {
          url: 'https://web.dev/articles/cls',
          title: 'Cumulative Layout Shift (CLS) — web.dev',
          source: 'web-dev',
          language: 'en',
          note: 'Как CLS связан с изображениями без width/height и как это исправить.',
        },
      ],
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'На 60 Гц мониторе у вас **~16.6 мс** на JS + layout + paint + composite. Промахнулись — пользователь видит «рывок» (jank). Два правила: использовать **дешёвые** свойства (`transform`, `opacity`) и **синхронизироваться** с кадрами через `requestAnimationFrame`.',
        },
        {
          type: 'callout',
          calloutType: 'tip',
          content:
            'Для простых анимаций предпочитайте **CSS transitions/animations** — они работают на compositor thread и не зависят от занятости главного потока. `requestAnimationFrame` нужен только для динамической логики: физика, игровой цикл, зависимость от пользовательского ввода.',
        },
        {
          type: 'code',
          language: 'javascript',
...
self.onmessage = (e) => {
  ...
  self.postMessage({ result });
      ...
      video: {
        ...
        language: 'en',
        durationSec: 35 * 60,
        description: 'Доклад инженера Chrome про event loop, microtask checkpoint и шаг рендеринга — когда браузер рисует кадр.',
      },
      links: [
        {
          ...
          source: 'web-dev',
          language: 'en',
          note: 'Объясняет, почему transform и opacity не вызывают reflow, и как правильно использовать rAF для плавных анимаций.',
        },
      ],
    },

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'На собеседовании по производительности спросят про все три метрики. Запомни аббревиатуры и цели: LCP < 2.5 с (когда появляется главный контент), INP < 200 мс (как быстро страница реагирует на клик), CLS < 0.1 (не прыгает ли макет). Самая частая ловушка: разработчики ставят loading="lazy" на LCP-картинку и этим ухудшают главную метрику — браузер откладывает загрузку самого важного элемента страницы.',
        },
        {
          type: 'list',
...
// onLCP(sendToAnalytics);`,
        ...
        { type: 'heading', content: 'Как улучшать каждую из метрик' },
        {
          type: 'text',
          content:
            'Хитрость с CLS: эта метрика портится незаметно. Картинка без width/height — браузер не знает её размер до загрузки и не резервирует место. Когда картинка загружается, весь текст под ней прыгает вниз. Пользователь уже начал читать — его глаза теряют позицию. Google считает это плохим UX и штрафует в ранжировании. Одна строка width и height на каждом img — и CLS обнуляется.',
        },
        {
          type: 'list',
      ...
      links: [
        {
          ...
          source: 'web-dev',
          language: 'en',
          note: 'Как измерять LCP и что именно его улучшает.',
        },
        {
          url: 'https://web.dev/articles/optimize-inp',
          title: 'Optimize INP — web.dev',
          source: 'web-dev',
          language: 'en',
          note: 'INP заменил FID в 2024: стратегии разбиения long tasks и приоритизации ввода.',
        },
      ],
    },
  ],

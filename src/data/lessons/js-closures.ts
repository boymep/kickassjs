...
export const jsClosuresLesson: Lesson = {
  ...
  intro: {
    whyItMatters: `Замыкание — функция, которая сохраняет доступ к переменным области, где она была создана, даже после того как внешняя функция завершилась. На замыканиях стоит огромная часть JS-экосистемы: хуки React, обработчики событий, мемоизация, debounce, реализация приватного состояния.

На собеседовании спрашивают не определение, а последствия: что выведет код с \`var\` в цикле, как замыкание удерживает объекты в памяти, чем приватность через замыкание отличается от приватных полей класса, как написать \`memoize\` или \`once\` с нуля. Это одна из тем, где без подготовки легко ошибиться на коде с \`setTimeout\` внутри цикла.`,
    estimatedMinutes: 30,
    interviewAngle:
      'На собеседовании важно показать не определение, а последствия: жизненный цикл захваченных переменных, влияние на сборку мусора, разница между \`var\` и \`let\` в цикле, реализация классических паттернов без библиотек. Сильный ответ отличается от слабого тем, что объясняет, почему результат именно такой, а не ограничивается фразой «это замыкание».',
    prerequisites: [],
  },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Лексическая область видимости означает, что доступность переменных определяется в момент написания кода, а не вызова. Функция, объявленная внутри другой, автоматически видит переменные внешней — независимо от того, откуда её потом позовут.',
        },
        {
          type: 'text',
          content:
            'Цепочка областей видимости работает так: при поиске переменной движок сначала проверяет локальную область, затем внешнюю, затем глобальную. Важно: имеет значение место объявления функции, а не место её вызова. Этот момент часто проверяют каверзными примерами.',
        },
        {
          type: 'code',
          language: 'javascript',
...
console.log(x); // 'global'`,
        },
        {
          ...
          calloutType: 'info',
          content:
            'Уровни областей видимости: global → module → function → block (let / const). \`var\` игнорирует блочную область — переменные «всплывают» до ближайшей функции, и это самый частый источник сюрпризов.',
        },
        {
          type: 'code',
          language: 'javascript',
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Замыкание — функция, которая помнит переменные из области, где была создана, даже после завершения внешней функции. Захват идёт по ссылке, а не по значению: если переменная изменится, замыкание увидит новое значение. Это маленькое уточнение и есть источник большинства багов с замыканиями.',
        },
        {
          type: 'visualization',
          content: '',
...
console.log(counter.count); // undefined`,
        },
        {
          type: 'text',
          content:
            'Каждый вызов \`makeCounter\` создаёт отдельное окружение. Два счётчика, созданные подряд, не делят состояние — это и делает замыкание удобным способом инкапсуляции до того, как в JS появились приватные поля \`#field\`.',
        },
        {
          type: 'code',
          language: 'javascript',
...
console.log(counter.get()); // 2`,
        },
        {
          type: 'text',
          content:
            'В эпоху ES-модулей потребность в IIFE сильно упала — каждый модуль и так имеет собственную область видимости. Но IIFE остаются удобны для разовой инициализации значения и для эмуляции top-level await там, где он не поддерживается.',
        },
      ],
      playground: {

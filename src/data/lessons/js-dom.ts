      ...
      blocks: [
        {
          type: 'text',
          content:
            '**DOM** (Document Object Model) — **дерево объектов**, которое браузер строит из HTML. Каждый тег становится узлом (`Element`, `Text`, `Comment`). JavaScript видит DOM через `document` и может читать и менять его в любой момент.',
        },
        {
          type: 'callout',
          calloutType: 'warning',
          content:
            'Работа с DOM — **самая дорогая операция** в браузере. Каждое изменение может вызвать пересчёт стилей, layout и перерисовку. На собеседовании спросят об оптимизации: batch update, `DocumentFragment`, `requestAnimationFrame`. Частый вопрос — разница между статическим `NodeList` от `querySelectorAll` и живой `HTMLCollection` от `getElementsByClassName`.',
        },
        {
          type: 'callout',
          calloutType: 'info',
...
div.classList.contains('active'); // true`,
        },
        {
          type: 'text',
          content:
            'Запомни различие между textContent и innerHTML: первый просто вставляет текст — теги становятся литералами. Второй парсит строку как HTML. Если ты когда-либо вставишь в innerHTML пользовательский ввод без экранирования — это XSS-уязвимость. Правило простое: для текста всегда textContent, для разметки — только если уверен в источнике или используешь DOMPurify.',
        },
        {
          type: 'callout',
          calloutType: 'tip',
...
Node.COMMENT_NODE;  // 8`,
      ...
      video: {
        ...
        language: 'en',
        durationSec: 18 * 60,
        description: 'Полный обзор addEventListener, фаз capture/bubble, опций once/passive и делегирования с примерами.',
      },
      links: [
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/API/Document_Object_Model/Introduction',
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Событие в DOM проходит **три фазы**: **capture** — от `window` вниз к цели, **target** — на самой цели, **bubble** — обратно вверх до `window`. По умолчанию `addEventListener` ловит события на фазе **bubble**.\n\nЧастый вопрос на собеседовании: в чём разница между `event.target` и `event.currentTarget`? `target` — элемент, **на котором произошло** событие; `currentTarget` — элемент, **на котором висит** обработчик. При всплытии они разные.',
        },
        {
          type: 'code',
          language: 'javascript',
...
el.addEventListener('click', fn, {
  ...
  signal: ac.signal, // отвязать через AbortController
        ...
        },
        {
          type: 'text',
          content:
            'Опция passive: true особенно важна для touch-событий на мобильных устройствах. По умолчанию браузер ждёт пока выполнится обработчик touchstart — вдруг ты вызовешь preventDefault и отменишь скролл. Это задержка в 50–300 мс на каждый тач-ивент. passive: true говорит браузеру "я не буду отменять дефолтное поведение" — скролл становится мгновенным. Chrome даже предупреждает в консоли если не указать passive для scroll-событий.',
        },
        {
          type: 'callout',
          calloutType: 'info',
...
// Кликают по link → выводится только 'clicked'`,
      ...
      checkpoint: [Q['jsdom-q1']!, Q['jsdom-q6']!, Q['jsdom-q4']!, {
        type: 'ordering',
        id: 'jsdom-ord1',
        description: 'Расставь фазы прохождения DOM-события в правильном порядке',
        items: [
          'Capturing phase: событие идёт от window вниз к target',
          'Target phase: событие достигло целевого элемента',
          'Bubbling phase: событие всплывает от target вверх к window',
        ],
        explanation:
          'DOM события проходят три фазы: сначала capturing (сверху вниз), потом target, потом bubbling (снизу вверх). addEventListener по умолчанию слушает bubbling; передай {capture: true} чтобы поймать на пути вниз.',
      }],
      docsLink: { url: 'https://learn.javascript.ru/bubbling-and-capturing', title: 'Всплытие и перехват — learn.javascript.ru' },
      links: [
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/API/Event',
          title: 'Event: target, stopPropagation, preventDefault — MDN',
          source: 'mdn',
          language: 'ru',
          note: 'Если нужно разобраться со свойствами Event — target vs currentTarget, bubbles, composed — всё здесь.',
        },
      ],
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            '**Делегирование событий** — один обработчик на общем предке обслуживает события от множества дочерних элементов. Работает благодаря всплытию: клик на `<li>` всплывает вверх до `<ul>` с обработчиком.',
        },
        {
          type: 'callout',
          calloutType: 'tip',
          content:
            'На собеседовании это классическая задача с `closest()`. Главное преимущество делегирования: обработчик **автоматически работает для новых элементов** — добавили `<li>` динамически, переподписываться не нужно. Для таблицы на 10 000 строк это разница между одним обработчиком и 10 000 замыканиями в памяти.',
        },
        {
          type: 'list',
...
document.querySelector('.button-group').addEventListener('click', (e) => {
  ...
  handleClick(btn);
        ...
        },
        {
          type: 'text',
          content:
            'Зачем closest вместо matches? Если внутри кнопки лежит иконка-span, то event.target будет тот самый span — и matches(".btn") вернёт false. closest же идёт вверх по дереву от target и найдёт ближайшую кнопку. Это классический подводный камень: люди пишут e.target.matches(".btn"), кликают по иконке внутри кнопки — и обработчик не срабатывает.',
        },
        {
          type: 'callout',
          calloutType: 'tip',
...
span.dispatchEvent(new MouseEvent('click', { bubbles: true }));`,
      ...
      checkpoint: [Q['jsdom-q9']!, {
        ...
        pairs: [
          { left: 'event.stopPropagation()', right: 'Останавливает дальнейшее всплытие/capturing' },
          { left: 'event.preventDefault()', right: 'Отменяет действие браузера по умолчанию' },
          { left: 'event.target', right: 'Элемент, на котором произошло событие' },
          { left: 'event.currentTarget', right: 'Элемент, на котором висит обработчик' },
        ],
        explanation:
          'target — это всегда тот элемент куда кликнули. currentTarget — тот где addEventListener. При всплытии они расходятся: target не меняется, currentTarget обновляется на каждом уровне.',
      }],
      docsLink: { url: 'https://learn.javascript.ru/event-delegation', title: 'Делегирование событий — learn.javascript.ru' },
      video: {
        source: 'youtube',
        id: 'YL1F4dCUlLc',
        title: 'How LocalStorage and Event Delegation work — JS30 #15',
        channel: 'Wes Bos',
        language: 'en',
        durationSec: 21 * 60,
        description: 'Live-кодинг: один обработчик на родителе работает для динамически добавленных элементов.',
      },
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            '**CustomEvent** — встроенный способ создать собственные события и использовать DOM как шину сообщений. Поле `detail` несёт произвольные данные, флаги `bubbles` и `cancelable` управляют распространением.',
        },
        {
          type: 'text',
          content:
            '**MutationObserver** — асинхронный наблюдатель за изменениями DOM. Колбэк вызывается на микрозадачах, браузер сам батчит несколько мутаций в один вызов. Используется в полифиллах, отладчиках и фреймворках для отслеживания изменений.',
        },
        {
          type: 'code',
          language: 'javascript',
...
document.dispatchEvent(event);`,
        },
        {
          type: 'text',
          content:
            'Обрати внимание на bubbles: true в примере — это означает что событие всплывёт по дереву и его можно поймать на любом предке. Без этого флага событие остаётся только на том элементе где было диспатчено. Для шины сообщений удобно диспатчить на document — тогда слушатель можно повесить в любом месте приложения.',
        },
        {
          type: 'text',
          content:
...
observer.disconnect(); // когда больше не нужен`,
      ...
      links: [
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/API/MutationObserver',
          title: 'MutationObserver — MDN',
          source: 'mdn',
          language: 'ru',
          note: 'Справочник по observe, disconnect и MutationRecord с примерами.',
        },
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/API/IntersectionObserver',
          title: 'IntersectionObserver — MDN',
          source: 'mdn',
          language: 'ru',
          note: 'Для lazy-load и бесконечной прокрутки — эффективнее обработчика scroll.',
        },
      ],
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Каждое изменение DOM может вызвать **reflow** (пересчёт раскладки) и **repaint** (перерисовку). Когда ты читаешь `offsetHeight` или `getBoundingClientRect` после изменений стилей — браузер пересчитывает layout синхронно прямо сейчас. Это называется **forced synchronous layout**.',
        },
        {
          type: 'callout',
          calloutType: 'warning',
          content:
            '**Layout Thrashing** — чередование чтений и записей в цикле: браузер пересчитывает layout на каждой итерации вместо одного раза. Решение простое: **сначала все чтения — потом все записи**. На собеседовании по производительности это спрашивают в первую очередь.',
        },
...
elements.forEach((el, i) => {
  el.style.height = heights[i] * 2 + 'px';             // батч записей
        ...
        },
        {
          type: 'text',
          content:
            'Правило "сначала читай — потом пиши" кажется очевидным, но на практике его нарушают постоянно. Особенно когда код разнесён по разным функциям — одна читает offsetHeight и передаёт дальше, другая меняет стили. Это всё равно thrashing. DevTools показывают такие места как "Forced reflow" фиолетовым цветом в Performance-панели — это хороший способ найти проблему.',
        },
        {
          type: 'callout',
          calloutType: 'warning',
...
console.log(elems.map((e) => e.style.height).join(','));`,
      ...
      links: [
        {
          url: 'https://web.dev/articles/avoid-large-complex-layouts-and-layout-thrashing',
          title: 'Avoid layout thrashing — web.dev',
          source: 'web-dev',
          language: 'en',
          note: 'Команда Chrome объясняет, что форсирует layout и как группировать read/write.',
        },
        {
          url: 'https://web.dev/articles/dom-size-and-interactivity',
          title: 'DOM size and interactivity — web.dev',
          source: 'web-dev',
          language: 'en',
          note: 'Как большое DOM-дерево влияет на INP и время до интерактивности.',
        },
      ],
    },
  ],

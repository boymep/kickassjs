      ...
      blocks: [
        {
          type: 'text',
          content:
            'JavaScript выполняется в **одном потоке** — одно дело в каждый момент. Но браузер не зависает, пока ждёшь сервер. Как? **Event loop**: тяжёлые операции (fetch, таймеры) уходят в Web APIs, их колбэки кладутся в очередь, а когда стек пуст — event loop достаёт следующий колбэк.',
        },
        {
          type: 'text',
          content:
            'Аналогия: повар в ресторане — он один, но пока тушится мясо (ждёт сервер), режет овощи (выполняет другой код). Мясо готово — сигнал, возвращается к нему. **На собеседовании** тебя попросят предсказать порядок вывода в коде с таймерами и промисами — без понимания стека и очередей угадать нельзя.',
        },
        {
          type: 'list',
...
// Вывод: Start → End → Promise → Timeout`,
        ...
        { type: 'visualization', content: '', vizId: 'event-loop' },
        {
          type: 'text',
          content:
            'Посмотри на пример выше: setTimeout с задержкой 0 всё равно выполняется последним. Многие думают что "0 миллисекунд" значит "немедленно" — но нет. setTimeout передаёт колбэк в Web API, тот кладёт его в очередь макрозадач, а event loop доберётся до неё только когда очистит стек и все микрозадачи. Это один из самых частых вопросов на интервью по event loop.',
        },
        {
          type: 'callout',
          calloutType: 'info',
      ...
      checkpoint: [Q['jsel-q1']!, {
        type: 'ordering',
        id: 'jsel-ord1',
        description: 'Расставь фазы event loop в правильном порядке',
        items: [
          'Call stack выполняет синхронный код',
          'Microtask queue: Promise.then, queueMicrotask',
          'Render (если нужно перерисовать UI)',
          'Macrotask queue: setTimeout, setInterval, I/O',
        ],
        explanation:
          'Event loop: сначала весь синхронный код, затем ВСЕ microtask (включая новые добавленные во время выполнения), затем один render-шаг, затем одна macrotask — и цикл повторяется.',
      }],
      video: {
        source: 'youtube',
        id: '8aGhZQkoFbQ',
        ...
        language: 'en',
        durationSec: 26 * 60,
        description: 'Каноническая лекция JSConf EU 2014. Лучшее интуитивное объяснение event loop, стека вызовов и Web APIs — с живой визуализацией.',
      },
      links: [
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/JavaScript/Event_loop',
          title: 'Цикл событий — MDN (ru)',
          source: 'mdn',
          language: 'ru',
          note: 'Каноническая справка MDN: стек, очередь, run-to-completion, never-blocking-model.',
        },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Не все асинхронные задачи одинаковы. `Promise`-колбэки (**microtask**) выполняются раньше `setTimeout`-колбэков (**macrotask**) — даже если `setTimeout(fn, 0)`. Главное правило:\n\n1. Весь синхронный код\n2. **Все** микрозадачи (включая добавленные во время их выполнения)\n3. Один render-шаг\n4. Одна макрозадача → и снова с шага 2',
        },
        {
          type: 'callout',
          calloutType: 'tip',
          content:
            '**На собеседовании** тебе покажут код с несколькими `setTimeout` и `Promise` в разном порядке — и попросят предсказать вывод. Ловушка: микрозадача может добавить новую микрозадачу прямо в процессе — она тоже выполнится до следующей макрозадачи.',
        },
        {
          type: 'list',
...
// Вывод: 1 → 2 → micro-1 → micro-2 → macro-1 → macro-2`,
        },
        {
          type: 'text',
          content:
            'Обрати внимание: micro-2 создаётся прямо внутри micro-1 и всё равно выполняется до макрозадач. Это ключевое свойство — очередь микрозадач опустошается "до дна", включая те, что добавлены в процессе. Только когда она пуста, event loop берёт следующую макрозадачу. Поэтому при бесконечной цепочке Promise.then страница может полностью зависнуть — так же как от бесконечного цикла.',
        },
        {
          type: 'callout',
          calloutType: 'warning',
...
// возвращает управление; D — синхронно; затем microtask checkpoint → B`,
      ...
      checkpoint: [Q['jsel-q2']!, Q['jsel-q4']!, {
        ...
        explanation:
          'В microtask queue: Promise callbacks (.then/.catch/.finally), queueMicrotask(), MutationObserver. В macrotask: setTimeout, setInterval, I/O, requestAnimationFrame (в некоторых браузерах).',
      }],
      video: {
        source: 'youtube',
        id: 'cCOL7MC4Pl0',
      ...
      blocks: [
        {
          type: 'text',
          content:
            '`requestAnimationFrame` встраивается в цикл рендеринга браузера: на 60 Гц мониторе колбэк выполняется **перед каждой отрисовкой** (~16.6 мс). `setTimeout(fn, 16)` такой гарантии не даёт — может выстрелить рассинхронизированно с экраном.',
        },
        {
          type: 'callout',
          calloutType: 'tip',
          content:
            'rAF выполняется **после микрозадач, но до paint** — изменения стиля применятся в текущем кадре. `setTimeout` — макрозадача следующего тика: отрисует браузер кадр или нет — непредсказуемо. На интервью по производительности это различие спрашивают обязательно.',
        },
        {
          type: 'text',
          content:
...
// setInterval(() => el.style.left = ... , 16);`,
        },
        {
          type: 'text',
          content:
            'Обрати внимание: в примере выше анимация рекурсивно вызывает сама себя через requestAnimationFrame. Это правильный паттерн — rAF запускается ровно один раз, потому каждый кадр нужно перезаписываться заново. Если забыть про рекурсию — анимация просто остановится после первого кадра. На собеседовании могут попросить объяснить почему нельзя просто поставить setInterval(fn, 16) вместо rAF.',
        },
        {
          type: 'callout',
          calloutType: 'warning',
      ...
      links: [
        {
          ...
          source: 'web-dev',
          language: 'en',
          note: 'Практическое руководство: когда использовать rAF, CSS animations, Web Animations API.',
        },
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/API/Performance/now',
          title: 'performance.now() — MDN (ru)',
          source: 'mdn',
          language: 'ru',
          note: 'Высокоточный таймер для измерения времени кадра в rAF-колбэке.',
        },
      ],
...

    {
      ...
      blocks: [
        ...
        },
        {
          ...
          calloutType: 'warning',
          content:
            'Классическая ловушка: async-функция запускается **синхронно** до первого `await`. До него — как обычная функция. После `await` — всё продолжение уходит в микрозадачу. Порядок вывода в коде с несколькими async-функциями часто удивляет — разбирай примеры по строкам.',
        },
        {
          type: 'code',
          language: 'javascript',
  ...
  return Promise.resolve(fetchData()).then((x) => {
    ...
    return x;
        ...
        },
        {
          type: 'text',
          content:
            'Эти две записи — async/await и цепочка .then — делают одно и то же, но первая читается как синхронный код. Именно в этом ценность async/await: не в новой функциональности, а в читаемости. Но понимать что происходит "под капотом" нужно — иначе не объяснить почему console.log после await выполняется позже console.log снаружи функции.',
        },
        {
          type: 'callout',
          calloutType: 'warning',
...
window.addEventListener('unhandledrejection', (event) => {
  ...
  // event.preventDefault() — подавить дефолтный лог в консоль
      ...
      links: [
        {
          url: 'https://tc39.es/ecma262/#sec-promise-jobs',
          title: 'Promise Jobs — ECMAScript spec',
          source: 'spec',
          language: 'en',
          note: 'Формальное определение PromiseReactionJob — как await ставит продолжение в очередь микрозадач.',
        },
      ],
      ...
      blocks: [
        {
          type: 'text',
          content:
            '**Jank** — видимые заминки интерфейса. Причина: главный поток занят слишком долго. На 60 Гц мониторе на каждый кадр — **16.6 мс**. Задача дольше **50 мс** — это **long task** по Web Vitals: ухудшает INP и видна в Performance-панели DevTools как красный треугольник.',
        },
        {
          type: 'callout',
          calloutType: 'tip',
          content:
            'Типичный вопрос на интервью: «как починить тяжёлый цикл?» — **разбить на чанки через `setTimeout`**. Важный нюанс: `queueMicrotask` не поможет — микрозадачи выполняются до рендеринга и браузер не успеет нарисовать кадр между чанками.',
        },
        {
          type: 'code',
          language: 'javascript',
  ...
  for (const item of items) {
    heavyTransform(item);
        ...
        },
        {
          type: 'text',
          content:
            'Этот код ужасен не потому что он неправильный — он делает что написано. Ужасен тем что пока он работает, браузер не может делать ничего другого: кнопки не реагируют, анимации замирают, скролл не работает. Три секунды зависшего интерфейса — это гарантированный уход пользователя. Поэтому в реальном коде любая тяжёлая операция должна либо разбиваться на чанки, либо уходить в Web Worker.',
        },
        {
          type: 'list',
      ...
      links: [
        {
          ...
          title: 'Chrome DevTools Performance — chrome.com',
          source: 'article',
          language: 'en',
          note: 'Как читать flame chart и находить long tasks, которые вызывают jank.',
        },
        {
          url: 'https://web.dev/optimize-long-tasks/',
          title: 'Optimize long tasks — web.dev',
          source: 'web-dev',
          language: 'en',
          note: 'Chunking, scheduler.yield(), isInputPending — современные способы нарезать длинные задачи.',
        },
      ],
      docsLink: { url: 'https://learn.javascript.ru/event-loop', title: 'Цикл событий — learn.javascript.ru' },
      playground: {
      ...
      blocks: [
        {
          type: 'text',
          content:
            '**`debounce`** и **`throttle`** — два паттерна для контроля частоты вызова функций. Оба построены на замыканиях и таймерах event loop. На собеседовании просят реализовать оба с нуля — они показывают понимание замыканий и асинхронности.',
        },
        {
          type: 'callout',
          calloutType: 'tip',
          content:
            'Аналогия: **debounce** — лифт, который ждёт пока все зайдут (ждёт паузу после последнего события). **throttle** — лифт с расписанием (отправляется раз в N мс). Выбор: поиск → debounce (нужен результат последнего запроса), скролл → throttle (нужна равномерная реакция).',
        },
...
// Пользователь печатает быстро — fetchResults вызовется только после паузы 300мс.`,
        },
        {
          type: 'text',
          content:
            'Обрати внимание на clearTimeout внутри debounce: это сердце паттерна. Каждый новый вызов отменяет предыдущий таймер и создаёт новый. Пока пользователь печатает быстро — таймер постоянно сбрасывается. Только когда печать прекращается на ms миллисекунд — таймер наконец срабатывает. Понять это через замыкание (timer хранится в замыкании между вызовами) — ключевое понимание для собеседования.',
        },
      ...
      links: [
        {
          ...
          title: 'queueMicrotask — MDN',
          source: 'mdn',
          language: 'en',
          note: 'Когда использовать queueMicrotask вместо Promise.resolve().then() — примеры в справке.',
        },
        {
          url: 'https://developer.mozilla.org/en-US/docs/Web/API/Scheduler/postTask',
          title: 'Scheduler.postTask — MDN',
          source: 'mdn',
          language: 'en',
          note: 'Новый API приоритизации задач — background, user-visible, user-blocking.',
        },
      ],
      docsLink: { url: 'https://learn.javascript.ru/event-loop', title: 'Цикл событий — learn.javascript.ru' },
      playground: {

...
Event loop — алгоритм, который позволяет работать с асинхронностью без блокировок: тяжёлые операции (\`setTimeout\`, \`fetch\`, обработчики DOM) выполняются вне главного потока, а их коллбэки складываются в очереди. Понимание порядка выполнения позволяет предсказывать, когда сработает \`Promise.then\`, почему \`setTimeout(fn, 0)\` не означает «немедленно», и почему длинный цикл замораживает интерфейс.`,
    estimatedMinutes: 32,
    interviewAngle:
      'Интервьюера интересуют три вещи: умение предсказать порядок вывода в коде с миксом \`setTimeout\` / \`Promise\` / \`queueMicrotask\`, понимание разницы между микро- и макрозадачами и моментом рендеринга, и умение объяснить причины jank в реальном UI-коде.',
...
// Start → End → Promise → Timeout`,
        ...
        },
        {
          type: 'text',
          content:
            '\`setTimeout(fn, 0)\` не означает «немедленно». Браузер регистрирует таймер в Web APIs, после 0 миллисекунд кладёт коллбэк в очередь макрозадач, и event loop возьмёт его только после очистки стека и опустошения очереди микрозадач.',
        },
      ],
      ...
      blocks: [
        {
          type: 'text',
          content:
            '\`requestAnimationFrame\` встраивается в цикл рендеринга: на 60 Гц мониторе коллбэк выполняется перед каждой отрисовкой, то есть примерно раз в 16.6 мс. \`setTimeout(fn, 16)\` такой гарантии не даёт — может выстрелить рассинхронизированно с экраном.',
        },
        {
          type: 'list',
...
requestAnimationFrame(animate);`,
        ...
        },
        {
          type: 'text',
          content:
            'Задача дольше 50 мс блокирует кадр: браузер не успевает к моменту vsync, кадр пропускается, появляется заметное «дёргание». В DevTools такие задачи помечаются в Performance-панели.',
        },
      ],
      playground: {
...
// 1 → 4 → 2 → 3`,
        },
        {
          type: 'text',
          content:
            'Необработанный rejected-promise попадает в глобальный обработчик \`unhandledrejection\`. Если используется \`.then\` без \`.catch\` или \`await\` без \`try/catch\`, ошибка может «пропасть» и проявиться только в консоли как UnhandledPromiseRejection.',
        },
      ],
    },
...
**Чанкование через \`setTimeout\` / \`scheduler.yield\`** — разбить массив на части, между ними отдавать управление event loop.
      ...
      blocks: [
        {
          type: 'text',
          content:
            '\`debounce\` и \`throttle\` — два паттерна для контроля частоты вызова функций. Оба построены на замыканиях и таймерах event loop. На собеседовании часто просят реализовать оба с нуля — они показывают понимание замыканий и асинхронности.',
        },

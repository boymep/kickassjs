      ...
      blocks: [
        {
          type: 'text',
          content:
            '**HTTP** — протокол клиент-серверного взаимодействия. Запрос = **метод** + **URL** + **заголовки** + тело. Ответ = **статус-код** + заголовки + тело. Во вкладке Network в DevTools — каждая строка это один такой запрос-ответ.',
        },
        {
          type: 'callout',
          calloutType: 'tip',
          content:
            '**Ключевое про методы — идемпотентность.** На собеседовании спросят не «перечисли методы», а «какой можно безопасно повторить»: `GET`, `PUT`, `DELETE` — идемпотентны (повтор не меняет состояние). `POST` — нет (повтор создаст дубль). Именно поэтому повторять `POST` автоматически нельзя.',
        },
...
// await safeRetry(() => fetch('/api/orders', { method: 'POST', body }));`,
      ...
      checkpoint: [Q['jsnet-q9']!, {
        type: 'ordering',
        id: 'jsnet-ord1',
        description: 'Расставь этапы HTTP запроса через fetch() в правильном порядке',
        items: [
          'fetch() вызван, создаётся Promise',
          'DNS lookup + TCP connection (если нет keep-alive)',
          'HTTP запрос отправлен на сервер',
          'Сервер обработал и отправил ответ',
          'Promise резолвится с объектом Response',
          'response.json() / response.text() читает тело',
        ],
        explanation:
          'fetch() возвращает Promise который резолвится когда получены заголовки ответа — не всё тело. Чтение тела (.json(), .text()) — это отдельный асинхронный шаг, поэтому нужен второй await.',
      }],
      ...
      video: {
        ...
        language: 'en',
        durationSec: 38 * 60,
        description: 'Практический разбор HTTP-методов, статусов и заголовков с примерами в DevTools.',
      },
      links: [
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/HTTP/Status',
          title: 'Коды статуса HTTP — MDN',
          source: 'mdn',
          language: 'ru',
          note: 'Быстрый справочник по кодам ответа — удобно при отладке, когда сервер вернул что-то неожиданное.',
        },
      ],
    },

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'CORS-ошибка происходит ТОЛЬКО в браузере — curl или Postman CORS не блокируют. Это самое частое заблуждение у новичков: "на бэкенде всё работает, а в браузере ошибка". Именно так и должно быть — CORS это браузерная защита, а не серверная. Исправляется только на сервере через заголовки — никакими трюками на клиенте не обойти. Поэтому когда видишь CORS-ошибку — не ищи решение во фронтенд-коде, иди на бэкенд.',
        },
        {
          type: 'callout',
          calloutType: 'info',
...
- **«Method not allowed in preflight»** — сервер не перечислил DELETE/PUT в Allow-Methods.`,
      ...
      checkpoint: [Q['jsnet-q3']!, Q['jsnet-q11']!, {
        type: 'match-pairs',
        id: 'jsnet-mp1',
        description: 'Сопоставь HTTP статус-код с его смыслом',
        pairs: [
          { left: '200 OK', right: 'Запрос успешен, тело содержит результат' },
          { left: '301 Moved Permanently', right: 'Ресурс перемещён навсегда, браузер кешируют редирект' },
          { left: '401 Unauthorized', right: 'Нужна аутентификация' },
          { left: '404 Not Found', right: 'Ресурс не существует на сервере' },
          { left: '500 Internal Server Error', right: 'Ошибка на стороне сервера' },
        ],
        explanation:
          '2xx — успех, 3xx — редиректы, 4xx — ошибка клиента (неправильный запрос/нет прав), 5xx — ошибка сервера. 401 vs 403: 401 — не аутентифицирован, 403 — аутентифицирован но нет доступа.',
      }],
      ...
      video: {
        ...
        language: 'en',
        durationSec: 2 * 60,
        description: 'Сжатый разбор того, зачем существует CORS и почему preflight нужен — за две минуты.',
      },
      links: [
        {
          url: 'https://learn.javascript.ru/fetch-crossorigin',
          title: 'Fetch: Cross-Origin запросы — learn.javascript.ru',
          source: 'learn-js',
          language: 'ru',
          note: 'Глава Ильи Кантора о CORS, preflight, credentials с подробными примерами.',
        },
      ],
    },

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Ключевое различие: cookies автоматически отправляются с каждым запросом к домену — даже без твоего участия. localStorage и sessionStorage — нет, их нужно читать вручную и добавлять в заголовок. Это делает cookies единственным безопасным вариантом для хранения сессионных токенов, если их защитить флагом HttpOnly. JWT в localStorage — популярное решение, но небезопасное: XSS-уязвимость даёт атакующему полный доступ к токену.',
        },
...
- **Большие данные (offline-кеш, файлы)** → IndexedDB. Cookies/localStorage не подходят по размеру.`,
      ...
      links: [
        ...
        },
        {
          ...
          source: 'mdn',
          language: 'ru',
          note: 'Справочник по localStorage и sessionStorage с примерами.',
        },
      ],
    },

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Это контр-интуитивно и это одна из самых распространённых ловушек на собеседованиях. Код написан правильно, try/catch стоит — но при 500 ошибка тихо "съедается". Причина: fetch считает задачей доставить HTTP-ответ, а не интерпретировать его статус. Поэтому 500 — это не исключение, это успешный ответ с плохим статусом. Привычка всегда проверять res.ok спасёт от сотен часов отладки в продакшне.',
        },
        {
          type: 'code',
          language: 'javascript',
      ...
      links: [
        {
          ...
          source: 'mdn',
          language: 'ru',
          note: 'Самый полный разбор Fetch: как читать тело разными методами, что делает credentials и как подружить с AbortController.',
        },
      ],
    },

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Типичный сценарий который проверяют на собеседовании: поле поиска, пользователь быстро набирает "react". На каждое нажатие уходит запрос. Сетевые ответы могут вернуться в любом порядке — медленный ответ для "re" может прийти позже быстрого ответа для "react" и перезатереть актуальные данные. Это race condition. AbortController решает это элегантно: перед каждым новым запросом отменяй предыдущий — и только последний запрос обновит UI.',
        },
      ...
      links: [
        {
          ...
          source: 'mdn',
          language: 'ru',
          note: 'Справочник: abort, signal, AbortSignal.timeout, использование в fetch.',
        },
      ],
      playground: {
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Хорошая новость: для статики кеш настраивается один раз и работает автоматически. Versioned assets — это файлы с хешем в имени (app.3f1a2c.js). Когда код меняется — меняется хеш — меняется URL — браузер считает файл новым и загружает. Старый файл берётся из кеша бесконечно долго. Это идеальная стратегия: max-age=31536000 (год) на весь JS и CSS, no-cache на HTML чтобы браузер всегда получал актуальные ссылки на файлы.',
        },
        {
          type: 'code',
          language: 'http',
      ...
      links: [
        {
          ...
          source: 'web-dev',
          language: 'en',
          note: 'Детальный гайд: Cache-Control, ETag, условные запросы, CDN и Service Worker.',
        },
      ],
    },
  ],

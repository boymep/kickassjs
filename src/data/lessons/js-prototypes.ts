      ...
      blocks: [
        {
          type: 'text',
          content:
            'У каждого объекта в JavaScript есть скрытая ссылка **`[[Prototype]]`** — другой объект. При обращении к свойству движок ищет его сначала на самом объекте, затем на `[[Prototype]]`, затем на `[[Prototype]]` прототипа — вплоть до `null`. Это и есть **цепочка прототипов** (prototype chain) — фундамент всей объектной модели JS.',
        },
        {
          type: 'code',
          language: 'javascript',
...
'map' in arr;              // true — оператор in идёт по цепочке`,
        },
        {
          type: 'text',
          content:
            'Аналогия: представь справочную службу. Тебе звонят с вопросом — ты проверяешь свои записи. Не знаешь — переключаешь на старшего коллегу. Тот не знает — на директора. Директор не знает — «не знаем» (undefined). Интервьюер проверит понимание разницы между «своим» свойством и унаследованным — знай `hasOwnProperty` и чем он отличается от оператора `in`.',
        },
        {
          type: 'text',
          content:
      ...
      checkpoint: [Q['jsp-q1']!, Q['jsp-q5']!, {
        type: 'ordering',
        id: 'jsp-ord1',
        description: 'Расставь шаги поиска свойства через цепочку прототипов',
        items: [
          'Проверить собственные свойства объекта (hasOwnProperty)',
          'Перейти к [[Prototype]] объекта',
          'Повторять пока не найдено или [[Prototype]] === null',
          'Вернуть undefined если не нашли',
        ],
        explanation: 'JavaScript ищет свойство сначала на самом объекте, затем идёт вверх по цепочке прототипов через [[Prototype]]. Цепочка заканчивается на Object.prototype — его [[Prototype]] равен null.',
      }],
      video: {
        source: 'youtube',
        id: 'XskMWBXNbp0',
        ...
        language: 'en',
        durationSec: 6 * 60,
        description: 'Анимированная визуализация цепочки прототипов — как движок поднимается по [[Prototype]] при поиске свойства.',
      },
      links: [
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/JavaScript/Inheritance_and_the_prototype_chain',
          title: 'Прототипная цепочка — MDN (ru)',
          source: 'mdn',
          language: 'ru',
          note: 'Каноническая справка: [[Prototype]], Object.prototype, цепочка и её конец.',
        },
      ],
      ...
      blocks: [
        {
          type: 'text',
          content:
            '`Object.create(proto)` — самый прямой способ создать объект с заданным прототипом. До появления `class` в ES2015 всё наследование строилось именно через эту функцию, и на собеседовании тебя могут попросить реализовать его без `class` чтобы проверить понимание прототипов под капотом.',
        },
        {
          type: 'text',
          content:
...
rex instanceof Animal;  // true`,
        ...
        },
        {
          type: 'text',
          content:
            'Три шага наследования без `class` — это классический вопрос на собеседовании. Интервьюер хочет убедиться что ты понимаешь: без `Animal.call(this, name)` новый объект не получит поля родителя; без `Object.create(Animal.prototype)` не будет работать `instanceof`; без восстановления `constructor` сломаются фабричные паттерны. Каждый шаг важен.',
        },
...
for (const key in dict) {
  // только собственные ключи, без сюрпризов
      ...
      video: {
        source: 'youtube',
        id: 'wstwjQ1yqWQ',
        title: 'Prototype in JavaScript — Namaste JavaScript',
        channel: 'Akshay Saini',
        language: 'en',
        durationSec: 21 * 60,
        description: 'Как Object.create создаёт прототипную цепочку, чем отличается от new — с разбором в DevTools.',
      },
      links: [
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Object/create',
          ...
          source: 'mdn',
          language: 'ru',
          note: 'Создание объекта с заданным прототипом — паттерн Object.create(null) как безопасный словарь.',
        },
      ],
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Синтаксис `class` появился в ES2015, но это не настоящие классы как в Java или Python — под капотом всё те же прототипы. Это просто удобный способ записать то что раньше делалось через функции-конструкторы и ручную настройку `prototype`. Понимать это важно чтобы не теряться когда что-то идёт не так.',
        },
        {
          type: 'text',
          content:
...
new Animal('Кот').speak(); // 'Кот говорит'`,
        ...
        },
        {
          type: 'text',
          content:
            'На собеседовании часто спрашивают: «Чем `class` отличается от function-конструктора?» Ответ: `class` нельзя вызвать без `new` (TypeError), методы класса не перечисляются (non-enumerable), `class` не всплывает как `var`. Это не просто синтаксический сахар — есть реальные отличия в поведении которые важно знать.',
        },
...
new BankAccount().MAX; // undefined — static не наследуется экземплярами`,
      ...
      checkpoint: [Q['jsp-q7']!, Q['jsp-q10']!, {
        type: 'match-pairs',
        id: 'jsp-mp1',
        description: 'Сопоставь метод с тем, что он делает',
        pairs: [
          ...
          { left: 'obj.hasOwnProperty(key)', right: 'Проверяет что свойство принадлежит объекту, не прототипу' },
          { left: 'Object.getPrototypeOf(obj)', right: 'Возвращает [[Prototype]] объекта' },
        ],
        explanation: 'Эти методы — основной инструментарий для работы с prototype chain. `Object.create` — единственный способ явно задать прототип при создании. `hasOwnProperty` критичен при итерации чтобы не захватить унаследованные свойства.',
      }],
      links: [
        {
          ...
          source: 'mdn',
          language: 'ru',
          note: 'Полная справка: синтаксис, extends, super, static, private fields.',
        },
        {
          url: 'https://2ality.com/2015/02/es6-classes-final.html',
          title: 'ES6 Classes in depth — 2ality',
          source: 'article',
          language: 'en',
          note: 'Аксель Раушмайер: как классы транспилируются в прототипы и что «под капотом» у extends.',
        },
      ],
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Три инструмента для проверки объектов, и у каждого своя ниша. `instanceof` проверяет цепочку прототипов — но ломается при передаче объектов между iframe. `hasOwnProperty` смотрит только на сам объект — важно при итерации. `in` идёт по всей цепочке — нужен когда надо учитывать унаследованные свойства.',
        },
        {
          type: 'text',
          content:
...
myInstanceof([], String);  // false`,
        ...
        },
        {
          type: 'text',
          content:
            'Подводный камень `instanceof`: он проверяет не тип объекта, а есть ли `Constructor.prototype` в цепочке прототипов. Если заменить `Array.prototype` (хотя делать это не надо), `[] instanceof Array` перестанет работать. Поэтому для массивов лучше `Array.isArray()` — он надёжнее и работает между разными контекстами исполнения.',
        },
...
- \`for...in\` перебирает все enumerable свойства, **включая** прототипные — фильтруйте через \`Object.hasOwn\`.`,
      ...
      links: [
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Operators/instanceof',
          title: 'instanceof — MDN (ru)',
          source: 'mdn',
          language: 'ru',
          note: 'Как instanceof проверяет цепочку прототипов и Symbol.hasInstance.',
        },
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Global_Objects/Object/getPrototypeOf',
          title: 'Object.getPrototypeOf — MDN (ru)',
          source: 'mdn',
          language: 'ru',
          note: 'Стандартный способ читать [[Prototype]] — вместо устаревшего __proto__.',
        },
      ],
      ...
      blocks: [
        {
          type: 'text',
          content:
            'С прототипами легко сделать что-то что «работает» но работает неправильно. Поменял `prototype` целиком — сломал `instanceof`. Добавил метод на `Array.prototype` — сломал чужую библиотеку. Написал стрелочную функцию как метод — она не попала на прототип, а значит у каждого экземпляра своя копия. Все три ошибки выглядят безобидно в маленьком примере, но дорого стоят в реальном коде.',
        },
        {
          type: 'text',
          content:
            'Прототипы дают большую гибкость, и именно поэтому с ними легко выстрелить себе в ногу. Три классических ловушки: потеря `constructor` при замене прототипа, расширение нативных прототипов которое ломает чужой код, и стрелочные функции на прототипе которые не попадают на прототип совсем.',
        },
...
Foo.prototype = { bar: 1, constructor: Foo };`,
        ...
        },
        {
          type: 'text',
          content:
            'Аналогия с `constructor`: представь что ты сделал ксерокопию паспорта, но забыл вписать своё имя — теперь там имя предыдущего владельца. `constructor` — это «имя» в паспорте объекта. Когда ты заменяешь `prototype` целиком, ксерокопия берёт имя из нового шаблона, а там `Object`. Три строки наследования без `class` — это классический вопрос на собеседовании, интервьюер ждёт все три шага.',
        },
...
Array.prototype[LAST] = function () { /* ... */ };`,
        ...
        },
        {
          type: 'text',
          content:
            'Расширение нативных прототипов — это антипаттерн известный как monkey-patching. Он может казаться удобным, но создаёт конфликты с другими библиотеками и с будущими версиями стандарта. `Array.prototype.flat` появился в ES2019 — и сломал код библиотек которые уже добавили свой `flat`. Всегда используй утилитарные функции вместо расширения прототипов.',
        },
...
// Зато this всегда привязан к экземпляру лексически (полезно для callback).`,
      ...
      links: [
        {
          url: 'https://learn.javascript.ru/class-inheritance',
          title: 'Наследование классов — learn.javascript.ru',
          source: 'learn-js',
          language: 'ru',
          note: 'extends и super: как наследование классов работает через прототипную цепочку.',
        },
      ],
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Движок JavaScript не просто следует цепочке прототипов — он её агрессивно оптимизирует. Объекты с одинаковой формой (одни и те же ключи в одном порядке) делят внутреннюю структуру — и доступ к свойствам становится почти бесплатным. Но некоторые операции убивают эти оптимизации.',
        },
        {
          type: 'text',
          content:
      ...
      links: [
        {
          ...
          title: 'Fast properties in V8 — V8 blog',
          source: 'v8-blog',
          language: 'en',
          note: 'Как V8 хранит свойства объектов: hidden classes, inline caches и почему динамическое изменение прототипа медленное.',
        },
      ],
      docsLink: { url: 'https://learn.javascript.ru/prototypes', title: 'Прототипы — learn.javascript.ru' },
    },

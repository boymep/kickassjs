...
console.log(alice.name); // 'Алиса'`,
        },
        {
          ...
          calloutType: 'info',
          content:
            'Приоритет связываний: \`new\` > explicit > implicit > default. Если правил несколько применимы одновременно, действует то, что выше по приоритету. Например, функция, вызванная через \`new\`, никаким предыдущим \`bind\` не «перепривязывается»: \`this\` будет указывать на новый объект.',
        },
      ],
      checkpoint: [Q['jst-q1']!, {
        type: 'ordering',

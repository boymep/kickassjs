...
new Timer().start();`,
        },
        {
          ...
          calloutType: 'warning',
          content:
            'Если стрелочная функция определена как метод объекта, \`this\` внутри неё не становится этим объектом — он остаётся тем, что был во внешней области. Поэтому стрелочная функция не подходит на роль метода.',
        },
        {
          type: 'code',
          language: 'javascript',
...
Идиома: использовать стрелочную функцию как коллбэк внутри метода, чтобы сохранить \`this\` метода.

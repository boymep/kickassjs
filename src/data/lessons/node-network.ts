      ...
      blocks: [
        {
          type: 'text',
          content:
            'Node.js умеет поднимать HTTP-сервер без единой зависимости — через встроенный модуль `http`. Вызываешь `http.createServer()`, передаёшь коллбек, вызываешь `.listen()` — и сервер работает. Никакого Express, никаких `npm install`. Это важно понимать, потому что именно так устроен любой фреймворк под капотом: он просто добавляет удобные обёртки поверх этого же механизма.',
        },
        {
          type: 'text',
          content:
            'На собеседовании этот код часто просят написать на доске — не чтобы проверить знание синтаксиса, а чтобы убедиться, что ты понимаешь: `req` это Readable stream (тело POST-запроса не приходит сразу, его нужно собирать по кускам), `res` это Writable stream, и если ты забудешь `res.end()` — клиент будет ждать ответа до таймаута. Express не скрывает эту механику, он её оборачивает.',
        },
        {
          type: 'code',
          language: 'javascript',
      ...
      links: [
        {
          ...
          source: 'nodejs-docs',
          language: 'en',
          note: 'Полная документация модуля http: createServer, IncomingMessage, ServerResponse, Agent.',
        },
      ],
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Middleware в Express — это функция `(req, res, next)`, которая стоит на пути каждого запроса. Она может залогировать его, проверить авторизацию, распарсить тело — а потом либо вызвать `next()` и передать управление дальше, либо ответить самостоятельно. Это как конвейер: каждый middleware — один шаг обработки.',
        },
        {
          type: 'text',
          content:
            'Важнее всего понять порядок: middleware выполняются строго в том порядке, в котором зарегистрированы через `app.use()`. Если ты поставишь auth-проверку после роутов — она никогда не выполнится для них. Самая частая ошибка новичков — забыть вызвать `next()` в middleware, который не отправляет ответ. Запрос тогда просто «зависает» — никакого сообщения об ошибке, просто таймаут.',
        },
        {
          type: 'code',
          language: 'javascript',
...
console.log(req.log); // ожидаем 'ABC'`,
      ...
      checkpoint: [
        ...
        Q['nodenet-q2']!,
        {
          type: 'ordering',
          id: 'nnet-ord1',
          description: 'Расставь порядок обработки HTTP-запроса через Express middleware chain',
          items: [
            'app.use() — глобальный middleware (логгер, bodyParser)',
            'app.use(\'/api\') — middleware для роута',
            'router.get(\'/users\') — route handler',
            'res.json() — отправка ответа клиенту',
          ],
          explanation:
            'Express выполняет middleware в порядке регистрации через app.use(). Каждый middleware вызывает next() чтобы передать управление следующему. Если next() не вызван — цепочка прерывается.',
        },
      ],
      ...
      links: [
        {
          ...
          source: 'article',
          language: 'en',
          note: 'Официальный гайд про middleware: типы (application/router/error-handling), порядок выполнения, next().',
        },
      ],
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Роутинг — это сопоставление входящего запроса `(метод + путь)` с нужным обработчиком. В Express это делается через `app.get()`, `app.post()` и так далее, но под капотом это просто массив правил, по которому идёт линейный поиск. Понять это важно, потому что порядок регистрации роутов имеет значение: первый совпавший маршрут побеждает.',
        },
        {
          type: 'text',
          content:
            'Динамические сегменты вроде `/users/:id` реализованы через регулярные выражения. Express берёт паттерн `/users/:id`, превращает его в regex `/^\/users\/([^/]+)$/`, и при совпадении извлекает `params.id` из захватывающей группы. На собеседовании иногда просят написать такой минимальный роутер самостоятельно — это классическая задача на понимание регулярок и структур данных.',
        },
        {
          type: 'code',
          language: 'javascript',
      ...
      links: [
        {
          ...
          title: 'Routing — Express guide',
          source: 'article',
          language: 'en',
          note: 'Официальный гайд по роутингу в Express: Router, параметры, регулярные выражения в путях.',
        },
      ],
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'CORS — это когда браузер не пускает твой фронтенд на `localhost:3000` достучаться до API на `api.example.com`. Звучит как ограничение сервера, но на самом деле это политика самого браузера: он смотрит на заголовки ответа и решает, показывать данные твоему JS или нет. Curl и Postman CORS не знают — они просто шлют запросы.',
        },
        {
          type: 'text',
          content:
            'Самая частая ловушка: разработчик ставит `Access-Control-Allow-Origin: *` (разрешить всем), но потом добавляет `credentials: "include"` на клиенте — и всё ломается. Браузер не разрешает `*` с credentials: это дыра в безопасности. Нужно указать конкретный origin. Второй частый вопрос на интервью — что такое preflight: это OPTIONS-запрос, который браузер автоматически шлёт перед «сложными» запросами (PUT, DELETE, application/json) — и сервер обязан на него ответить.',
        },
        {
          type: 'list',
...
app.use(corsMiddleware);`,
      ...
      video: {
        ...
        language: 'en',
        durationSec: 2 * 60,
        description: 'Сжатое объяснение CORS: origin, preflight и почему серверу нужно явно разрешать домены.',
      },
      links: [
        {
          url: 'https://developer.mozilla.org/ru/docs/Web/HTTP/CORS',
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Если у тебя есть форма логина без rate limiting — любой бот за несколько минут переберёт миллион паролей. Rate limiting ставит счётчик: «не больше 5 попыток с этого IP в минуту». При превышении сервер отвечает 429 Too Many Requests и клиент ждёт. Это базовая защита, которую нужно добавить до того, как приложение выйдет в продакшн.',
        },
        {
          type: 'text',
          content:
            'Ловушка при масштабировании: если хранить счётчики в памяти Node.js-процесса, то при запуске двух инстансов каждый будет считать только свои запросы. Пользователь сможет слать по 5 запросов на каждый инстанс — и rate limit не сработает. В кластере счётчики нужно держать в Redis, который общий для всех процессов. На собеседовании этот нюанс часто проверяют.',
        },
        {
          type: 'list',
      ...
      links: [
        {
          ...
          source: 'spec',
          language: 'en',
          note: 'Чек-лист безопасности: helmet, rate limiting, валидация входных данных, защита от prototype pollution.',
        },
        {
          url: 'https://web.dev/articles/rate-limit',
          title: 'Rate limiting strategies — web.dev',
          source: 'web-dev',
          language: 'en',
          note: 'Алгоритмы: fixed window, sliding window, token bucket — с наглядными объяснениями.',
        },
      ],
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'HTTP/1.1, HTTP/2 — это не просто версии протокола, это разные ответы на вопрос «как не тратить время на установку соединения». Без Keep-Alive каждый запрос — это TCP-handshake заново: три обмена пакетами, и только потом данные. На latency 50 мс это 150 мс накладных расходов на каждый запрос. Страница с 50 ассетами — это 7 секунд только на handshake.',
        },
        {
          type: 'text',
          content:
            'HTTP/2 решает проблему по-другому: одно соединение, но много параллельных запросов по нему (мультиплексирование). На практике в production HTTP/2 почти всегда терминируется на Nginx или балансировщике, а Node.js-сервер говорит с Nginx по HTTP/1.1 keep-alive. Это нюанс, который интервьюер может уточнить: «а сам Node.js говорит с балансировщиком по HTTP/2?» — и правильный ответ обычно «нет».',
        },
        {
          type: 'list',
...
**HTTP/3** идёт ещё дальше: переходит с TCP на QUIC (поверх UDP), убирая head-of-line blocking уже на транспортном уровне.`,
      ...
      checkpoint: [
        Q['nodenet-q9']!,
        {
          type: 'match-pairs',
          id: 'nnet-mp1',
          description: 'Сопоставь HTTP метод с его семантикой и идемпотентностью',
          pairs: [
            { left: 'GET', right: 'Получение ресурса, идемпотентный, без тела' },
            { left: 'POST', right: 'Создание ресурса, НЕ идемпотентный' },
            { left: 'PUT', right: 'Полная замена ресурса, идемпотентный' },
            { left: 'PATCH', right: 'Частичное обновление, НЕ всегда идемпотентный' },
            { left: 'DELETE', right: 'Удаление ресурса, идемпотентный' },
          ],
          explanation:
            'Идемпотентность: повторный запрос даёт тот же результат. GET, PUT, DELETE — идемпотентны. POST — нет (каждый создаёт новый ресурс). PATCH зависит от реализации.',
        },
      ],

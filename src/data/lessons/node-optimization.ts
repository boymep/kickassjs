      ...
      blocks: [
        {
          type: 'text',
          content:
            'Оптимизировать без измерений — всё равно что лечить вслепую. Ты думаешь, что тормозит вот эта функция, а на самом деле виноват совсем другой участок кода. Профилирование — это способ узнать правду: какая функция жрёт больше всего CPU, где event loop блокируется, где растёт heap. Без этих данных любая оптимизация — это угадайка.',
        },
        {
          type: 'text',
          content:
            'Главная метрика для Node.js-сервера — event loop lag. Если p99 lag больше 100 мс, значит на основном потоке есть что-то тяжёлое: синхронный JSON.parse большого объекта, CPU-intensive вычисление или просто бесконечный цикл. На собеседовании спросят именно про этот подход: сначала измерь, потом оптимизируй. Не наоборот.',
        },
        {
          type: 'list',
...
setInterval(() => {
  ...
  histogram.reset();
      ...
      video: {
        ...
        language: 'en',
        durationSec: 30 * 60,
        description: 'Маттео Коллина находит и устраняет узкие места шаг за шагом: clinic, autocannon, flame graphs, профайлинг event loop.',
      },
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'LRU-кеш — это как твой стол: ты держишь на нём то, с чем работаешь прямо сейчас. Когда места не хватает, убираешь то, к чему давно не прикасался. Точно так же LRU хранит N последних использованных элементов и выбрасывает «самый старый по обращению» при переполнении. Это идеальная политика, если твои данные имеют «временну́ю локальность» — одни пользователи возвращаются часто, другие исчезают.',
        },
        {
          type: 'text',
          content:
            'На собеседовании LRU — классическая задача. Условие: реализуй `get(key)` и `put(key, value)` за O(1). Ловушка: многие начинают с `Map` и думают о сортировке, но правильный ответ — Map сам сохраняет порядок вставки в JavaScript. Главная ошибка в реализации: в методе `get()` забыть сделать `delete` перед `set`. Без этого ключ остаётся на старой позиции и LRU превращается в FIFO.',
        },
        {
          type: 'code',
          language: 'javascript',
...
console.log(cache.get(2)); // -1`,
      ...
      checkpoint: [
        ...
        Q['nodeopt-q12']!,
        {
          type: 'multi-select',
          id: 'nopt-ms1',
          description: 'Что помогает избежать блокировки event loop в Node.js?',
          options: [
            'worker_threads для CPU-intensive задач',
            'Синхронные fs операции (fs.readFileSync)',
            'Async/await с неблокирующим I/O',
            'setImmediate() для разбивки длинных вычислений',
            'child_process.fork() для изоляции тяжёлых задач',
          ],
          correctIndices: [0, 2, 3, 4],
          explanation:
            'Блокирует event loop: любой синхронный код с долгим выполнением, fs.readFileSync, crypto sync методы. Не блокирует: worker_threads (отдельный поток), async I/O (libuv), setImmediate (даёт event loop \'вздохнуть\'), child_process (отдельный процесс).',
        },
      ],
      ...
      links: [
        {
          url: 'https://github.com/isaacs/node-lru-cache',
          title: 'node-lru-cache — production LRU от автора npm',
          source: 'github',
          language: 'en',
          note: 'Production-grade LRU с TTL, max-size и весом записей в байтах. Эталон для собственной реализации.',
        },
      ],
      playground: {
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Мемоизация — это когда функция запоминает свои результаты: вызвал с `(5, 3)`, получил 15, запомнил. При следующем вызове с `(5, 3)` — возвращает из кеша, не считает заново. Звучит как магия, но работает только для чистых функций: для одних и тех же аргументов всегда одинаковый результат. Если функция зависит от внешнего состояния — мемоизация даст неверный ответ.',
        },
        {
          type: 'text',
          content:
            'На сервере наивная мемоизация через `Map` без ограничений — мина замедленного действия. Если аргументы — это userId или поисковый запрос, количество уникальных ключей растёт с каждым запросом пользователя. Map никогда не очищается сам, GC не может освободить значения, heap растёт линейно. Рано или поздно процесс падает с `ENOMEM`. Правило: любая мемоизация на сервере — только с LRU или TTL.',
        },
        {
          type: 'code',
          language: 'javascript',
      ...
      links: [
        {
          ...
          source: 'article',
          language: 'en',
          note: 'Современный разбор кэширования в Node.js: in-process LRU, Redis, инвалидация по событию.',
        },
      ],
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Представь: ты выводишь список из 100 постов, и для каждого нужен автор. Наивный код делает 1 запрос на посты + 100 запросов на авторов = 101 запрос к БД. Это и есть N+1. Батчинг — способ собрать все 100 запросов `getUser(id)` в один `SELECT * FROM users WHERE id IN (1, 2, 3, ...)`. Разница может быть x100 по скорости.',
        },
        {
          type: 'text',
          content:
            'Пул соединений с БД — ещё одна базовая оптимизация, о которой спрашивают на каждом собеседовании. Без пула каждый HTTP-запрос открывает новое соединение с PostgreSQL: TCP-handshake + аутентификация = 50–100 мс накладных расходов. Пул держит N готовых соединений, и новый запрос берёт свободное за долю миллисекунды. Главное правило: всегда освобождай соединение в `finally`, иначе пул протечёт.',
        },
      ...
      links: [
        {
          ...
          source: 'github',
          language: 'en',
          note: 'Эталонная реализация батчинга за один тик event loop. Используется во всех GraphQL-серверах.',
        },
      ],
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Представь: у тебя микросервис оплаты, который обычно отвечает за 50 мс, но вдруг начинает зависать на 30 секунд. Каждый новый запрос к нему ждёт и держит соединение. Через минуту event loop твоего сервера забит тысячами зависших промисов. Твой сервис «лежит» — хотя в его коде нет ни одной ошибки. Это каскадный отказ.',
        },
        {
          type: 'text',
          content:
            'Circuit breaker — это «автомат» как в электрощите. Когда видит N ошибок подряд — «размыкает цепь» и начинает сразу выбрасывать ошибку, не делая сетевого запроса. Через timeout пробует один запрос — если прошёл, «замыкает» обратно. На собеседовании по System Design или отказоустойчивости обязательно спросят про три состояния: CLOSED (норма), OPEN (fail-fast), HALF-OPEN (проба). Нарисуй диаграмму состояний — произведёшь впечатление.',
        },
        {
          type: 'list',
...
// 5 ошибок подряд → OPEN на 30 секунд → HALF-OPEN → проба`,
      ...
      checkpoint: [
        Q['nodeopt-q7']!,
        {
          type: 'match-pairs',
          id: 'nopt-mp1',
          description: 'Сопоставь паттерн оптимизации с проблемой которую он решает',
          pairs: [
            { left: 'LRU Cache', right: 'Дорогие повторные вычисления / запросы к БД' },
            { left: 'DataLoader / Batching', right: 'N+1 проблема при множественных запросах' },
            { left: 'Circuit Breaker', right: 'Каскадные отказы при недоступности зависимости' },
            { left: 'Worker Threads', right: 'CPU-intensive задачи блокирующие event loop' },
          ],
          explanation:
            'LRU даёт bounded cache с вытеснением старых записей. DataLoader батчирует запросы в один. Circuit Breaker \'размыкает цепь\' при ошибках чтобы не ждать таймаута. Worker threads выносят тяжёлые вычисления из основного потока.',
        },
      ],
      docsLink: { url: 'https://metanit.com/web/nodejs/4.1.php', title: 'HTTP-сервер — metanit.com' },
      links: [
        {
          ...
          source: 'article',
          language: 'en',
          note: 'Классический пост: как Netflix защитил API от каскадных отказов через circuit breaker (Hystrix).',
        },
      ],
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Node.js однопоточный по природе: весь твой код выполняется на одном ядре. Это отлично для I/O (ждём ответа БД — не блокируем поток), но катастрофа для CPU. Если ты вычисляешь SHA-256 для 10 МБ файла синхронно — event loop заморожен на несколько секунд, и все входящие запросы ждут. Worker Threads решают именно эту проблему.',
        },
        {
          type: 'text',
          content:
            'Вопрос на интервью: «чем `worker_threads` отличается от `cluster`?» — отвечают неправильно примерно в половине случаев. `worker_threads` — отдельный поток в том же процессе, для одной тяжёлой задачи. `cluster` — несколько полноценных Node.js-процессов, каждый слушает один порт, для увеличения пропускной способности HTTP. Одно — для CPU-нагрузки, другое — для масштабирования RPS.',
        },
        {
          type: 'list',
          content: `Сравнение:
  ...
  _exec(slot, data, resolve, reject) {
    ...
    slot.worker.postMessage(data);
      ...
      video: {
        ...
        language: 'en',
        durationSec: 25 * 60,
        description: 'Глубокий разбор: как CPU-bound код «забивает» event loop, как это измерить и какие паттерны спасают latency.',
      },
      links: [
        {
          url: 'https://nodejs.org/api/worker_threads.html',
      ...
      blocks: [
        {
          type: 'text',
          content:
            'V8 управляет памятью автоматически, но «автоматически» не значит «без твоего участия». Если ты держишь ссылку на объект — GC его не освободит. Три самых частых источника утечек в Node.js: глобальная Map без ограничения размера, забытые подписчики EventEmitter, и замыкание которое держит ссылку на большой буфер. Все три паттерна выглядят безобидно в коде, но убивают сервер через несколько часов.',
        },
        {
          type: 'text',
          content:
            'Найти утечку — отдельное искусство. Первый признак: `heapUsed` растёт линейно при нагрузке и не падает после простоя. Инструмент диагностики: два heap-снапшота в Chrome DevTools (до и после нагрузки), режим Comparison — ищешь классы с большим `Δ Retained Size`. Интервьюер по Node.js может попросить рассказать алгоритм поиска утечки — это стандартный senior-вопрос.',
        },
        {
          type: 'list',

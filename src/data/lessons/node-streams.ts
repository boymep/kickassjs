      ...
      blocks: [
        {
          type: 'text',
          content:
            '**Стримы** — абстракция для работы с данными **по кускам (chunks)**, а не загрузки всего в память. 10 Гб файл: со стримом нужен буфер в несколько мегабайт, без — OutOfMemory. Все стримы — наследники `EventEmitter`.',
        },
        {
          type: 'text',
          content:
            'Аналогия: конвейер на заводе. Первый рабочий делает деталь и сразу передаёт второму — не ждёт пока сделает все. `pipe()` соединяет стримы именно так: первый производит, второй обрабатывает, третий записывает — одновременно.',
        },
        {
          type: 'list',
...
// Любой Duplex — и Readable, и Writable.`,
      ...
      checkpoint: [
        Q['nodes-q1']!,
        {
          type: 'ordering',
          id: 'nst-ord1',
          description: 'Расставь путь данных через Transform stream в правильном порядке',
          items: [
            'Readable stream эмитирует chunk данных',
            'Transform._transform() получает chunk и обрабатывает',
            'Transform вызывает this.push() с преобразованными данными',
            'Writable stream получает преобразованный chunk',
          ],
          explanation:
            'Transform — это дуплексный стрим: он одновременно Readable и Writable. pipeline(readable, transform, writable) соединяет их с автоматической обработкой backpressure и cleanup при ошибках.',
        },
      ],
      ...
      video: {
        ...
        language: 'en',
        durationSec: 25 * 60,
        description: 'Hussein Nasser объясняет 4 типа стримов с фокусом на сетевую модель: почему backpressure — это в первую очередь про TCP.',
      },
      links: [
        {
          url: 'https://nodejs.org/docs/latest/api/stream.html',
      ...
      blocks: [
        {
          type: 'text',
          content:
            '**pipe()** появился в Node.js 0.x — выглядит красиво, но у него два критичных недостатка: **ошибки не пробрасываются по цепочке** и **при сбое не освобождаются ресурсы** (файловые дескрипторы остаются открытыми).\n\n**pipeline()** (Node 10+) — современная замена: при ошибке автоматически вызывает `destroy()` на всех стримах цепочки. Возвращает Promise — удобно с `await/try/catch`. В production-коде `pipe` уже не используют.',
        },
        {
          type: 'code',
          language: 'javascript',
...
async function compress(src, dst) {
  ...
  fs.createWriteStream('output.log'),
      ...
      video: {
        ...
        language: 'en',
        durationSec: 30 * 60,
        description: 'Маттео Коллина — мейнтейнер стримов — показывает почему pipeline решает backpressure корректно и как async iteration меняет работу со стримами.',
      },
      links: [
        {
          url: 'https://2ality.com/2019/11/nodejs-streams-async-iteration.html',
      ...
      blocks: [
        {
          type: 'text',
          content:
            '**Transform-стрим** — это фильтр в цепочке: получает чанк на вход, возвращает преобразованный чанк на выход.\n\nДля своего Transform наследуетесь от `Transform` и реализуете два метода:\n- **`_transform(chunk, encoding, callback)`** — основная обработка; вызывайте `this.push()` чтобы отдать результат вниз по цепочке, потом `callback()`\n- **`_flush(callback)`** — вызывается один раз в конце; можно «дослать» накопленные данные',
        },
        {
          type: 'text',
          content:
            'В современном коде Transform-класс часто заменяют **async generator** — короче и читается как обычная функция. `pipeline()` принимает их напрямую в цепочку без оборачивания в Readable.',
        },
        {
          type: 'code',
          language: 'javascript',
      ...
      links: [
        {
          ...
          source: 'article',
          language: 'en',
          note: 'Свежее объяснение: highWaterMark, objectMode, AbortSignal в pipeline и async generator как Transform.',
        },
      ],
      playground: {
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Стримы решают реальные задачи: построчное чтение огромных логов, потоковая обработка записей из БД, сборка буфера когда без вариантов. Эти паттерны встречаются в продовом коде постоянно — полезно знать их наизусть.',
        },
        {
          type: 'text',
          content:
            'На собеседовании любимый вопрос: «как прочитать 10 Гб файл?». Правильный ответ — стримами. fs.readFile загрузит всё в память — процесс упадёт. pipeline + async generator = расход памяти ограничен размером буфера, а не файла.',
        },
      ...
      links: [
        {
          ...
          source: 'github',
          language: 'en',
          note: 'Классическая книга по стримам. Хорошо объясняет философию pipe и практические паттерны.',
        },
      ],
      playground: {
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Ошибки со стримами часто не бросают исключение сразу — они тихо роняют сервер через событие error без подписчика. Или накапливают данные в памяти потому что кто-то забыл про backpressure. Эта глава — про то где прячутся баги.',
        },
        {
          type: 'text',
          content:
            'Самая коварная ловушка: стрим однопроходный. Прочитал один раз — всё. Нельзя «перемотать» и прочитать снова из того же объекта. Если данные нужны дважды — либо буферизуй (для маленьких), либо открывай новый стрим.',
        },
...
// ✅ Выберите ОДИН способ потребления.`,
      ...
      checkpoint: [
        Q['nodes-q11']!,
        {
          type: 'match-pairs',
          id: 'nst-mp1',
          description: 'Сопоставь тип стрима с конкретным примером',
          pairs: [
            { left: 'Readable', right: 'fs.createReadStream(), HTTP request на сервере' },
            { left: 'Writable', right: 'fs.createWriteStream(), HTTP response' },
            { left: 'Transform', right: 'zlib.createGzip(), crypto.createCipher()' },
            { left: 'Duplex', right: 'net.Socket, WebSocket соединение' },
          ],
          explanation:
            'Readable — источник данных. Writable — приёмник. Transform — трансформирует данные (сжатие, шифрование, парсинг). Duplex — независимые read/write каналы (в отличие от Transform где output зависит от input).',
        },
      ],
      docsLink: { url: 'https://metanit.com/web/nodejs/11.1.php', title: 'Потоки (Streams) — metanit.com' },
    },

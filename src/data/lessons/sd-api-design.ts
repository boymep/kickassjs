...
Лучший кейс       | публичный API| BFF, мобайл| микросервисы  | full-stack TS`,
        },
        {
          type: 'text',
          content:
            'REST не является строгим стандартом — у каждой команды своя интерпретация «настоящего REST». Многие API называются RESTful, но на деле это просто HTTP с JSON. Сильный ответ на вопрос «расскажи о своём REST API» сравнивает архитектурные стили и обосновывает выбор именно REST в конкретном контексте, а не ограничивается определением.',
        },
        {
          type: 'callout',
          calloutType: 'tip',
          content:
            'Вопрос «REST или GraphQL?» вне контекста некорректен. Корректный ответ начинается с уточнения: кто клиент, насколько изменчив запрос, нужен ли HTTP-кеш, сколько микросервисов, каков язык бэкенда. «Лучшего» стиля вне контекста не существует.',
        },
      ],
    },
  ...
  User: {
    ...
    posts: (user, _, ctx) => ctx.db.posts.findByAuthor(user.id),
        ...
        },
        {
          type: 'text',
          content:
            'GraphQL не является универсальным решением. Сложнее кешировать (нет стандартных HTTP кеш-заголовков для POST). Классическая проблема — N+1: 100 пользователей и для каждого отдельный SQL-запрос на посты. Стандартное решение — DataLoader для батчинга запросов.',
        },
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'gRPC требует явного контракта в \`.proto\` и кодогенерации до сборки. tRPC использует общий TypeScript-тип роутера: клиент импортирует тип напрямую, кодогенерация не нужна. gRPC в браузере напрямую не работает — браузер не предоставляет API для управления HTTP/2-фреймами, необходимыми gRPC; для веб-клиентов используется gRPC-Web и транслирующий прокси. tRPC применяется, когда весь стек на TypeScript и API не публичный: end-to-end типизация без отдельной схемы.',
        },
...
- **Минус для веба**: браузер не умеет читать HTTP/2-trailer, нужен gRPC-Web (через прокси).`,
        ...
        { type: 'heading', content: 'tRPC: TypeScript как контракт' },
        {
          type: 'text',
          content:
            'tRPC решает задачу: в монорепо, где сервер и клиент написаны на TypeScript, схема контракта дублирует уже существующие типы. Клиент импортирует тип серверного роутера и получает полностью типизированные вызовы, без runtime-схемы и кодогенерации.',
        },
        {
          type: 'code',
          language: 'typescript',
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Обратная совместимость означает, что старые клиенты продолжают работать после обновления API. Ломающее изменение нарушает это свойство даже без явных правок на стороне клиента. Распространённая ловушка — breaking change в «minor» версии: семантически 1.2.0 → 1.3.0 безопасен, но удаление поля или изменение его типа всё равно ломает клиентов, независимо от номера. Корректный deprecate-флоу: заголовок \`Sunset\` с датой, параллельная поддержка старой версии, мониторинг использования старых эндпоинтов и предупреждения клиентам.',
        },
...
- Новый код ошибки (если клиент обрабатывает «неизвестные» как 5xx).`,
        ...
        },
        {
          type: 'text',
          content:
            'Распространённое заблуждение: «добавление поля — не breaking change, а удаление — лёгкая правка». На самом деле удаление, переименование или изменение типа поля — это всегда breaking change для клиентов, опирающихся на это поле. Добавление поля breaking change не является.',
        },
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Offset считает позицию от начала выборки, поэтому вставки и удаления между запросами сдвигают границы страниц: одни записи пропускаются, другие отдаются дважды. Cursor хранит непрозрачный токен с позицией последнего элемента, поэтому вставки не влияют на позицию пагинации. Поэтому для real-time-лент (Twitter, Instagram) применяется cursor: только он гарантирует, что записи не пропадут и не повторятся, пока пользователь листает.',
        },
...
- Считать total на больших таблицах дорого (COUNT без условий).`,
        },
        {
          ...
          calloutType: 'warning',
          content:
            '\`OFFSET 1 000 000\` в PostgreSQL — секунды CPU и full index scan. Бесконечная прокрутка с offset на больших таблицах создаёт неприемлемую нагрузку на БД и непригодна для продакшен-лент.',
        },
        {
          type: 'text',
          content:
            'Поэтому в продуктовых лентах cursor-пагинация — стандарт: позиция запоминается в виде ключа последнего элемента, и каждый следующий запрос идёт по индексу, а не пересчитывает смещение от начала таблицы.',
        },
...

{
  ...
  "data": null
        ...
        },
        {
          ...
          calloutType: 'info',
          content:
            'HTTP-коды лучше подходят для REST: с ними работают CDN, прокси и стандартные retry-стратегии клиентов (5xx ретраить, 4xx нет). Envelope-подход уместен для GraphQL и WebSocket, где транспорт всегда возвращает «успех».',
        },
...
Можно расширять: errors[], traceId, retryAfter, balance — что угодно.`,
        ...
        },
        {
          type: 'text',
          content:
            'Частая ошибка — возвращать \`200 OK\` с флагом \`success: false\` в теле для всех случаев. Это нарушает кеширование на CDN, ломает стандартные retry-стратегии HTTP-клиентов и делает невозможным мониторинг по статус-кодам. Корректный HTTP-код несёт семантику ошибки: 422 (валидация) не равно 400 (некорректный запрос), 503 (временно недоступно) не равно 500 (внутренняя ошибка) — и эти различия используются всей инфраструктурой.',
        },

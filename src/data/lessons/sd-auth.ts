  ...
  chapters: [
    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'JWT привлекателен отсутствием серверного хранилища сессий. Но его нельзя инвалидировать до истечения TTL: если токен украден, он остаётся валидным до конца срока жизни. С сессиями достаточно удалить запись из Redis. Грамотный выбор между ними формулируется как обоснование трейдоффа: когда серверная сессия лучше JWT и наоборот.',
        },
...
app.post('/login', async (req, res) => {
  ...
  req.session.regenerate(() => res.json({ ok: true })); // защита от fixation
        ...
        { type: 'heading', content: 'Структура JWT' },
        {
          type: 'text',
          content:
            'JWT — три части, разделённые точками: \`header.payload.signature\`. Каждая закодирована в base64url. Header содержит алгоритм и тип; payload — claims (полезную нагрузку); signature — подпись header и payload секретом сервера (HMAC) или приватным ключом (RSA, ECDSA). Стандартные claims: \`iss\` (издатель), \`sub\` (subject, обычно user id), \`aud\` (для кого предназначен токен), \`exp\` (срок истечения), \`nbf\` (не действителен раньше), \`iat\` (время выпуска), \`jti\` (уникальный id токена, используется для revocation).',
        },
        {
          type: 'callout',
          calloutType: 'warning',
          content:
            'Уязвимость \`alg=none\`: ранние версии библиотек принимали токены без подписи, если в header указано \`"alg":"none"\`. При вызове \`jwt.verify(token, secret)\` без явного списка алгоритмов библиотека могла поверить такому токену. Всегда передавайте \`algorithms: [\'HS256\']\` (или нужный) — это обязательная защита.',
        },
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Главная защита: атрибут HttpOnly запрещает доступ к cookie из JavaScript — это ключевой механизм против XSS-кражи токена. \`localStorage\` удобен и не требует серверной настройки, поэтому его часто используют в туториалах. Но любой успешный XSS на странице (включая скрипты из npm-зависимостей) получает полный доступ к \`localStorage\`. Поэтому токен сессии хранится в HttpOnly-cookie, а не в \`localStorage\`.',
        },
        {
          type: 'list',
...
- **In-memory** — переменная в JS-модуле или Redux store. XSS не «вытащит» её через document.cookie/localStorage, но JS-инъект всё равно прочитает её через захват API-вызовов; теряется при F5.`,
        },
        {
          type: 'code',
          language: 'ts',
          content: `// Корректно: HttpOnly + Secure + SameSite cookie.
...
// Антипаттерн: токен в localStorage.
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'OAuth и OIDC часто путают. OAuth решает задачу авторизации («какому приложению разрешён доступ к ресурсу»), OIDC построен поверх OAuth и решает задачу аутентификации («кто этот пользователь»). Различие закрепляется в двух токенах: \`access_token\` (OAuth) — для вызова API; \`id_token\` (OIDC) — содержит профиль пользователя и предназначен для клиентского приложения.',
        },
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Одноразовое использование refresh-токена позволяет детектировать утечку: повторное предъявление того же токена (его уже сменил легитимный клиент) — однозначный сигнал, что копия токена попала к атакующему. Сервер отзывает всю семью токенов, относящуюся к этой сессии, и принудительно завершает её.',
        },
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Главная защита: никогда не вставляй пользовательский ввод напрямую в HTML. React делает это автоматически (JSX экранирует строки), но `dangerouslySetInnerHTML` — дыра. Httponly cookie недоступна из JavaScript — ключевая защита от XSS-кражи токена. Но помните: XSS может делать запросы от имени пользователя даже без доступа к cookie, просто вызвав fetch с credentials: include.',
        },
        {
          type: 'list',
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'CSRF часто путают с XSS. Разница: XSS — злоумышленник выполняет код на вашем сайте, CSRF — злоумышленник заставляет ваш браузер сделать запрос к вашему сайту со своего. HttpOnly cookie защищает от кражи токена через XSS, но НЕ защищает от CSRF — потому что браузер всё равно прикрепляет её к запросам. Именно для защиты от CSRF и нужен SameSite.',
        },
        {
          type: 'code',
          language: 'html',
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Цель хеша пароля — сделать перебор экономически невыгодным даже при утечке базы. SHA-256 без замедления вычисляется за миллисекунды, поэтому даёт атакующему миллиарды попыток в секунду на GPU. \`bcrypt\` и \`argon2id\` спроектированы так, чтобы один хеш считался десятки миллисекунд (cost factor), а \`argon2id\` дополнительно требует много памяти (memory-hardness). Это делает атаки на GPU и ASIC экономически нецелесообразными.',
        },
        {
          type: 'code',
          language: 'ts',
...
- **Короткий TTL** + sliding expiration + обязательный logout, инвалидирующий запись на сервере.`,
        },
        {
          ...
          calloutType: 'warning',
          content:
            'Регенерация sessionId после логина — обязательная защита от fixation. Прочие меры (HttpOnly, Secure) полезны, но без регенерации атакующий, заранее «посеявший» свой sid, получит доступ.',
        },
      ],
    },
    ...
    {
      id: 'mfa-passkeys',
      title: 'MFA, WebAuthn и пасскеи',
      estimatedMinutes: 5,
      blocks: [
        {
          type: 'text',
          content:
            'Многофакторная аутентификация (MFA) добавляет к паролю второй фактор: что-то, что пользователь имеет (телефон с TOTP-приложением, аппаратный ключ) или чем является (биометрия). Это снимает риск компрометации одного фактора — украденный пароль без второго фактора бесполезен.',
        },
        { type: 'heading', content: 'TOTP — одноразовые коды' },
        {
          type: 'text',
          content:
            'TOTP (Time-based One-Time Password, RFC 6238) — короткий 6-значный код, который меняется раз в 30 секунд. Алгоритм детерминированный: общий секрет между сервером и приложением плюс текущее время дают код, генерируемый офлайн. Используется в Google Authenticator, 1Password, Authy. На сервере проверка — пересчёт нескольких соседних окон для компенсации рассинхронизации времени.',
        },
        { type: 'heading', content: 'WebAuthn и пасскеи' },
        {
          type: 'text',
          content:
            'WebAuthn — стандарт W3C для аутентификации через аппаратные ключи (YubiKey) и биометрию устройства (Touch ID, Windows Hello). На устройстве генерируется пара ключей: приватный остаётся в защищённом анклаве, публичный отправляется на сервер. При входе сервер шлёт challenge, устройство подписывает его — фишинг невозможен, потому что подпись привязана к origin сайта.',
        },
        {
          type: 'text',
          content:
            'Passkeys (пасскеи) — реализация WebAuthn с синхронизацией ключа между устройствами пользователя через провайдеров (iCloud Keychain, Google Password Manager). Заменяют пароль полностью: один Touch ID на телефоне работает на всех устройствах того же аккаунта. С 2023 года поддерживаются крупнейшими провайдерами и активно вытесняют пароли в новых продуктах.',
        },
        {
          type: 'callout',
          calloutType: 'info',
          content:
            'WebAuthn устойчив к фишингу на уровне протокола: подпись привязана к точному origin (\`https://example.com\`), и устройство не подпишет challenge от \`https://exampIe.com\` с подменённой буквой. Это принципиальное отличие от TOTP, который можно ввести на фишинговой странице.',
        },
      ],
    },
    ...
    // ─────────────────────────────────────────────────────────────
    {
      ...
      title: 'Авторизация: RBAC, ABAC, ReBAC',
      estimatedMinutes: 5,
      blocks: [
        {
          type: 'text',
          content:
            'Аутентификация отвечает на вопрос «кто это», авторизация — «что этому пользователю позволено». Это разные задачи, и они решаются разными моделями.',
        },
        { type: 'heading', content: 'RBAC — role-based' },
        {
          type: 'text',
          content:
            'Самая простая модель: у пользователя одна или несколько ролей (admin, editor, viewer), у каждой роли — список разрешений (\`posts.create\`, \`posts.delete\`). Подходит для большинства приложений с фиксированным набором функций. Минусы — слабая гранулярность: разрешение на «удаление поста» либо есть, либо нет, без учёта, чей это пост.',
        },
        { type: 'heading', content: 'ABAC — attribute-based' },
        {
          type: 'text',
          content:
            'Разрешения вычисляются из атрибутов пользователя, ресурса и контекста. Пример правила: «пользователь может редактировать пост, если он автор поста ИЛИ модератор раздела, в котором пост опубликован, И сейчас рабочее время». Подходит для гибких правил, но сложнее аудитировать и тестировать. Стандарт — XACML.',
        },
        { type: 'heading', content: 'ReBAC — relationship-based' },
        {
          type: 'text',
          content:
            'Разрешения выводятся из графа связей между объектами. Пример: «пользователь может читать документ, если документ лежит в папке, доступ к которой есть у пользователя через команду, в которой он состоит». Модель популяризовала Google Zanzibar; в open-source её реализуют SpiceDB, OpenFGA, Permify. Подходит для документоориентированных систем (Google Docs, Notion, GitHub-репозитории).',
        },
        { type: 'heading', content: 'Logout и revocation' },
        {
          type: 'text',
          content:
            'Корректный logout — это не просто удаление токена на клиенте. Серверная сессия должна удаляться явно (записью в БД или в Redis). Для JWT, который нельзя инвалидировать по природе, используется одна из стратегий: короткий TTL + refresh rotation; blacklist отозванных \`jti\` в Redis (с TTL равным сроку жизни токена); версионирование секрета подписи (kid rotation). При выходе со всех устройств обнуляется serial number у пользователя, и все ранее выпущенные токены становятся недействительными.',
        },
      ],
    },
  ],

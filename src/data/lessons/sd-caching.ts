...
Главный язык кеширования — HTTP-заголовки \`Cache-Control\`, \`ETag\`, \`Last-Modified\`, \`Vary\`. На сервере и в приложениях работают паттерны cache-aside, write-through, stale-while-revalidate. Самая сложная часть — инвалидация: на собеседовании по System Design проверяют понимание трейдоффа между свежестью данных, нагрузкой и сложностью инвалидации.`,
    estimatedMinutes: 35,
    interviewAngle:
  ...
  chapters: [
    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Запрос останавливается на первом попадании в кеш, поэтому выгоднее всего располагать данные ближе к пользователю: горячие — в браузере, частые — на CDN, редкие — в Redis или в базе. Общее правило: кешировать на самом верхнем уровне, на котором допустимо одинаковое значение для всех пользователей этой группы.',
        },
        {
          type: 'list',
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Распространённое заблуждение: \`Cache-Control: no-cache\` воспринимается как «никогда не кешировать». На самом деле \`no-cache\` означает «кешируй, но перед использованием обязательна валидация с сервером». Полный запрет на кеширование — \`no-store\`. Различие проверяется на собеседовании регулярно и показывает, действительно ли кандидат разбирался со спецификацией.',
        },
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Инвалидация кеша на CDN решается тремя подходами: ждать истечения TTL (просто, но данные могут устареть); явный purge через API (быстро, но дорого при full purge); versioned URLs с \`immutable\` (надёжно — старый URL остаётся, новый контент получает новый URL). В продакшене обычно комбинируют: статика — versioned URLs, HTML — короткий \`s-maxage\` плюс targeted purge по тегу из вебхука. Различают также purge (немедленное удаление с edge) и invalidation (пометка как stale, удаление по требованию) — у разных CDN это разные операции с разной ценой.',
        },
        {
          type: 'list',
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Распространённый сюрприз при масштабировании: in-memory LRU-кеш в Node.js виден только одному инстансу. При запуске двух подов каждый держит свой кеш — инвалидация в одном не затрагивает другой, и данные начинают расходиться. При нескольких инстансах необходим общий кеш в Redis или согласованная инвалидация через pub/sub.',
        },
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Стратегии инвалидации — обязательная часть темы. Универсального ответа нет: используются TTL (простой, но данные устаревают), event-driven (точный, но сложнее), versioned URLs (надёжный для статики). Сильный ответ называет трейдофф каждого подхода, а не ограничивается перечислением.',
        },
...
async function get(id: string) {
  ...
  return promise; // все ждут одну сборку
        ...
        },
        {
          ...
          calloutType: 'tip',
          content:
            'TTL jitter — добавление к TTL случайной поправки ±10%, чтобы 10 000 ключей не устарели в одну секунду. Простой приём, заметно сглаживающий нагрузку на origin.',
        },
        { type: 'heading', content: 'Distributed lock против stampede' },
        {
          type: 'text',
          content:
            'Альтернатива single-flight на одном инстансе — распределённый лок. Перед тяжёлым rebuild в Redis выставляется ключ через \`SET key NX EX ttl\` (атомарная установка только если ключа нет). Только один процесс получает лок и собирает значение, остальные либо ждут (с тайм-аутом), либо возвращают stale-ответ. Без TTL такой лок может «зависнуть» при падении владельца — поэтому TTL обязателен.',
        },
        { type: 'heading', content: 'Soft TTL и hard TTL' },
        {
          type: 'text',
          content:
            'Полезно различать два порога свежести. Soft TTL — момент, после которого значение считается stale, но всё ещё может быть использовано (с фоновой ревалидацией). Hard TTL — момент окончательной непригодности, после которого значение приходится получать заново синхронно. \`stale-while-revalidate\` опирается именно на эту пару: \`max-age\` задаёт soft TTL, \`stale-while-revalidate\` — время, в течение которого допустимо отдавать stale.',
        },
        { type: 'heading', content: 'Negative caching' },
        {
          type: 'text',
          content:
            'Кеширование ответов с ошибками (404, 5xx) с коротким TTL защищает origin от шторма запросов на несуществующие или временно сломанные ресурсы. Без negative caching сканер каталога легко создаст лавину однотипных промахов, доходящих до базы. Слишком длинный TTL для 5xx опасен противоположным образом: восстановление сервиса не виден клиентам, пока кеш не истечёт, — поэтому TTL для негативных ответов обычно держат в десятках секунд.',
        },
      ],
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Типичная ошибка — добавлять кеш везде «потому что оптимизация». Если данные меняются каждые 5 секунд, кеш на 60 секунд — это источник устаревших данных, а не оптимизация. Умение обоснованно отказаться от кеширования в конкретном случае показывает понимание трейдоффов не хуже, чем умение настроить кеш.',
        },
        {
          type: 'list',
...
Vary: Authorization`,
        ...
        { type: 'heading', content: 'Кеширование авторизованного контента' },
        {
          type: 'text',
          content:
            'Если ответ персонализирован (показывает имя пользователя, корзину, лимиты), CDN не должен его кешировать как public. Корректный подход — \`Cache-Control: private\` (только браузер). Заголовок \`Vary: Authorization\` ненадёжен в общих кешах: большинство CDN при наличии \`Authorization\` либо не кешируют ответ вообще, либо требуют дополнительной настройки, поэтому полагаться на \`Vary\` для разделения по пользователю не следует. Для приватного контента используется per-user edge cache (Cloudflare Workers KV с user-scoped ключами) или явный \`no-store\`.',
        },
        {
          type: 'callout',
          calloutType: 'tip',

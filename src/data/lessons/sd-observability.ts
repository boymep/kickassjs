...
/>`,
    options: ['debug', 'private', 'public', 'tracked'],
    correctIndex: 1,
    explanation:
...
// error budget ≈ __BLANK__ минут downtime в месяц`,
  ...
  },
  {
    ...
    code: `// Антипаттерн: алерт на каждый JS-error на любом устройстве:
  ...
  chapters: [
    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Три сигнала observability различаются по уровню детализации и по тому, на какие вопросы они отвечают. Логи — детальная запись событий с контекстом, удобная для расследования инцидентов; неструктурированный лог превращается в шум, поэтому в продакшене используется structured logging (JSON-логи с полями). Метрики — агрегированные числовые показатели за интервал, подходят для алертов и трендов, но не дают деталей конкретного запроса. Трейсы показывают путь одного запроса через все сервисы и позволяют точно найти, на каком шаге накопилась задержка. Распределённый трейс нужен, когда запрос проходит через несколько сервисов и нужно найти, где именно потеряли N мс; метрик достаточно, когда нужно ответить «растёт ли error rate».',
        },
        {
          type: 'callout',
          calloutType: 'info',
        ...
        {
          type: 'list',
          content: `Metrics — агрегированные числовые временные ряды (RPS, error rate, p95 latency, RAM). Дёшевы в хранении, подходят для dashboards и алертов.
Logs — структурированные события с контекстом (\`{level, ts, traceId, userId, msg, ...}\`). Восстанавливают, что именно произошло.
Traces — деревья спанов одного пользовательского запроса через все сервисы. Отвечают на вопрос «где мы потеряли N мс».`,
        },
        { type: 'heading', content: 'Типы метрик' },
        {
          type: 'text',
          content:
            'Метрики делятся на четыре стандартных типа, на которых построены Prometheus, OpenTelemetry и большинство систем мониторинга.',
        },
        {
          type: 'list',
          content: `Counter — счётчик, который только растёт (количество запросов, количество ошибок). Сбрасывается при перезапуске процесса.
Gauge — мгновенное значение, которое может как расти, так и убывать (использование памяти, размер очереди, количество активных соединений).
Histogram — распределение значений по бакетам (latency запросов с разбивкой по диапазонам). Позволяет считать процентили (\`p50\`, \`p95\`, \`p99\`) на стороне Prometheus, а не клиента.
Summary — похож на histogram, но процентили вычисляются на стороне клиента и фиксируются заранее. Менее гибкий, чем histogram, поэтому в новых системах применяется реже.`,
        },
        { type: 'heading', content: 'Уровни логирования и агрегация' },
        {
          type: 'text',
          content:
            'Стандартные уровни: \`debug\` (только для разработки), \`info\` (бизнес-события), \`warn\` (потенциальная проблема), \`error\` (произошёл сбой), \`fatal\` (несовместимая с работой ошибка, обычно ведёт к exit). В продакшене обычно держат уровень \`info\` или \`warn\`, а \`debug\` включают точечно для расследования. Логи отправляются в централизованную систему: ELK (Elasticsearch + Logstash + Kibana), Loki + Grafana, Splunk, Datadog Logs. Без агрегации логи 50 микросервисов невозможно сопоставить — поэтому correlation ID или trace ID добавляется в каждую запись.',
        },
        {
          type: 'code',
          language: 'typescript',
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'RUM-данные шумные по природе: боты, пользователи со старыми устройствами на медленных сетях, офисные сети с прокси — всё это попадает в выборку и искажает среднее. Поэтому используется сегментация по типу устройства, сети, географии. Среднее значение LCP — слабая метрика: mean скрывает хвост распределения, в котором находятся самые пострадавшие пользователи. Корректный показатель — \`p75\` или \`p95\` (Google Core Web Vitals использует \`p75\` для каждой метрики).',
        },
        {
          type: 'callout',
          calloutType: 'info',
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Хорошая система алертинга на ошибки реагирует только на значимые инциденты. Rate limiting в Sentry — частая ловушка: слишком жёсткое сэмплирование пропускает критическую ошибку, которая произошла ровно за порогом лимита. Группировка ошибок строится через fingerprinting: Sentry по умолчанию группирует по нормализованному стек-трейсу, но одна ошибка с разными динамическими параметрами (id, токены, метки времени) создаёт тысячи групп. Корректная настройка — кастомные fingerprints для таких случаев, сэмплирование с приоритетом редких ошибок и алерты только на новые группы или резкий рост частоты известных.',
        },
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Session replay фиксирует действия пользователя в интерфейсе (клики, ввод, навигацию) для последующего воспроизведения при дебаге. Без маскировки в записи попадают номера карт, пароли и другие персональные данные — это прямое нарушение GDPR и аналогичных законов. Корректная настройка — privacy-first по умолчанию: маскировка всех input-полей, отключение записи на страницах с PII, запись только при ошибке. Трейдофф privacy vs debugging решается сэмплированием с приоритетом ошибочных сессий, а не сплошной записью.',
        },
        {
          type: 'callout',
          calloutType: 'warning',
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Feature flag позволяет включить и выключить функциональность без релиза кода. Распространённая ловушка — флаг без даты удаления: через год в коде накапливаются десятки мёртвых флагов, которые никто не решается удалить, потому что неясно, нужны ли они ещё. Стратегия gradual rollout (1% → 10% → 100%) снижает риск и одновременно позволяет на каждом шаге измерять метрики и убеждаться, что ключевые показатели (конверсия, error rate) не ухудшились до расширения аудитории.',
        },
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'SLO 99.99% для MVP — это около 52 минут даунтайма в год; SLO 99.999% — около 5 минут. Достижение таких показателей возможно только с zero-downtime deploy, автофейловером и геораспределённой репликацией — это месяцы инженерной работы. Понятие error budget решает противоречие между надёжностью и темпом разработки: если бюджет ошибок (например, 0.5% за месяц) исчерпан, выпуск новых фич приостанавливается до восстановления надёжности. Это формализованный процесс, заменяющий неформальное «мы стараемся не ломать».',
        },
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Alert fatigue — состояние, в котором после серии ложных срабатываний дежурный перестаёт реагировать на сигналы. Алерты должны срабатывать только на то, что требует немедленного действия человека; остальное — в dashboard для расследования. Google в SRE-практике предлагает концепцию golden signals: latency (время ответа), traffic (RPS), errors (процент ошибок), saturation (заполненность ресурсов) — четыре метрики, достаточные как первый уровень алертинга для большинства сервисов. В дополнение к ним применяются RED-метрики (Rate, Errors, Duration — для веб-сервисов) и USE-метрики (Utilization, Saturation, Errors — для инфраструктурных ресурсов).',
        },
        {
          type: 'list',

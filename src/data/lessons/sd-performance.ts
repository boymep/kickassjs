...
onCLS((m) => beacon('/rum', m));`,
  ...
  },
  {
    ...
    correctIndex: 1,
    explanation:
      'rel="preload" говорит браузеру: ресурс понадобится скоро на этой же странице, начните скачивать с высоким приоритетом. Обязателен атрибут as (image/font/script/style), без него preload игнорируется. prefetch — для будущей навигации (низкий приоритет). preconnect открывает TCP+TLS-соединение заранее.',
  },
  {
    type: "output",
    id: "sdp-q6",
...
export const sdPerformanceLesson: Lesson = {
  ...
  intro: {
    whyItMatters: `Производительность фронтенда — инженерная характеристика, которую измеряют и контролируют наравне с покрытием тестами. Google учитывает Core Web Vitals в поисковом ранжировании: LCP < 2.5 с, INP < 200 мс, CLS < 0.1. Лишние 100 мс задержки крупные ритейлеры связывают с заметными потерями выручки.

Профессиональный подход — измерять параллельно через RUM (реальные пользователи, web-vitals.js) и synthetic (Lighthouse CI, WebPageTest). На основе данных команда устанавливает performance budget, CI блокирует регрессии. Оптимизации делятся на четыре группы: загрузка (code splitting, preload), рендер (\`transform\`, critical CSS), ресурсы (AVIF / WebP, srcset, lazy), CPU (Web Workers, tree shaking).`,
    estimatedMinutes: 45,
    interviewAngle:
      'Интервьюера интересует умение поставить диагноз, а не определения. Знание Core Web Vitals и порогов, понимание разницы RUM и synthetic, способность различать оси оптимизации (LCP / INP / TTI / CLS) и не путать их.',
...
onLCP(send); onINP(send); onCLS(send); onTTFB(send); onFCP(send);`,
        },
        {
          type: "text",
          content:
            'Метрики, по которым Google ранжирует страницы, собираются с реальных пользователей через датасет CrUX (Chrome User Experience Report) и через собственные RUM-системы. Lighthouse в DevTools — синтетический тест в управляемых условиях: он полезен для регрессий в CI, но на ранжирование не влияет, и «хороший Lighthouse score» не равен «хорошим Core Web Vitals в продакшене». Реальные значения собираются библиотекой web-vitals.js и отправляются в собственную аналитику.',
        },
        ...
        },
        {
          ...
          calloutType: "warning",
          content:
            "Ключевые пороги — 2.5 с (LCP), 200 мс (INP), 0.1 (CLS). Превышение хотя бы одного исключает страницу из категории «Good» в Google Search Console.",
        },
      ],
      checkpoint: [Q["sdp-q1"]!, {
        type: 'multi-select',
      ...
      blocks: [
        {
          type: "text",
          content:
            "Есть два способа узнать, насколько быстр ваш сайт: запустить тест в лаборатории (Lighthouse) или собрать данные с реальных пользователей (RUM). Это не одно и то же. Первый даёт повторяемые цифры для CI, второй — правду о том, что чувствуют ваши пользователи. Performance budget — договорённость команды о жёстких лимитах, при превышении которых CI блокирует merge.",
        },
        {
          type: "text",
          content:
            "Lighthouse исполняется в фиксированной конфигурации (чистый кеш, заданная сеть и CPU), поэтому даёт повторяемые цифры для CI и регрессионных тестов. Реальные пользователи приходят с частично тёплым кешем, на разном железе и при разном качестве сети, поэтому RUM-данные шире и точнее. Для ранжирования в поиске Google использует именно RUM-источник CrUX, а не результаты Lighthouse.",
        },
        ...
        },
        {
          type: "text",
          content:
            "Synthetic-тесты воспроизводят регрессии в одинаковых условиях, RUM показывает картину реальной аудитории на её устройствах и сетях. Зрелый процесс мониторинга использует оба источника одновременно; опора только на Lighthouse CI без RUM оставляет производственные проблемы незамеченными.",
        },
        {
          type: "text",
          content:
            'Типичная ловушка: «добавили Lighthouse CI — значит всё под контролем». На самом деле Lighthouse гоняет чистый кеш и фиксированную сеть, а реальные пользователи приходят с тёплым кешем частично, со слабым Android и нестабильной сетью. Без RUM в реальной среде нельзя назвать p75 LCP на мобильных устройствах — а без этой цифры оптимизация ведётся вслепую.',
        },
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: "text",
          content:
            "\`async\` и \`defer\` оба не блокируют парсинг HTML, но порядок выполнения у них разный. \`async\` выполняет скрипт сразу после загрузки в произвольном порядке. \`defer\` дожидается окончания парсинга HTML и выполняет скрипты в порядке их появления в документе. \`preload\` отличается от обоих: он сообщает браузеру «начни загрузку прямо сейчас», но не означает «выполни прямо сейчас» — скрипт попадает в кеш и выполняется, когда до него доходит парсер.",
        },
...
- **dns-prefetch** — только DNS-резолв, дешевле preconnect; fallback для старых браузеров.`,
  ...
  <link rel="prefetch" href="/dashboard" as="document">
        ...
        },
        {
          type: "text",
          content:
            "Различие \`defer\` и \`async\` — базовый вопрос темы. Менее очевидный момент: \`async\` опасен в зависимом коде. Если скрипт аналитики загрузится раньше основного и начнёт обращаться к его API, возникнет \`TypeError\`. \`defer\` гарантирует порядок исполнения, \`async\` — нет. Поэтому \`async\` используется только для полностью независимых скриптов.",
        },
...
<script src="/legacy.js"></script>`,
        },
        {
          type: "text",
          content:
            "Распространённое заблуждение: один атрибут способен ускорить hero-картинку. На практике требуется комбинация: \`preload\` + \`fetchpriority=\"high\"\` на LCP-кандидате и отсутствие \`loading=\"lazy\"\` на нём. Это разные механизмы, работающие вместе.",
        },
  ...
  <img src="/hero-800.jpg" alt="Hero"
       ...
       fetchpriority="high">
        ...
        },
        {
          type: "text",
          content:
            "Частая ошибка на реальных проектах — глобальное применение \`loading=\"lazy\"\` ко всем картинкам через общий компонент. Для LCP-кандидата это даёт обратный эффект: браузер откладывает загрузку, и LCP вырастает на 1–2 секунды. Правило простое: \`loading=\"lazy\"\` не применяется к изображениям, видимым без скролла.",
        },
...
const RichTextEditor = lazy(() => import('./RichTextEditor'));`,
        },
        {
          type: "text",
          content:
            "Подводный камень code splitting, о котором редко говорят: если вы разбил бандл на чанки, но все они грузятся одновременно при открытии страницы — ничего не изменилось. Смысл в том, чтобы грузить чанк только когда он нужен. Поэтому Suspense fallback не должен быть пустым — это и есть момент когда пользователь ждёт чанк. Плохой fallback = плохой UX, даже при правильном splitting.",
        },
        {
          type: "text",
          content:
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: "text",
          content:
            "Reflow — пересчёт размеров и позиций всех элементов, на которые может повлиять изменение (width, height, margin, padding, font-size, добавление и удаление элементов). Repaint — перерисовка пикселей без изменения геометрии (background-color, color, box-shadow). Чтение \`offsetWidth\` или \`getBoundingClientRect()\` в середине JS-кода принуждает синхронный layout — эффект называется layout thrashing. \`will-change\`, добавленный на множестве элементов, ухудшает картину: браузер создаёт отдельный composite-слой для каждого, что увеличивает расход видеопамяти и замедляет композитинг.",
        },
        {
          type: "heading",
...
.good:hover { transform: translateX(100px) scale(1.2); opacity: 0.8; }`,
        },
        {
          type: "text",
          content:
            "Анимация через \`left\` и \`top\` визуально работает, но дёргается, потому что эти свойства вызывают reflow — браузер пересчитывает позиции всех соседних элементов. На слабом устройстве это даёт 50+ мс на кадр и заметные пропуски. \`transform: translate()\` не меняет flow документа и смещает уже готовый composite-слой на GPU — поэтому анимация остаётся плавной.",
        },
        {
          type: "text",
          content:
            'Классический вопрос: «почему анимация через left дёргается, а через transform плавная?». Ответ — left вызывает reflow, а transform работает только на этапе compositing. Это базовое понимание рендер-конвейера браузера.',
        },

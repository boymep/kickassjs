...
onLCP(send); onINP(send); onCLS(send); onTTFB(send); onFCP(send);`,
        },
        {
          type: "text",
          content:
            'Метрики собираются с реальных пользователей через CrUX — это не результаты Lighthouse в DevTools. Lighthouse — синтетический тест в управляемых условиях, он полезен для регрессий в CI, но не влияет на ранжирование Google. На собеседовании это классическая ловушка: «хороший Lighthouse score» не равен «хорошим Core Web Vitals в реальной среде». Настоящая картина — только web-vitals.js и аналитика поверх него.',
        },
        {
          type: "text",
          content:
            "Важно: метрики собираются с реальных пользователей (RUM), а не в лабораторных условиях. Lighthouse — синтетический тест, полезен но не заменяет реальные данные. На собеседовании спросят как измеряете метрики в продакшне — web-vitals.js + аналитика.",
        },
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: "text",
          content:
            "Представьте нагрузочный тест сервера в тихой серверной и реальный «чёрный пятница» с живым трафиком — первый предсказуем, второй показывает правду. Так и с производительностью: хороший Lighthouse score не означает хорошие метрики в проде. Lighthouse гоняет чистый кэш, фиксированную сеть и одну конфигурацию CPU; реальные пользователи приходят с тёплым кэшем частично, на слабом Android и с 3G. На собеседовании важный вопрос — чему доверять для ranking сигналов Google: только RUM через CrUX, Lighthouse на ранжирование не влияет.",
        },
        ...
        },
        {
          type: "text",
          content:
            "Аналогия: synthetic — это тест-драйв машины на полигоне в идеальных условиях. RUM — это статистика реальных аварий и поломок на дорогах. На собеседовании старший разработчик спросит «а как вы мониторите производительность в проде?» — и ответ «у нас Lighthouse CI» звучит как «мы тестируем только на полигоне».",
        },
        {
          type: "text",
          content:
...
- **dns-prefetch** — только DNS-резолв, дешевле preconnect; fallback для старых браузеров.`,
  ...
  <link rel="prefetch" href="/dashboard" as="document">
        ...
        },
        {
          type: "text",
          content:
            "На интервью часто спрашивают «чем defer отличается от async» — это классика, которую нужно знать как таблицу умножения. Но более интересный вопрос: почему async может навредить? Если скрипт аналитики загрузится раньше основного скрипта и начнёт обращаться к его API — вы получите TypeError. defer гарантирует порядок исполнения, async — нет. Поэтому async только для полностью независимых скриптов.",
        },

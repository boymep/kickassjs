  ...
  chapters: [
    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Выбор стратегии под конкретный продукт — типовая задача системного дизайна. Блог с редко меняющимся контентом подходит для SSG (одна сборка, CDN отдаёт за 30 мс). Новостная лента — для ISR или SSR (данные должны быть свежими). Корзина и личный кабинет — для CSR (за авторизацией, SEO не нужен). Корректный ответ формулируется как обоснование трейдоффа, а не как название одной стратегии.',
        },
        {
          type: 'list',
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Когда SSR оказывается хуже CSR — частый вопрос. Если SSR используется, но на клиент уходит 800 КБ JS для гидрации, TTI (время до первого клика) будет таким же плохим, как у CSR. SSR даёт хороший FCP, но не спасает от тяжёлого бандла. Современный ответ — SSR + RSC, которые уменьшают клиентский бандл.',
        },
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'ISR — это SSG с одной существенной добавкой: страница может устареть и пересобраться в фоне. При `revalidate: 60` первый посетитель после истечения минуты получит старую страницу, но в фоне запустится новая сборка. Следующий пользователь уже увидит свежую версию. ISR и SSR часто путают; ключевое отличие: ISR отдаёт статику, а SSR пересобирает HTML на каждый запрос.',
        },
        {
          type: 'callout',
          calloutType: 'info',
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Гидрация (hydration) — процесс присоединения интерактивности к серверному HTML. Сервер отдаёт готовый HTML, пользователь сразу видит страницу. Затем браузер загружает JS, и React прикрепляет обработчики к DOM. До этого момента страница выглядит нормально, но клики не работают. Это намеренный трейдофф архитектуры: задержка интерактивности компенсируется ранним показом контента.',
        },
        {
          type: 'text',
          content:
            'Если серверный и клиентский HTML не совпадают, React перерисовывает поддерево заново (hydration mismatch). Частые причины: \`Math.random()\`, \`Date.now()\`, браузерные API в коде, который запускается и на сервере. В dev-режиме React выводит подробное предупреждение в консоль; в production молча перерисовывает, теряя преимущества SSR. Частый источник mismatch — обращение к \`localStorage\` или \`window\` в коде, исполняющемся и на сервере, где этих API нет.',
        },
        {
          type: 'code',
          language: 'tsx',
...
hydrateRoot(document.getElementById('root')!, <App />);`,
        ...
        },
        {
          type: 'code',
          language: 'tsx',
          content: `// Антипаттерн: источники недетерминизма
function Now() {
  return <span>{new Date().toLocaleTimeString()}</span>;
...
// Корректно: рендер только после mount.
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'React Server Components — ещё один уровень: компонент исполняется только на сервере, и его код вообще не попадает в браузерный бандл. Большая markdown-библиотека, ORM, криптографические модули — всё это остаётся на сервере. Ключевое отличие RSC от SSR: при SSR компоненты исполняются на сервере, но их код всё равно отправляется в браузер для гидрации; код RSC в клиентский бандл не попадает.',
        },
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'В типичной новостной статье JS требуется только для отдельных интерактивных элементов: кнопки «лайк», формы подписки. Полная гидрация ради нескольких кнопок даёт неоправданный объём JS. Islands-архитектура оставляет остальное статичным HTML и гидрирует только «острова» интерактивности. Это одна из заметных современных альтернатив классической React SPA.',
        },
        {
          type: 'code',
          language: 'astro',
...
//   /app/*                  → CSR     (само приложение за логином)`,
        },
        {
          ...
          calloutType: 'warning',
          content:
            'Типичный антипаттерн — единая стратегия SSR для всего приложения: значительная часть страниц могла бы отдаваться как статика с CDN без потери функциональности. Обратный антипаттерн — единая SSG, при которой персонализация появляется только через CSR-flicker. Стратегия выбирается отдельно для каждого роута.',
        },
        {
          type: 'text',
          content:
            'Типичный системный кейс: «приходит дизайн нового маркетплейса — какие страницы делаются SSR, какие SSG, какие ISR, и почему». Ответ строится не из определений, а из требований к свежести, SEO, персонализации и нагрузке.',
        },
      ],
    },
  ],

...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Ключевой вопрос, который задают на интервью: «когда SSR хуже CSR»? Ответ неочевидный. Если вы делаете SSR, но тащите на клиент 800 кБ JS для гидрации — TTI (время до первого клика) будет таким же плохим, как у CSR. SSR даёт хороший FCP, но не спасает от тяжёлого бандла. Поэтому современный ответ — SSR + RSC, которые уменьшают клиентский бандл.',
        },
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'ISR — это SSG с одной умной добавкой: страница может «протухнуть» и пересобраться в фоне. Вы ставите `revalidate: 60`, и первый посетитель после истечения минуты получит старую страницу, но в фоне запустится новая сборка. Следующий пользователь уже увидит свежую версию. На интервью часто путают ISR и SSR — запомните: ISR отдаёт статику, SSR пересобирает на каждый запрос.',
        },
        {
          type: 'callout',
          calloutType: 'info',
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Гидрация (hydration) — это процесс «оживления» серверного HTML. Сервер отдаёт готовый HTML — пользователь сразу видит страницу. Потом браузер загружает JS и React «прикрепляет» обработчики к DOM. До этого момента страница выглядит нормально, но клики не работают. Это не баг, это архитектурная особенность: вы платите небольшой задержкой интерактивности ради быстрого первого контента.',
        },
        {
          type: 'text',
          content:
...

    {
      ...
      blocks: [
        ...
        },
        {
          type: 'text',
          content:
            'Аналогия: типичная новостная статья — это просто текст и картинки, JS там нужен только для кнопки «лайк» и формы подписки. Гидрировать всю страницу ради двух кнопок — расточительно. Islands говорит: остальное пусть остаётся статичным HTML. На интервью по архитектуре этот паттерн — хороший пример того, что знаете современные альтернативы React SPA.',
        },
        {
          type: 'code',
          language: 'astro',

...
В JavaScript обычный \`Array\` уже является стеком: \`push\` и \`pop\` работают за O(1). С очередью сложнее: \`Array.shift\` стоит O(n), потому что сдвигает все элементы влево. На собеседовании часто проверяют не только знание API, но и понимание, когда стек прячется в задаче (валидные скобки, монотонный стек, итеративный DFS) и как сделать FIFO за O(1).`,
    estimatedMinutes: 28,
    interviewAngle:
      'Интервьюера интересуют три вещи: что \`Array.shift\` стоит O(n) и как обойти это ограничение, в каких задачах применим монотонный стек и как доказать его O(n)-сложность, когда рекурсию стоит переписать на итерацию через явный стек.',
    prerequisites: [],
  },
      ...
      blocks: [
        {
          type: 'text',
          content:
            '**Стек** — структура данных по принципу LIFO: последний добавленный элемент извлекается первым. Аналогия — стопка тарелок: класть и брать можно только сверху.',
        },
        {
          type: 'visualization',
          content: '',
...
\`isEmpty()\` — проверка пустоты.`,
        },
        {
          type: 'text',
          content:
            'В JavaScript для стека не нужен отдельный класс — массив с \`push\` и \`pop\` уже даёт всё необходимое. V8 хранит массив в растущем буфере, поэтому \`push\` амортизированно O(1): иногда буфер удваивается за O(n), но в среднем по серии операций цена константная.',
        },
        {
          type: 'code',
          language: 'javascript',
...
const isEmpty = stack.length === 0;  // false`,
        },
        {
          type: 'text',
          content:
            'Стек присутствует во многих частях разработки: call stack JavaScript-движка — буквально стек вызовов функций. История переходов в браузере — стек (назад идёт pop). Undo / Redo в редакторе — тоже стек. Задачи «валидные скобки», «упрощение пути», «декодирование строки» — все сводятся к паттерну push / pop.',
        },
        {
          type: 'callout',
          calloutType: 'warning',
...
queue.shift();    // 2 → [3]`,
        },
        {
          ...
          calloutType: 'warning',
          content:
            '\`Array.shift\` сдвигает все оставшиеся элементы на одну позицию влево, поэтому стоит O(n). На очереди из миллиона элементов цикл с \`shift\` превращается в O(n²). Это превращает линейный BFS в квадратичный и часто упирается в таймаут (TLE — Time Limit Exceeded) на больших тестах.',
        },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Один из частых вопросов на собеседовании — реализовать очередь через два стека. \`inStack\` принимает новые элементы, \`outStack\` отдаёт старые. Когда \`outStack\` пустеет, элементы перекладываются из \`inStack\` — порядок зеркалится и получается FIFO.',
        },
    ...
    if (this.out.length === 0) {
      while (this.in.length) this.out.push(this.in.pop());
        ...
        },
        {
          ...
          calloutType: 'warning',
          content:
            'Перекладывать нужно только когда \`outStack\` пуст. Если делать это при каждом \`dequeue\`, уже правильный порядок в \`outStack\` сломается, и FIFO перестанет работать. Это популярный уточняющий вопрос на собеседовании.',
        },
        {
          type: 'callout',
          calloutType: 'info',
    ...
    // ─────────────────────────────────────────────────────────────
    {
      ...
      blocks: [
        ...
        { type: 'heading', content: 'Array.shift в горячем цикле' },
        {
          type: 'text',
          content:
            'Самая частая ошибка в задачах на BFS: использовать \`Array.shift\` для очереди. На малых входах работает, на 10⁶ элементах превращает линейный алгоритм в O(n²). Решение — указатель головы, библиотека \`denque\` или двусвязный список.',
        },
...
// хотя в стеке остались две незакрытые скобки.`,
        },
        {
          ...
          calloutType: 'warning',
          content:
            'Финальная проверка \`stack.length === 0\` (или эквивалент) обязательна для алгоритмов, требующих полного «закрытия». \`pop\` на пустом стеке возвращает \`undefined\`, а не бросает ошибку — это и помогает, и подводит.',
        },

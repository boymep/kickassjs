      ...
      blocks: [
        {
          type: 'text',
          content:
            '**Стек** — структура данных по принципу **LIFO** (Last In, First Out): последний добавленный — первый извлечённый. Аналогия: стопка тарелок — берёшь и кладёшь только сверху. В JavaScript специальный класс не нужен — обычный массив с `push()` и `pop()` уже является полноценным стеком.',
        },
        {
          type: 'visualization',
          content: '',
...
const isEmpty = stack.length === 0;  // false`,
        },
        {
          type: 'text',
          content:
            'Стек незаметно присутствует везде в разработке: call stack JavaScript-движка — это буквально стек вызовов функций, который ты видишь в DevTools. История переходов в браузере — стек (назад идёт pop). Undo/redo в редакторе — тоже стек. На собеседовании задачи на стек часто «замаскированы»: «валидные скобки», «упрощение пути», «декодирование строки» — за этим всегда прячется паттерн push/pop.',
        },
        {
          type: 'callout',
          calloutType: 'warning',
      ...
      checkpoint: [Q['sq-q1']!, Q['sq-q6']!, {
        type: 'ordering',
        id: 'sq-ord1',
        description: 'Расставь операции: после трёх push(1), push(2), push(3) — что вернут два pop()?',
        items: [
          'push(1) → стек: [1]',
          'push(2) → стек: [1, 2]',
          'push(3) → стек: [1, 2, 3]',
        ...
        explanation: 'Стек работает по принципу LIFO (Last In, First Out). Последний добавленный элемент извлекается первым. JavaScript массив: push() добавляет в конец, pop() берёт из конца.',
      }],
      ...
      video: {
        ...
        language: 'en',
        durationSec: 6 * 60,
        description: 'Аккуратное введение в стек как абстрактный тип данных с разбором операций.',
      },
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            '**Очередь** — структура FIFO (First In, First Out): первый пришедший уходит первым. В алгоритмах очередь — главная структура для обхода в ширину (BFS): порядок обработки гарантирует нахождение кратчайшего пути.',
        },
        {
          type: 'callout',
          calloutType: 'warning',
          content:
            'В JavaScript нет встроенного Queue — используют массив с `push()` и `shift()`. Но **`shift()` — O(n)**: сдвигает все элементы. Для производительного кода нужна реализация на двух указателях или двусвязном списке.',
        },
        {
          type: 'visualization',
          content: '',
...
queue.shift();    // 2 → [3]`,
        },
        {
          type: 'text',
          content:
            'В JavaScript нет встроенного класса Queue — все используют массив с push() и shift(). Это работает, но shift() стоит O(n): он сдвигает все элементы влево на одно место. На задачах LeetCode это часто не важно, но знать стоит. Если интервьюер спросит «а почему не shift?» — ты готов объяснить и предложить альтернативу с указателем головы.',
        },
        {
          type: 'callout',
          calloutType: 'warning',
      ...
      links: [
        {
          ...
          source: 'mdn',
          language: 'ru',
          note: 'Справочник: shift/unshift работают за O(n) — важно помнить при реализации очереди.',
        },
      ],
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Паттерн классических задач на стек: обходишь слева направо, кладёшь элемент на стек, при встрече «закрывающего» элемента — снимаешь с вершины и проверяешь. Научился одному — остальные по шаблону.',
        },
        {
          type: 'callout',
          calloutType: 'tip',
          content:
            'Задачи на стек часто **маскируются**: «найди ближайший меньший элемент справа» — это монотонный стек. Сигнал: «для каждого элемента найди что-то справа/слева» → думай про стек.',
        },
  ...
  return stack.length === 0;
        ...
        },
        {
          type: 'text',
          content:
            'Ловушка интервьюера в задаче со скобками — попросить тебя обработать граничные случаи: строка из одних открывающих скобок («(((»), строка из одних закрывающих («)))»), пустая строка. Все три решаются одной финальной проверкой `return stack.length === 0`. Если забудешь её — сдашь валидный алгоритм с багом, и опытный интервьюер сразу это поймает.',
        },
  ...
  getMin() { return this.m[this.m.length - 1]; }
      ...
      video: {
        ...
        language: 'en',
        durationSec: 8 * 60,
        description: 'Образцовый пример применения стека на собеседовании: сбалансированные скобки.',
      },
      links: [
        {
          url: 'https://leetcode.com/explore/learn/card/queue-stack/',
      ...
      blocks: [
        {
          type: 'text',
          content:
            '**Монотонный стек** — обычный стек с одним правилом: перед тем как положить новый элемент, выталкиваешь все, которые нарушают нужный порядок (убывающий или возрастающий). Именно это позволяет решать за **O(n)** задачи, которые наивно решаются за O(n²).',
        },
        {
          type: 'visualization',
          content: '',
  ...
  return res;
        ...
        },
        {
          type: 'text',
          content:
            'Монотонный стек — одна из тех тем, где интервьюер намеренно выбирает задачу, которая отделяет тех, кто знает паттерн, от тех, кто не знает. Если не знаешь — пишешь двойной цикл, получаешь O(n²) и TLE. Если знаешь — пишешь двадцать строк с одним while и O(n). Daily Temperatures (LeetCode 739), Next Greater Element, Largest Rectangle in Histogram — все решаются одним шаблоном с разницей в знаке сравнения.',
        },
      ...
      links: [
        {
          ...
          source: 'article',
          language: 'en',
          note: 'Определение монотонного стека и список задач, решаемых за O(n).',
        },
      ],
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Один из самых частых вопросов на интервью: реализовать очередь через два стека. `inStack` принимает новые элементы, `outStack` отдаёт старые. Когда `outStack` пустеет — перекладываем из `inStack`. Порядок разворачивается — получается FIFO.',
        },
        {
          type: 'callout',
          calloutType: 'warning',
          content:
            'Перекладывать нужно **только когда `outStack` пуст**, а не при каждом dequeue. Иначе сломаешь порядок уже правильных элементов в `outStack`. Это классический вопрос-уточнение от интервьюера.',
        },
    ...
    if (this.out.length === 0) {
      while (this.in.length) this.out.push(this.in.pop());
        ...
        },
        {
          type: 'text',
          content:
            'Типичная ловушка при объяснении очереди на двух стеках: интервьюер спрашивает «а перекладывать нужно при каждом dequeue или только когда out пуст?». Правильный ответ — только когда out пуст. Если перекладывать каждый раз, разрушишь уже правильный порядок элементов в out, и FIFO сломается. Один из тех вопросов, где важно не просто знать код, а понимать почему именно так.',
        },
...
console.log(q.dequeue()); // ?`,
      ...
      checkpoint: [Q['sq-q11']!, {
        type: 'match-pairs',
        id: 'sq-mp1',
        description: 'Сопоставь структуру данных с паттерном её применения',
        pairs: [
          { left: 'Stack (стек)', right: 'Undo/Redo, обход DFS, парные скобки' },
          { left: 'Queue (очередь)', right: 'Обход BFS, очередь задач, буфер событий' },
          { left: 'Monotonic stack', right: 'Поиск следующего большего/меньшего элемента' },
          { left: 'Deque (двусторонняя очередь)', right: 'Sliding window maximum, palindrome check' },
        ],
        explanation: "Стек (LIFO) идеален для задач где нужно 'вернуться назад'. Очередь (FIFO) — для обработки в порядке поступления. Монотонный стек — специализированная структура для задач на 'следующий больший элемент'.",
      }],
      links: [
        {
          url: 'https://cp-algorithms.com/data_structures/stack_queue_modification.html',
          ...
          source: 'article',
          language: 'en',
          note: 'Стек/очередь с минимумом и реализация деки на двух стеках — e-maxx канонически.',
        },
      ],
    },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Ошибки в задачах на стеки и очереди — не про алгоритм, а про детали: забытая проверка пустого стека, перепутанные `pop`/`shift`, отсутствующий `return stack.length === 0` в конце.',
        },
        {
          type: 'callout',
          calloutType: 'tip',
          content:
            'Интервьюер не скажет «нашёл баг» — он просто предложит «а попробуй на строке `"((("`». Так обнаруживается забытая финальная проверка. Привыкни проверять граничные случаи перед словом «готово».',
        },
...
// хотя стек содержит две незакрытые скобки.`,
        ...
        },
        {
          type: 'text',
          content:
            'Интервьюер часто не говорит, что нашёл баг — он просто предлагает «а попробуй запустить на строке "((("». Именно так обнаруживается забытая финальная проверка. Перед тем как сказать «готово», мысленно прогони три тест-кейса: строка с ошибкой в начале, строка с ошибкой в конце, и строка из одних открывающих скобок.',
        },

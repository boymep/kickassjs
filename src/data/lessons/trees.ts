...
На собеседовании деревья проверяют умение работать с рекурсией и понимание двух стратегий обхода: DFS (в глубину, через стек или рекурсию) и BFS (в ширину, через очередь). Сложность обхода — O(n) по времени и O(h) по памяти на стек рекурсии, где h — высота дерева. На сбалансированном дереве h = O(log n), на вырожденном (цепочка) — O(n), и тогда возникает риск переполнения стека вызовов.`,
    estimatedMinutes: 32,
    interviewAngle:
      'Интервьюер ждёт обоснованный выбор: DFS или BFS; pre-order, in-order или post-order; рекурсия или итерация со стеком. Каждый выбор должен быть привязан к условию задачи и ограничениям по глубине. Ответ «возьму DFS» без объяснений считается слабым; «здесь нужно знать высоту поддеревьев, значит post-order» — сильным.',
...
N-ary tree — у узла произвольное число детей в массиве \`children\`. Частая форма на фронтенд-собеседовании (DOM, JSON, файловая система).
    ...
    // ─────────────────────────────────────────────────────────────
    {
      ...
      blocks: [
        { type: 'heading', content: 'Переполнение стека вызовов' },
        {
          type: 'text',
          content:
            'JavaScript-движки ограничивают глубину рекурсии (~10⁴ кадров). На дереве, которое выродилось в цепочку (например, отсортированный массив, построенный как BST без балансировки), рекурсивный обход упадёт с \`RangeError: Maximum call stack size exceeded\`. Такой баг особенно опасен тем, что на тестовых данных малой глубины не воспроизводится и проявляется только в продакшене.',
        },
        {
          type: 'callout',
          calloutType: 'tip',

...
\`\`\``,
  ...
  // ваш код
    ...
    hints: [
      'Как сократить вдвое область поиска на каждом шаге, зная только середину текущего диапазона?',
      'Что нужно сделать с границами, если mid оказался меньше target? А если больше?',
      'При каком условии цикл должен продолжаться — и что означает, что он завершился без нахождения элемента?',
    ],
...
\`\`\``,
  ...
  // ваш код
    ...
    hints: [
      ...
      'Как выбрать между двумя соседними кандидатами — тем, что «левее», и тем, что «правее» target?',
      'Что делать при равном расстоянии до обоих кандидатов?',
    ],
...
**Подсказка:** подумай, как использовать сортировку и бинарный поиск для эффективного нахождения ближайшего товара для каждого покупателя.`,
  ...
  // ваш код
    ...
    hints: [
      'Как структура данных влияет на возможность эффективного поиска? Что нужно сделать с исходными данными?',
      'Для каждого покупателя нужно найти «ближайший» товар. Как бинарный поиск помогает быстро сузить кандидатов?',
      'После нахождения позиции — как выбрать между товаром слева и товаром справа от неё?',
    ],
...
\`\`\``,
  ...
  },
  {
    ...
    difficulty: 'easy',
    isContextual: false,
    description: `Перед тобой реализация классического бинарного поиска. Проследи выполнение для массива \`[1, 2, 4, 4, 5]\` и \`target = 4\`. Какой индекс выведет \`console.log\`?

**Подсказка:** посчитай \`mid\` вручную для каждой итерации и определи, на какой именно итерации сработает условие \`arr[mid] === target\`.`,
...
// но классический шаблон останавливается на первом найденном.`,
  },
  {
    ...
    difficulty: 'medium',
    isContextual: false,
    description: `Перед тобой реализация бинарного поиска, в которой содержится баг. На некоторых входах функция **зависает в бесконечном цикле** или возвращает неверный результат.

Найди ошибку и исправь её. Подумай, как сочетаются условие цикла и сдвиг границ.`,
    functionName: 'binarySearch',
  ...
  return -1;
    ...
    hints: [
      'Попробуйте запустить код мысленно на простейшем случае: массив из одного элемента, target совпадает. Что произойдёт?',
      'Подумайте об инварианте: в каком случае элемент в `mid` ещё может быть ответом — и что это говорит об условии цикла?',
    ],
    solutionCode: `function binarySearch(arr, target) {
  let left = 0;
  let right = arr.length - 1;
  ...
  },
  {
    ...
    difficulty: 'medium',
    isContextual: false,
    description: `Перед тобой линейный поиск элемента в отсортированном массиве. Он работает корректно, но за **O(n)** — на массиве из 100 000 элементов это слишком медленно.

Перепиши функцию так, чтобы она использовала **бинарный поиск** и работала за **O(log n)**. Контракт остаётся прежним: вернуть индекс \`target\` или \`-1\`, если элемента нет.
...
**Тест производительности:** на массиве из 100 000 элементов с \`target\` в середине решение должно завершаться за **50 мс**.`,
  ...
  return -1;
    ...
    hints: [
      'Линейный перебор работает, но медленно. Какое ключевое свойство отсортированного массива позволяет отбросить половину элементов за один шаг?',
      'Как изменяется временная сложность — и почему именно так?',
    ],
...
\`\`\``,
  ...
  // ваш код — только O(log n)!
    ...
    hints: [
      'Массив повёрнут, но не хаотичен — в нём всё ещё есть порядок. Что можно сказать об одной из двух половин в любой момент бинарного поиска?',
      'Как использовать «упорядоченную» половину, чтобы понять, в какой из двух стоит искать target?',
    ],
...
\`\`\``,
  ...
  // ваш код
    ...
    hints: [
      'Мы ищем не индекс, а значение. Над чем тогда запускать бинарный поиск?',
      'Как для заданного «кандидата» mid быстро проверить, является ли он k-м наименьшим — используя свойства отсортированных строк?',
      'Как двигать границы поиска в зависимости от результата этой проверки?',
    ],
...
\`\`\``,
  ...
  // ваш код
    ...
    hints: [
      'Нужно найти минимальную скорость. Как проверить, «подходит» ли конкретная скорость — и что это даёт для бинарного поиска?',
      'Над каким пространством значений запускать бинарный поиск — и каковы его границы?',
      'Как посчитать, сколько времени займёт доставка по одному заказу при заданной скорости?',
    ],
...
\`\`\``,
  ...
  // ваш код — O(log(min(m, n)))
    ...
    hints: [
      'Подумайте о задаче как о поиске такого «разреза» в обоих массивах, чтобы слева оказалась ровно половина элементов.',
      'Бинарный поиск идёт по позиции разреза в меньшем массиве. Позиция разреза во втором массиве определяется однозначно.',
      'Корректный разрез: max(leftA, leftB) <= min(rightA, rightB). Когда условие не выполнено — сдвигайте границы.',
    ],
...
**Подсказка**: ответ лежит в диапазоне [max(nums), sum(nums)]. Используйте бинарный поиск **по ответу**.`,
  ...
  // ваш код — бинарный поиск по значению ответа
    ...
    hints: [
      'Прямой перебор разбиений слишком долог. Какой диапазон значений может принимать ответ?',
      'Для фиксированного «лимита» X — как за O(n) проверить, можно ли разбить массив на ≤ k частей, у которых сумма ≤ X?',
      'На основе этой проверки запустите бинарный поиск по X в [max, sum].',
    ],

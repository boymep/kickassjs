...
\`\`\``,
  ...
  // ваш код
    ...
    hints: [
      "Элемент виден, когда его верхний или нижний край попадает в диапазон видимой области. Учитывайте threshold как расширение этого диапазона.",
...
**Подсказка:** посмотрите, на каждой ли итерации цикла происходит принудительный пересчёт (reflow) и сколько всего раз чтение приходится после «грязной» записи.`,
    code: `let reflows = 0;
...
console.log('reflows', reflows, 'total', total);`,
    ...
    hints: [
      ...
      "Проследите значение layoutDirty на каждой итерации. Как меняется total?",
    ],
...
Тестовый счётчик \`getReflowCount()\` считает reflow-ы. Для массива из 5 элементов функция должна вызывать **ровно 1** reflow. Найдите причину и исправьте.`,
  ...
  elements.forEach((el) => {
    ...
    hints: [
      ...
      "Сначала прочитайте всё необходимое, потом запишите всё за один проход.",
    ],

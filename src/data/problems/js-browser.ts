...
\`\`\``,
  ...
  // ваш код
    ...
    hints: [
      "Высота элемента фиксирована, и весь список умозрительно состоит из равных «полок». Если знать положение скролла и размер «полки», можно простым делением узнать, какая «полка» сейчас наверху и сколько их помещается на экране.",
      "Используйте `Math.floor(scrollTop / itemHeight)` для `startIndex` и `Math.ceil(containerHeight / itemHeight)` для количества видимых. `endIndex = Math.min(start + visibleCount, totalItems)` — чтобы не вылезти за границы. `offsetY = startIndex * itemHeight` для трансляции.",
      `Почему \`floor\` для старта и \`ceil\` для количества: первый видимый элемент может быть наполовину «отрезан» сверху — его всё равно нужно отрисовать, а \`floor\` гарантирует, что мы возьмём именно его. Аналогично, последний может быть отрезан снизу — поэтому видимых элементов всегда **на один больше**, чем целиком влезает в контейнер, отсюда \`ceil\`. Граничный случай — когда \`scrollTop + containerHeight\` уходит за конец списка: \`Math.min(..., totalItems)\` отсекает выход за границы, иначе виртуальный список попытается отрисовать несуществующие индексы.
...
\`\`\``,
  ...
  // ваш код
    ...
    testCases: [
      ...
      hints: [
      "Два отрезка (изображение и viewport) пересекаются, когда верх одного ниже низа другого И наоборот. Threshold расширяет нижнюю границу видимой области — изображения чуть ниже экрана начинают грузиться заранее.",
      "Вычислите `viewTop = viewport.scrollY` и `viewBottom = viewport.scrollY + viewport.height + threshold`. Изображение видимо, если `img.top < viewBottom && img.top + img.height > viewTop`. Затем через `.filter(...).map(img => img.url)` соберите массив URL.",
      `Условие пересечения двух отрезков — частая ошибка в задачах с координатами. Простой тест «верх изображения попадает в viewport» неверен: высокое изображение может иметь \`top\` выше экрана, но его середина и низ — внутри. Правильная формула: изображения пересекаются, если **верх одного ниже низа другого И наоборот**. Threshold обычно прибавляют только к нижней границе — это «preload вперёд по направлению скролла», — но в реальных проектах его симметрично прибавляют к обеим, чтобы при быстром скролле обратно изображения не моргали.

С чего начать:
\`\`\`js
function getVisibleImages(images, viewport, threshold = 0) {
...
\`\`\``,
  ...
  // ваш код
    ...
    testCases: [
      ...
      hints: [
      "На экране с высокой плотностью один CSS-пиксель содержит несколько физических. Чтобы картинка не была размытой, её реальная ширина должна быть не меньше `viewportWidth * dpr`. Среди подходящих вариантов выбирают самый маленький, иначе трафик будет лишним.",
      "Отсортируйте `srcset` по `width`. Через `find` найдите первый, чья ширина `>= viewportWidth * dpr`. Если такого нет — возьмите последний (самый большой). Сортировка не должна мутировать вход — используйте `[...srcset].sort(...)`.",
      `Главная идея — баланс между трафиком и качеством. Картинка меньше нужного даст «мыло», особенно на Retina-экранах, где \`dpr = 2\` или больше. Картинка крупнее нужного — это лишние мегабайты, и на мобильной сети это видно сразу. Поэтому правило «минимальная подходящая или самая большая, если все меньше». Тонкость с мутацией: \`.sort()\` сортирует входной массив **на месте** — если вызвать его на \`srcset\`, попавшем в функцию по ссылке, вы поменяете и внешние данные. \`[...srcset].sort()\` или \`srcset.slice().sort()\` создают копию.

С чего начать:
\`\`\`js
function pickSrcset(srcset, viewportWidth, dpr) {
...
\`\`\``,
  ...
  // ваш код
    ...
    testCases: [
      ...
      hints: [
      "Селектор может содержать несколько имён классов, склеенных без пробелов (`.btn.active`). Из такой строки нужно извлечь все имена и проверить, есть ли среди них хоть одно из набора используемых.",
      "Найдите все вхождения паттерна `.имя_класса` через регулярку `/\\.([\\w-]+)/g` и `matchAll`. Используемые классы переведите в `Set` — проверка `.has(cls)` за O(1). Через `.filter` оставьте правила, в которых хотя бы один класс из селектора есть в наборе.",
      `Почему \`Set\` лучше массива: \`arr.includes(x)\` работает за O(n), \`set.has(x)\` — за O(1). На паре сотен правил и десятке классов разница незаметна, но настоящие CSS-бандлы — это десятки тысяч правил и сотни используемых классов, и O(n*m) против O(n) превращается в осязаемые секунды. Это упрощённая модель critical CSS — реальные инструменты учитывают также комбинаторы (\`>\`, \`+\`), псевдо-классы (\`:hover\`), media queries и контекст (на каком DOM-узле класс реально используется).

С чего начать:
\`\`\`js
function extractCriticalCSS(rules, usedClasses) {
...
function applyStyles(elements, stylesFn) {
  // ваш код
    ...
    testCases: [
      ...
      hints: [
      "Задача — на разделение чтения и записи: сначала собрать значения с помощью функции-читателя, потом одной фазой применить изменения. В этой постановке методы массива решают всё.",
      "Для `batchRead` нужен метод, который возвращает новый массив той же длины с применённой к каждому элементу функцией — это `Array.prototype.map`. Для `applyStyles` нужен метод-итератор, который не возвращает массив, но передаёт элемент и индекс — `forEach`.",
      `Зачем вообще разделять read и write фазы — это профилактика **layout thrashing**: чтения вроде \`offsetHeight\`, \`getBoundingClientRect\`, \`scrollTop\` принудительно заставляют браузер пересчитать layout, если перед этим была запись стилей. Чередуя \`read → write → read → write\`, вы получаете reflow на каждой итерации. Если же все чтения идут пачкой, layout пересчитывается один раз, а затем серия записей просто помечает дерево «грязным» — браузер пересчитает его при следующем кадре, разом для всех.

С чего начать:
\`\`\`js
function batchRead(elements, reader) {
  // ...
}
function applyStyles(elements, stylesFn) {
  // ...
...
function applyStyles(elements, stylesFn) {
  ...
  },
  {
    ...
    difficulty: "medium",
    isContextual: false,
    description: `Перед вами упрощённая модель браузера: переменная \`reflows\` увеличивается каждый раз, когда **читается** геометрия элемента, а layout перед этим стал «грязным» (\`layoutDirty = true\`). Запись стиля помечает layout как «грязный». Чтение \`offsetWidth\` сбрасывает флаг и при необходимости прибавляет \`+1\` к \`reflows\`.

Это рабочая модель **layout thrashing** — ситуации, когда чтения и записи перемешаны и браузер вынужден пересчитывать layout снова и снова. Введите последнюю напечатанную строку: значение \`reflows\` и финальный \`total\`, разделённые пробелом — в том же формате, что выводит \`console.log\`.`,
    code: `let reflows = 0;
...
console.log('reflows', reflows, 'total', total);`,
    expected: "reflows 3 total 60",
      hints: [
      "Шаг 1: счётчик `reflows` увеличивается только когда мы **читаем** при «грязном» layout. Запись делает layout грязным, чтение — сбрасывает флаг. Значит, важно понять, в каком состоянии флаг входит в каждое чтение.",
      "Шаг 2: начало — `layoutDirty = true`. Первая итерация: READ (dirty → reflow=1, флаг сбрасывается), WRITE (флаг снова dirty). Вторая итерация: READ (dirty → reflow=2), WRITE. Третья: READ (dirty → reflow=3), WRITE. Каждое чтение видит флаг, испорченный предыдущей записью.",
      "Шаг 3: `total = 10 + 20 + 30 = 60`. Итоговый вывод: `reflows 3 total 60`.",
    ],
...
// поэтому каждое чтение принудительно вызывает reflow.`,
  },
  {
    ...
    id: "jsb-p7",
    topicId: "js-browser",
    title: "Найдите баг: слишком много reflow при обходе элементов",
    difficulty: "easy",
    isContextual: false,
...
Тестовый счётчик \`getReflowCount()\` считает reflow-ы. Для массива из 5 элементов функция должна вызывать **ровно 1** reflow. Найдите причину и исправьте.`,
  ...
  elements.forEach((el) => {
    ...
    testCases: [
      ...
      hints: [
      "Шаг 1: каждая запись делает layout «грязным», каждое следующее чтение принуждает браузер пересчитать его заново. В цикле, где `read → write → read → write`, мы платим reflow на каждой итерации.",
      "Шаг 2: разделите фазы. Сначала пройдитесь по всем элементам и соберите ширины в массив через `map` — это один reflow на все чтения подряд. Затем во втором проходе запишите удвоенные значения — записи layout не пересчитывают.",
      "Тот же приём — разделение фаз чтения и записи — нужен не только для `offsetWidth`, но и для всего семейства «forced reflow»-свойств: `clientHeight`, `getBoundingClientRect`, `getComputedStyle`, `scrollTop`. Все они вынуждают браузер немедленно пересчитать layout, если до этого была запись. Поэтому в реальных приложениях с виртуальным скроллом и анимациями работа над DOM в обработчике `scroll` или `resize` строится строго в два прохода. Граничный случай — пустой массив: ни одного `read`, ни одного `write`, и счётчик reflow остаётся нулём.",
    ],
...
Перепишите функцию так, чтобы все узлы сначала собирались в **DocumentFragment**, а в конце фрагмент **одной операцией** вставлялся в \`parent\`. Корректность: финальное содержимое \`parent.children\` должно совпадать с прежним — элемент к элементу, в том же порядке.
...
В тестах используется mock-DOM со счётчиком \`getInsertCount()\`. Правильная реализация делает ровно **1** вставку в \`parent\` (фрагмент), независимо от количества items.`,
  ...
  for (const item of items) {
    ...
    testCases: [
      ...
      hints: [
      "Каждая вставка узла, лежащего в реальном DOM, провоцирует пересчёт layout и paint. Нужно собрать все узлы в «оффлайн»-контейнере, а в реальный DOM их перенести одной операцией.",
      "Используйте `document.createDocumentFragment()`: его дети не участвуют в layout, пока сам фрагмент не добавлен в документ. Соберите всё через `fragment.appendChild(node)` в цикле, затем `parent.appendChild(fragment)` — это единственная «настоящая» вставка в parent.",
      `Почему \`DocumentFragment\` — а не просто временный \`<div>\`: фрагмент при вставке в DOM **исчезает сам**, в дерево попадают только его дети. Если же собирать в \`<div>\` и вставить его, в результате вы получите лишний обёрточный узел, который сломает селекторы и CSS. И ещё фундаментальная причина: пока узел не подсоединён к документу, любые операции с ним (создание детей, навешивание классов) **не вызывают reflow** — браузер просто меняет JS-объекты в памяти. Полноценный пересчёт layout случается только в момент \`parent.appendChild(fragment)\`.

С чего начать:
\`\`\`js
function buildList(parent, items, createNode) {
...
Верните объект \`{ destroy() }\` для очистки.`,
  ...
  return { destroy() {} };
    ...
    testCases: [
      ...
      hints: [
      "Чтобы скроллбар отражал размер всего списка, нужен «пустой» элемент с суммарной высотой `items.length * itemHeight`. Видимые элементы рендерятся отдельно и позиционируются поверх — так пользователь видит только нужный фрагмент.",
      "Создайте spacer с `height = totalHeight` для скроллбара. На событие `scroll` пересчитывайте `startIdx = Math.floor(scrollTop / itemHeight) - overscan` и `endIdx = Math.ceil((scrollTop + containerH) / itemHeight) + overscan`. Видимые элементы позиционируйте через `position: absolute; transform: translateY(i * itemHeight)`. В `destroy` снимите listener и очистите контейнер.",
      `Overscan — это маленький буфер (3–5 элементов) сверху и снизу видимой области. Без него при быстром скролле пользователь успевает увидеть «пустоту» там, где элементы ещё не отрендерились. Почему \`transform: translateY\`, а не \`top: Npx\`: \`transform\` обрабатывается на этапе композиции (GPU) и не вызывает reflow, тогда как изменение \`top\` — это полноценная перекомпоновка. И обязательно снимайте listener в \`destroy\`: если \`container\` живёт дольше виртуального списка (например, в SPA при смене страницы), \`scroll\`-обработчик удержит весь массив \`items\` через замыкание — классическая утечка памяти.

С чего начать:
\`\`\`js
function createVirtualList({ container, itemHeight, items, renderItem }) {
  function render() {
    // ...
  }
...
\`\`\``,
  ...
  return {
    ...
    testCases: [
      ...
      hints: [
      "Каждый новый вызов должен «погасить» предыдущий — для этого нужен объект, способный сигнализировать предыдущему запросу: «всё, ты больше не нужен». И этот объект надо помнить между вызовами.",
      "Используйте `AbortController`: храните текущий контроллер в замыкании. При каждом `fetch` сначала вызывайте `currentController?.abort()`, потом создавайте новый и передавайте его `signal` в `fetch(url, { signal })`. На `AbortError` верните «вечный» промис (`new Promise(() => {})`), чтобы устаревший вызов не реджектил пользовательский код.",
      `Почему отдельно обрабатываем \`AbortError\`: когда мы сами вызываем \`abort()\`, \`fetch\` реджектится именно с этой ошибкой. Если её не отловить — пользовательский код, который сделал \`await manager.fetch(...)\` для **отменённого** запроса, получит непредвиденный реджект. Возврат «вечного» промиса (\`new Promise(() => {})\`) — компромисс: устаревший вызов просто никогда не завершится и не сломает поток. Альтернатива — возвращать \`undefined\` или специальный sentinel, но тогда вызывающий код должен учитывать оба варианта. Все **другие** ошибки (сетевые, парсинг) обязательно пробрасываем дальше — иначе глотаем баги.
...
Такая утилита постоянно встречается в React-проектах, и её часто просят реализовать на собеседовании.`,
  ...
  // ваш код
    ...
    testCases: [
      ...
      hints: [
      "Аргументы бывают четырёх видов: строка (просто берём), массив (рекурсивно обрабатываем каждый элемент), объект (берём ключи с truthy-значениями), всё falsy — игнорируем. Естественно решать через рекурсивную функцию-помощник, которая накапливает кусочки в общий массив.",
      "Внутри функции-помощника проверяйте `typeof x`: для строки `push`; для массива — рекурсивно `add` каждого элемента; для объекта — `Object.entries(x)` и `push` ключа, если значение truthy. В конце `parts.join(' ')`.",
      `Тонкости порядка проверок. Проверку \`if (!x) return\` нужно делать **первой** — иначе \`null\` пролетит в ветку \`typeof === 'object'\` (потому что \`typeof null === 'object'\`) и упадёт на \`Object.entries(null)\`. Похожая ловушка с массивами: \`Array.isArray\` должен идти **до** общей проверки на \`'object'\`, иначе массив будет обработан как объект и его индексы попадут в результат как имена классов. Граничный случай — число \`0\` и пустая строка: считаются falsy и игнорируются, что обычно правильно (например, \`{ 'col-0': index !== 0 }\` интуитивно не должно добавлять \`col-0\` при \`index === 0\`).
...
\`\`\``,
  ...
  // ваш код — requestAnimationFrame, не setTimeout
    ...
    testCases: [
      ...
      hints: [
      "Браузер перерисовывается ~60 раз в секунду. Если обработчик вызывается чаще, лишние вызовы бесполезны — нужно привязать их к ритму отрисовки и при этом запомнить аргументы только **первого** вызова в окне.",
      "Используйте флаг `scheduled` и `requestAnimationFrame`. На первый вызов: ставите флаг, сохраняете аргументы, планируете RAF. На последующие — пока флаг стоит, просто `return`. В колбэке RAF: вызываете `fn` с сохранёнными аргументами, сбрасываете флаг и сохранённые args.",
      `Почему \`requestAnimationFrame\`, а не \`setTimeout(fn, 16)\`: RAF синхронизирует обработчик с **кадром браузера** — он запустит вашу функцию ровно тогда, когда браузер готов перерисовывать, и не вызовет вовсе, если вкладка свернута (а \`setTimeout\` будет тикать впустую и тратить батарею). Тонкий вопрос — кто выигрывает: первый или последний вызов в окне. Здесь первый: мы сохраняем \`savedArgs\` один раз и следующие вызовы игнорируем. Для обработчиков \`mousemove\` это даёт «лидирующее» поведение — реакция мгновенно при движении, без задержки до конца кадра. Для \`scroll\`-обработчиков иногда нужен обратный подход (последний выигрывает) — тогда \`savedArgs\` обновляется на каждом вызове.
...
Хорошее упражнение на знание DOM API и работы с формами.`,
  ...
  // ваш код — без new FormData()
    ...
    testCases: [
      ...
      hints: [
      "Форма знает все свои поля через одну коллекцию. Решение — пройтись по ней, для каждого поля принять решение (включать ли его) и аккуратно собрать значения, превращая повторяющиеся `name` в массив.",
      "Обходите `form.elements`. Пропускайте: без `name`, тип `submit`/`button`/`reset`, неотмеченные `checkbox`/`radio`. Если ключ уже есть в результате — превращайте его в массив (или дополняйте существующий), иначе кладите как обычное значение.",
      `Логика «превратить в массив при втором появлении» — стандартное поведение нативного \`FormData.getAll(name)\` и формы, отправленной классическим способом. Тонкость: при первом совпадении мы записываем строку, и нужно потом её аккуратно завернуть в массив \`[old, new]\`. Если же сразу всегда писать массив, формат поменяется — каждое значение будет в массиве, даже одиночные, что обычно неудобно. Граничные случаи, которые часто пропускают: \`<select multiple>\` — нативно выдаёт массив только через \`el.selectedOptions\`, а \`el.value\` вернёт только первое значение, поэтому простая реализация не покроет multi-select полностью.

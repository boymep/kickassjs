...
console.log(a()); // ?`,
    ...
    hints: [
      ...
      "Следите за состоянием a и b независимо — они никогда не пересекаются.",
    ],
...
// a и b не пересекаются — это и есть смысл замыкания.`,
  ...
  return fns;
    ...
    hints: [
      "Подумайте, какую область видимости создаёт `var` в цикле `for` и чем она отличается от блочной.",
...
**Подсказка:** можно использовать вспомогательное замыкание или \`Map\`-кеш на уровне модуля.`,
  ...
  return fib(n - 1) + fib(n - 2);
    ...
    hints: [
      "Попробуйте вручную посчитать, сколько раз вычисляется fib(5) при наивном fib(7). Чем больше n, тем хуже.",
...
function makeStack() {
  return {
    ...
    hints: [
      "Найдите, где объявлен `items`. Какую область видимости он имеет относительно функции `makeStack`?",
...
\`\`\``,
  ...
  // ваш код
    ...
    hints: [
      'Внутри замыкания держите переменную `timerId`. На каждый новый вызов — `clearTimeout(timerId)`, затем `timerId = setTimeout(...)`.',
      'Помните: `fn` должна получить аргументы **последнего** вызова — собирай их через rest-параметры (`...args`).',
      'Чтобы сохранить `this` для методов объекта, используйте обычную функцию (не стрелочную) и `fn.apply(this, args)`. Для прохождения тестов хватит и `fn(...args)`.',
    ],

...
// Итог: "undefined", затем "7".`,
  },
  {
    ...
    difficulty: 'medium',
    isContextual: false,
    description: `Класс \`Counter\` должен увеличивать внутренний счётчик и возвращать новое значение. Но автор задал метод \`increment\` как **стрелочную функцию на прототипе** — а это ломает поведение \`this\`. Найди ошибку и перепиши метод как обычный, чтобы тесты прошли.
...
Counter.prototype.increment = () => {
  ...
  },
  {
    ...
    difficulty: 'medium',
    isContextual: false,
    description: `Класс \`Tracker\` написан в старом стиле: используется идиома \`var that = this\` и \`.bind(this)\` в конструкторе для каждого метода. Перепиши класс на современный синтаксис **ES2022**:

- Используй **поля класса** (class fields) со стрелочными функциями — вместо ручного \`bind\` в конструкторе.
- Замени \`that = this\` внутри методов на стрелочные функции — они захватят \`this\` лексически.
- Сохрани публичный API: методы \`add(item)\`, \`countAll()\`, \`reset()\` и поле \`items\`.
...
\`\`\``,
  ...
  },
  {
    ...
    difficulty: 'hard',
    isContextual: false,
    description: `Внимательно проследи, какой \`this\` у каждого вызова: класс, стрелочная функция, метод, извлечённый в переменную.
...
//   → выводит "1".`,
  },
  {
    ...
    topicId: 'js-this',
    kind: 'predict-output',
    title: 'Предскажи вывод: this в методе vs в распакованной функции',
    difficulty: 'easy',
    isContextual: false,
...
//   undefined.name → TypeError → "TypeError".`,
  },
  {
    ...
    difficulty: 'hard',
    isContextual: false,
    description: `Реализуй \`autoBind(obj)\` — функцию, которая **привязывает (\`.bind\`) все собственные методы объекта \`obj\` к самому \`obj\`**, заменяя их на привязанные версии. Возвращает тот же объект (мутирует его), чтобы вызовы можно было выстраивать в цепочку.
...
\`\`\``,
  ...
  // ваш код
    ...
    hints: [
      ...
      'Для каждого свойства проверь `typeof obj[key] === "function"` и замени на `obj[key].bind(obj)`.',
  ...
  },
  {
    ...
    difficulty: 'hard',
    isContextual: false,
    description: `Реализуй метод \`Function.prototype.myApply(ctx, argsArray)\`, который ведёт себя как нативный \`Function.prototype.apply\`:
...
Хорошая задача на понимание того, что \`this\` определяется именно **способом вызова**, а не местом объявления функции.`,
  ...
  // ваш код — без apply/call/bind
    ...
    hints: [
      'Чтобы вызвать `fn` с заданным `this`, присвой её как временное свойство `ctx`, вызови через точку и удали свойство.',
      'В качестве ключа возьми `Symbol()` — он уникален и не пересечётся с существующими свойствами объекта.',
      'Не забудь обработать случай, когда `ctx` равен `null` или `undefined`: подставь `globalThis`.',
    ],

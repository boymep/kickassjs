...
\`\`\``,
  ...
  handleClick() {
    ...
    hints: [
      ...
      "Два подхода: либо в конструкторе сделать `this.handleClick = this.handleClick.bind(this)`, либо объявить метод как **class field со стрелочной функцией** (`handleClick = () => { ... }`) — стрелка лексически захватит `this` экземпляра.",
      `Разница между двумя подходами не косметическая. \`bind\` в конструкторе создаёт **новую функцию-обёртку на каждом экземпляре** — это та же память, что и классовые поля со стрелками. А вот обычный метод класса живёт **на прототипе** — один на всех, но теряет \`this\` при вытаскивании. Выбор зависит от сценария: если метод никогда не передаётся как колбэк — оставьте на прототипе ради памяти; если передаётся часто (обработчики событий, таймеры) — поле со стрелкой или \`bind\` в конструкторе.
...
Counter.prototype.increment = () => {
  ...
  return this.value;
    ...
    hints: [
      "Стрелочная функция захватывает `this` в момент объявления (лексически) и игнорирует, как её вызвали. Здесь стрелка объявлена в модульной/глобальной области, поэтому `this` внутри неё — не экземпляр. Нужен метод, у которого `this` определяется способом вызова.",
      "Обычные методы класса (или функции в `prototype`) получают `this` от вызывающего объекта. Уберите стрелку и объявите `increment()` как обычный метод внутри `class Counter`.",
      "Главный урок — **стрелочные функции и методы решают разные задачи**. Стрелка хороша, когда нужно «приклеить» `this` к месту объявления (типичный пример — колбэки внутри методов класса). Но в роли самого метода она категорически не работает: её `this` определяется в момент создания функции, а не в момент вызова `counter.increment()`. Тут хочется именно «контекстного» `this` — поэтому метод должен быть обычной функцией, и место ему — внутри `class`, а не на прототипе через присваивание.",
    ],
...
Тесты проверяют, что методы продолжают работать даже при потере контекста — если метод вытащен в отдельную переменную или передан как колбэк. Отдельный тест на производительность проверяет, что массовое создание экземпляров укладывается в отведённое время.`,
  ...
  reset() {
    ...
    hints: [
      ...
      `Тут важна цена решения. **Классовые поля со стрелками** дают «бесплатную» привязку \`this\` — методы переживают вытаскивание в переменную и передачу как колбэк. Но каждый экземпляр получает **свой собственный** комплект функций, а не одну общую копию на прототипе. Для нескольких десятков экземпляров это незаметно, а вот при создании тысяч объектов (как в perf-тесте) разница в памяти и времени конструирования становится заметной. Современный движок справляется хорошо, но это не «бесплатно» — выбор «поле со стрелкой vs метод на прототипе» всегда компромисс.

...
\`\`\``,
  ...
  put(key, value) {
    ...
    hints: [
      "Нужно знать, какой элемент использовался «давнее всего». Какая структура данных в JS сохраняет порядок вставки?",
      "При get элемент становится «свежим». Как переместить его в конец структуры, не создавая отдельного списка?",
    ],
...
\`\`\``,
  ...
  // ваш код
    ...
    hints: [
      "Как строить ключ кеша из произвольного набора аргументов?",
      "Кеш с TTL отличается от обычного тем, что хранит не только значение. Что ещё нужно сохранить, чтобы знать, устарел ли результат?",
    ],
...
\`\`\``,
  ...
  release(resource) {
    ...
    hints: [
      "Пул управляет тремя состояниями: свободные ресурсы, активные, ожидающие. Что нужно знать, чтобы решить — выдать ресурс сразу, создать новый или поставить в очередь?",
      "Когда ресурс возвращается — кого обслуживать первым: ожидающих или класть в «запас»?",
    ],
...
\`\`\``,
  ...
  // ваш код — возвращает функцию add(item)
    ...
    hints: [
      "add возвращает Promise. Как «отложить» выполнение batch до нужного момента и всё-таки вернуть результат конкретному вызывающему?",
      "Когда нужно делать flush: принудительно при переполнении или по таймеру — что запускается раньше.",
    ],
...
\`\`\``,
  ...
  // ваш код — возвращает обёрнутую функцию
    ...
    hints: [
      "Circuit Breaker — конечный автомат. Какие состояния нужны и какие события вызывают переходы между ними?",
      "В состоянии OPEN запросы не проходят, но нужно знать, когда снова попробовать. Как это определить?",
    ],
...
**Подсказка:** используйте \`Map\` и его свойство «порядок вставки» — самый старый ключ это \`map.keys().next().value\`.`,
  ...
  return function (...args) {
    ...
    hints: [
      "Кеш растёт без ограничений. Что нужно проверять перед каждой новой записью?",
      "При переполнении — какой элемент удалить? Какое свойство Map помогает найти «самый старый»?",
    ],
...
Идея: \`Map\` сохраняет порядок вставки. При \`get\` удаляйте и вставляйте ключ заново — он переедет в конец как «свежий». При \`put\` в полный кэш удаляйте \`map.keys().next().value\`.`,
  ...
  put(key, value) {
    ...
    hints: [
      "Текущая реализация использует два массива. Почему поиск по ключу медленный — и как это исправить?",
      "Нужна структура, которая и быстро ищет по ключу, и сохраняет порядок вставки. Что подходит?",
    ],
    solutionCode: `class LRUCache {
  constructor(capacity) {
    this.capacity = capacity;
...
Найдите и исправьте **утечку памяти**. Кеш должен хранить **не более 1000** записей — при добавлении 1001-й удалять самую старую.`,
  ...
  get(key) {
    ...
    hints: [
      "Обычный объект не сохраняет надёжный порядок вставки. Какая структура данных это гарантирует?",
      "Как определить, какой элемент был добавлен раньше всех, чтобы вытеснить его при переполнении?",
    ],
...
Перепишите через \`Set\` / \`Map\` так, чтобы алгоритм работал за **O(n)**.`,
  ...
  return duplicates.sort((a, b) => a - b);
    ...
    hints: [
      "Нужно отличать элементы, которые встречались один раз, от тех, что встречались больше одного. Как отследить это за один проход?",
    ],
...
class WorkerPool {
  ...
  destroy() {
    ...
    hints: [
      "Как распределять задачи между несколькими воркерами? Нужно ли учитывать загруженность каждого?",
      "run возвращает Promise. Как связать его с ответом от конкретного воркера?",
    ],
...
\`\`\``,
  ...
  // ваш код — обрабатывайте построчно, не копите всё в памяти!
    ...
    hints: [
      "Стрим приходит чанками, но строки не совпадают с границами чанков. Как накапливать данные и разбивать по строкам?",
      "Последняя строка может не заканчиваться переносом. Когда нужно обработать оставшийся буфер?",
    ],

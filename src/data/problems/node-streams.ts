...
\`\`\``,
  ...
  // ваш код
    ...
    hints: [
      "Подумайте, как «прокатить» одно значение через несколько функций так, чтобы каждая получала результат предыдущей.",
      "Это классический сценарий для `Array.prototype.reduce`: начальное значение — `value`, аккумулятор обновляется как `fn(acc)`. Если функций нет, `reduce` без callback вернёт само начальное значение.",
      `Это синхронный аналог \`stream.pipeline\`: одно значение проходит через цепочку преобразований. Важно — порядок применения здесь слева направо (\`compose\` в FP-смысле наоборот, справа налево). Если в одной из функций кинется исключение, оно прервёт всю цепочку, потому что \`reduce\` идёт синхронно — обработка ошибок ложится на вызывающий код, а не на саму pipeline.
...
\`\`\``,
  ...
  // ваш код
    ...
    hints: [
      "Идите по строке шагами длиной `size` и собирайте подстроки.",
      "Цикл `for (let i = 0; i < str.length; i += size)` и `str.slice(i, i + size)` дают нужные куски без хитростей с длиной. Последний кусок получится короче автоматически, если строка не делится нацело.",
      `Граничный случай — \`size\` больше длины строки: \`slice\` корректно вернёт всю строку, отдельная ветка не нужна. А вот при \`size <= 0\` цикл с \`i += size\` зациклится насмерть или не сделает ни шагу — в реальном коде стоит явно валидировать \`size\`. Ещё тонкость: при работе с emoji и составными символами Unicode индексация по \`length\` режет суррогатные пары пополам — для таких данных лучше идти через \`Array.from(str)\` и собирать обратно.
...
\`\`\``,
  ...
  pipe(otherStream) {
    ...
    hints: [
      "Стрим должен где-то хранить обработанные чанки между `write` и `read`, а ещё помнить, кому пересылать данные при наличии подключения.",
      "Заведите массив-буфер и поле `pipeTo`. В `write` примените `transformFn(chunk)`, кладите результат в буфер и, если есть `pipeTo`, тут же вызовите `pipeTo.write(result)`. `read` — это `buffer.shift() ?? null`. `pipe(other)` сохраняет ссылку и возвращает `other`, чтобы можно было чейнить.",
      `Главная разница с настоящим Node.js Transform — здесь нет backpressure: при наличии \`pipeTo\` каждый \`write\` синхронно проталкивает данные дальше, и медленный downstream не может сказать «подожди». В реальных Transform-стримах \`pipe\` подписывается на событие \`drain\` и приостанавливает upstream через \`pause()\`. Также возврат \`other\` из \`pipe\` — это та же сигнатура, что у Node.js: именно благодаря ей возможны цепочки \`a.pipe(b).pipe(c)\`.
...
\`\`\``,
  ...
  getBufferSize() {
    ...
    hints: [
      "Запись должна накапливаться, пока её не наберётся достаточно, и сбрасываться либо при переполнении, либо по явному запросу.",
      "Храните поле-буфер (строку), в `write` делайте `this.buffer += data` и сразу проверяйте `this.buffer.length >= this.bufferSize` — тогда зовите `this.flush()`. В `flush()` сначала проверяйте, что буфер непустой, и только тогда вызывайте `flushFn(buffer)` и обнуляйте его.",
      `Тонкий момент — что считать «переполнением». Если порог \`>= bufferSize\`, то один большой \`write\` (например, на 1 МБ при лимите 10 КБ) уйдёт в \`flushFn\` целиком одним куском, а не разрежется. Это обычно ок для логов, но критично для сетевых пакетов фиксированного размера — там нужна явная нарезка. И ещё — \`flush\` пустого буфера не должен дёргать \`flushFn\`: иначе при graceful shutdown получите лишний пустой пакет в выходной поток.
...
\`\`\``,
  ...
  // ваш код
    ...
    hints: [
      "Разбейте текст на отдельные строки, обработайте каждую и уберите ненужные.",
      "Цепочка `text.split('\\n').map(l => l.trim()).filter(l => l.length > 0)` решает все три требования сразу: разрезает по переводу строки, обрезает пробелы и убирает пустые элементы.",
    ],
...
console.log('after');`,
    expected: "before\ndata: A\ndata: B\nafter\nend",
    hints: [
      "Внимательно посмотрите на `makeStream`: какие вызовы происходят синхронно прямо во время `push`, а какие откладываются.",
      "`push(chunk)` для не-null чанка синхронно зовёт `handlers.data(chunk)` — это эмуляция flowing-режима. `push(null)` ставит вызов `handlers.end` в микрозадачу через `Promise.resolve().then`, то есть он выполнится только после того, как закончится текущий синхронный код.",
      `Пошагово:
- 'before' — синхронный console.log до push.
- push('A') → 'data: A' прямо в момент вызова.
- push('B') → 'data: B'.
- push(null) ставит end в микрозадачу; синхронный код продолжается → 'after'.
- Очередь микро: запускается end → 'end'.
- Итого: before, data: A, data: B, after, end.`,
    ],
...
Сигнатура: \`runPipeline(source, transform, sink) → string[]\` (массив сообщений ошибок в порядке появления).`,
  ...
  return errors;
    ...
    hints: [
      "Сейчас одна ошибка в `transform` пробрасывается наружу и обрывает цикл — нужно её локализовать на уровне одной итерации, чтобы остальной поток продолжал работать.",
      "Оберните тело цикла в `try { transform(item); sink.push(out); } catch (err) { errors.push(err.message); }`. Это аналог подписки на `error` у Transform-стрима: ошибка не убивает поток, а уходит в боковой канал.",
      "Это типичный «обрыв пайплайна» из мира стримов: один сбойный элемент валит весь поток, и подписчики downstream остаются с неполными данными. В реальном Node.js та же проблема решается через `stream.pipeline` с обработчиком ошибок или через Transform-стрим, у которого в callback `cb(err, ...)` ошибка уводится в боковой канал `error`, а основной поток продолжает работать. Здесь — синхронный try/catch на уровне одной итерации.",
    ],
...
Корректность: результат должен совпадать. Производительность не критична (тест на корректность только) — главное, не делать лишних проходов и промежуточных массивов размером O(N).`,
  ...
  return filtered.length;
    ...
    hints: [
      "Главная проблема в исходном коде — два прохода и два промежуточных массива. Замените их одной итерацией со счётчиком.",
      "Сделайте async-генератор, который `yield`-ит строки по одной, и пройдите по нему через `for await...of`. На каждой итерации проверяйте `line.includes('ERROR')` и инкрементируйте счётчик — никаких `map`/`filter`.",
      "Главный выигрыш стримов — постоянная память O(1) вместо O(N). Исходный код через `map` + `filter` сначала создаёт массив всех trimmed-строк, потом ещё один с отфильтрованными — на гигабайтном логе это десятки гигабайт в RSS и почти гарантированный OOM. Async-генератор обрабатывает по одной строке и тут же её отпускает — сборщик мусора освобождает память сразу. В реальном проекте здесь обычно стоит `readline.createInterface(fs.createReadStream(...))` — он именно так и работает под капотом.",
    ],
...
async function parseCsvStream(readable) {
  // ваш код
    ...
    hints: [
      "Стрим приходит чанками, и одна CSV-строка может быть разорвана между ними. Подумайте, как накапливать данные и обрабатывать только полные строки.",
      "Подпишитесь на `readable.on('data')`, держите буфер: `buffer += chunk.toString()`, режьте `buffer.split('\\n')`, последний элемент возвращайте обратно в буфер. Первую распарсенную строку запоминайте как заголовки. Запятые внутри кавычек игнорируйте — для этого ведите флаг `inQuotes` в парсере одной строки.",
      `Граница чанка может разрезать всё что угодно: имя поля, значение, даже середину UTF-8 символа. Поэтому правило простое — \`split('\\n')\` и **обязательно** возвращайте последний элемент обратно в буфер: только пришедшие до \`\\n\` строки можно считать «целыми». На \`end\` обработайте остаток буфера, иначе потеряете последнюю строку, если файл не оканчивается переводом. И не забудьте \`readable.on('error', reject)\` — без него ошибка чтения зависнет промис навечно.
...
Это ключевой паттерн Node.js streams — избегать переполнения памяти при разной скорости producer/consumer.`,
  ...
  // ваш код — используйте backpressure!
    ...
    hints: [
      "Если потребитель не успевает за producer'ом, нужно временно прекратить чтение, иначе память заполнится непрочитанными чанками.",
      "`writable.write(chunk)` возвращает `false`, когда внутренний буфер превышает `highWaterMark`. В этот момент вызывайте `readable.pause()`, а на событии `writable.once('drain', ...)` — `readable.resume()`. На `readable.on('end')` зовите `writable.end()`, и резолвите промис по событию `'finish'`.",
      `Это и есть тот самый паттерн, ради которого существуют \`pipe\`/\`pipeline\` — без обработки backpressure при быстром producer и медленном consumer внутренний буфер \`writable\` растёт неограниченно, и Node «утекает» по памяти, пока процесс не убьёт OOM-killer. Именно поэтому в продакшене вместо ручной развязки \`data\`/\`drain\` обычно используют \`stream.pipeline\` — он сам корректно подписывается на ошибки, гасит оба стрима при падении и не оставляет утечек обработчиков.

С чего начать:
\`\`\`js
async function createThrottledPipeline(readable, writable, bytesPerSecond) {
  return new Promise((resolve, reject) => {
    readable.on('data', (chunk) => { /* ... */ });
...
\`\`\``,
  ...
  // ваш код
    ...
    hints: [
      "Async-итератор обходится одним специальным циклом — нужно по очереди забирать из него чанки и собирать их вместе.",
      "Используйте `for await (const chunk of asyncIterable)`. Накапливайте чанки в массив, а в конце сделайте `chunks.join('')` — это эффективнее, чем `+=` для строки на больших объёмах.",
    ],
...
Типичный паттерн на бэкенде: чтение потоков логов, обработка вывода дочернего процесса (\`child_process\`) и других построчных источников данных.`,
  ...
  // ваш код
    ...
    hints: [
      "Чанки нарезаются по сети как попало, и одна JSON-строка может прийти склеенной из частей двух чанков. Нужна буферизация — копите данные, пока не наберётся целая строка.",
      "Ведите `buffer = ''`. На каждый чанк: `buffer += chunk`, потом `parts = buffer.split('\\n')` и `buffer = parts.pop()` — последняя часть всегда «хвост», который может быть неполным. Целые строки парсите через `JSON.parse`. После окончания итерации проверьте оставшийся `buffer` — если непустой, распарсите и его.",
      `Самая частая ошибка — \`JSON.parse(chunk)\` напрямую без буферизации. Сеть и \`fs.createReadStream\` режут данные по своему усмотрению, и \`chunk\` почти никогда не совпадает с одной JSON-записью: иногда это половина объекта, иногда — пять объектов сразу. Поэтому правило — **парсить только то, что заведомо завершено**, то есть всё, что находится до последнего \`\\n\`. И не глотать ошибку парсинга: одна битая строка в NDJSON-логах обычно означает повреждение данных, а не штатный случай.

С чего начать:
\`\`\`js
async function* parseNDJSON(asyncIterable) {
  let buffer = '';
  for await (const chunk of asyncIterable) {
    // ...
  }
...
Это паттерн, лежащий в основе Node.js \`stream.pipeline\` и аналогичных конвейеров.`,
  ...
  // ваш код — верните async-итератор
    ...
    hints: [
      "Конвейер из трансформ — это применение каждой к выходу предыдущей. Подумайте, как описать эту операцию одним выражением, а не вложенными вызовами.",
      "`Array.prototype.reduce` идеально подходит: стартуем с `source`, на каждой итерации делаем `transform(iter)`. Финальное значение и есть готовый итератор, по которому пользователь будет ходить через `for await`.",
      `Хитрость в том, что \`composeStreams\` сам ничего не запускает — он только собирает «лестницу» итераторов. Данные начинают течь, только когда вызывающий код делает \`for await\` по результату: каждый трансформ тянет следующее значение из своего источника через \`next()\`, тот — из своего, и так до самого \`source\`. Это и есть «pull-модель» async-итераторов — в отличие от push-модели Node.js Readable. Поэтому если ни один потребитель не читает финальный итератор, источник вообще не выполняется.

С чего начать:
\`\`\`js
function composeStreams(source, ...transforms) {

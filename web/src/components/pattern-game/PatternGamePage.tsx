...
const PROBLEMS: Problem[] = [
  ...
  {
    description:
      'Сервис аналитики Сервис логирует временные метки запросов. Нужно найти максимальное количество запросов за любое непрерывное окно в K секунд. Временные метки идут в хронологическом порядке.',
  ...
  {
    description:
      'Модерация Сервис проверяет тексты объявлений. Дана строка, нужно определить, является ли она палиндромом, игнорируя пробелы и регистр. Решение должно работать за O(1) дополнительной памяти.',
    correctAnswer: 'two-pointers',
    explanation:
  ...
  {
    description:
      'В системе рекомендаций Сервис нужно найти самую длинную непрерывную подстроку без повторяющихся символов в строке запроса пользователя. Строка может содержать любые ASCII-символы.',
    correctAnswer: 'sliding-window',
    explanation:
  ...
  {
    description:
      'Система антифрода Сервис получает поток событий. Нужно в реальном времени отслеживать медиану значений в скользящем окне последних N событий. При добавлении нового события самое старое удаляется.',
    correctAnswer: 'stacks-queues',
    explanation:

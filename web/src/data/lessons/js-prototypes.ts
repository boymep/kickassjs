...
Отличие от классических ООП-языков (Java, C++): в JavaScript нет «отдельных» классов, существующих рядом с экземплярами. Класс — это объект-прототип. Можно изменить \`Array.prototype\` в рантайме — и все массивы в программе мгновенно увидят новый метод. Это даёт огромную гибкость и одновременно создаёт целый класс багов: monkey-patching, конфликты с будущими стандартами, утечки в \`Object.prototype\`.
      ...
      blocks: [
        {
          type: 'text',
          content:
            'V8 и другие движки агрессивно оптимизируют поиск по прототипам. Объекты с одинаковой «формой» (одинаковые ключи в одинаковом порядке) делят скрытый класс (hidden class), и доступ к свойствам компилируется в простую загрузку по смещению — почти бесплатно по производительности.',
        },
        {
          type: 'list',
          content: `- Изменение прототипа существующего объекта через \`Object.setPrototypeOf\` или \`obj.__proto__ = ...\` дорого: объект «деоптимизируется» и теряет hidden class. MDN явно рекомендует **создавать объект сразу с нужным прототипом** через \`Object.create\` или литерал.

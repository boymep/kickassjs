      ...
      blocks: [
        {
          type: 'text',
          content:
            'Event loop в Node.js — это бесконечный цикл, выполняемый библиотекой **libuv** на C/C++. На каждой итерации цикл последовательно проходит шесть фаз. У каждой фазы своя очередь callback-ов, а порядок фаз фиксирован.',
        },
        {
          type: 'list',
      ...
      blocks: [
        {
          type: 'text',
          content:
            '**Starvation** — ситуация, когда одна очередь не даёт циклу перейти к следующим фазам, и часть callback-ов никогда не выполняется. В Node.js это чаще всего случается из-за рекурсивного `process.nextTick` или забивания микрозадач.',
        },
        {
          type: 'code',
          language: 'javascript',
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Главное правило сервера на Node.js — **не блокировать event loop**. Пока один callback выполняет тяжёлый цикл, никакие другие соединения не обрабатываются: новые HTTP-запросы стоят в очереди ОС, таймеры опаздывают, health-check падает.',
        },
        {
          type: 'code',
          language: 'javascript',
      ...
      blocks: [
        {
          type: 'text',
          content:
            '**EventEmitter** — базовый класс модуля `events`. На нём построены HTTP-сервер, стримы, child_process, кластер. Понимание его семантики важно, потому что `emit` ведёт себя не так, как многие ожидают.',
        },
        {
          type: 'code',
          language: 'javascript',
      ...
      blocks: [
        {
          type: 'text',
          content:
            'С Node.js 15+ необработанный rejected promise по умолчанию завершает процесс с кодом 1. Это поведение управляется флагом `--unhandled-rejections=strict|warn|throw`. На production это означает, что **любая** забытая обработка `.catch()` может уронить сервер.',
        },
        {
          type: 'code',
          language: 'javascript',

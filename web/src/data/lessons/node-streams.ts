...
await pipeline(
  ...
  fs.createWriteStream('users.csv'),
        ...
        },
        {
          ...
          calloutType: 'info',
          content:
            'На собеседовании любимый вопрос: «как прочитать 10 ГБ файл?». Правильный ответ — стримами с потоковой обработкой. `fs.readFile`/`readFileSync` для таких размеров = OOM. `pipeline` + Transform/async generator + `fs.createWriteStream` = O(highWaterMark) памяти.',
        },
      ],

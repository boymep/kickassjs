...
export const sdApiDesignLesson: Lesson = {
  ...
  intro: {
    whyItMatters: `Дизайн API определяет, насколько долго проект сможет жить и развиваться без переписываний. Плохо спроектированный API — это «сломанный контракт»: каждое изменение разрушает клиентов, каждый ретрай создаёт дубликаты, каждый запрос со ста миллионов записей кладёт базу. Хороший API наоборот: десятилетиями принимает новых клиентов, переживает миграции БД и смену команд.

В этом уроке вы научитесь выбирать **стиль API под задачу**: REST для публичных HTTP-ресурсов с кешированием на CDN, GraphQL для фронтенда, который собирает данные из десятков источников, gRPC для межсервисного взаимодействия с бинарным протоколом и стримингом, tRPC для full-stack TypeScript-монорепо с end-to-end типизацией. Разберём ключевые механики, которые отличают senior-инженера от junior: **идемпотентность** (почему POST не идемпотентен и как делает его таким Idempotency-Key), **версионирование** (URL vs header vs query, плюсы и минусы каждого), **пагинацию** (почему offset/limit ломается на миллионе записей и как его заменяет cursor), **обработку ошибок** (HTTP-коды vs envelope, RFC 7807 problem+json), **контракты** (OpenAPI, Protobuf, GraphQL SDL — single source of truth), **обратную совместимость** (какие изменения ломают клиентов, как объявлять Sunset). Без этих знаний интервьюер услышит «REST — это когда GET и POST», и собеседование закончится; с ними — вы спроектируете API, который перенесёт пять лет роста продукта.`,
    estimatedMinutes: 45,
    interviewAngle:
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Стиль API — это ответ на вопрос: **в какой форме клиент описывает свои намерения, и в какой форме сервер описывает свой контракт**. У каждого стиля своя ниша. Современная индустрия не выбирает один — крупные системы комбинируют: REST наружу, GraphQL для фронта, gRPC для межсервисного, tRPC для внутреннего тулинга.',
        },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'REST оперирует **ресурсами** — сущностями, идентифицируемыми URI. URI — существительное во множественном числе (`/users`, `/orders/42/items`), HTTP-метод — глагол. Это разделение даёт стандартную семантику кеша, ретраев и инструментов API-gateway.',
        },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'GraphQL — язык запросов и runtime для API. У него **один эндпоинт** (обычно POST /graphql) и **типизированная схема** на SDL (Schema Definition Language). Клиент сам описывает, какие поля он хочет, и получает ровно их — это решает классические проблемы REST: over-fetching (лишние поля) и under-fetching (несколько запросов вместо одного).',
        },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Любой API живёт дольше своего автора. Через год после релиза у вас будут клиенты, которых вы не контролируете: мобильные приложения с медленным циклом обновления, third-party интеграции, скрипты пользователей. Версионирование — это контракт о том, **что вы можете менять, не ломая клиентов**.',
        },
      ...
      blocks: [
        {
          type: 'text',
          content:
            'Пагинация — то, что отделяет API, переживший рост, от API, который кладёт БД на тысячном пользователе. Существует два основных подхода: **offset/limit** (просто, но не масштабируется) и **cursor** (сложнее, но устойчиво ко всему).',
        },
